...
class Update extends Group_Controller {
    ...
    function index_post(){
        ...
        $cek = $this->db->where('LOWER(alasan)',strtolower($alasan))->where('id!=',$id)->get('mst_sipk_alasan');
        if( $cek->num_rows()>0 ){
            $this->response([
                'status'=>false,
        ...
        $this->db->where('id',$id)->update('mst_sipk_alasan',[
            'alasan'=>$alasan,
        ...
        $this->response([
            'status'=>true,
            'message'=>"Alasan berhasil diupdate",
            'data'=>$data
