...
class Tambah_hak_akses extends Group_Controller {
    ...
    function index_post(){
        ...
        if( $cek->num_rows()>0 ){
            $row = $cek->row_array();
            if ($row['no_pokok']==null || $row['no_pokok']=='') $arr_np = array();
        	else $arr_np = explode(',', $row['no_pokok']);

            array_push($arr_np, $this->post('no_pokok'));
