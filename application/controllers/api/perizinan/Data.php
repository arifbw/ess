...
//            ], MY_Controller::HTTP_BAD_REQUEST);
            ...
            /*if(empty($this->get('jenis_perizinan'))){
                ...
                $no=0;
                $get_data_izin = $this->perizinan->get_perizinan($params)->result();
                foreach($get_data_izin as $tampil){
                    ...
                    $row['sumber_data'] = $tampil->is_machine=='1'?'Mesin Pamlek':'Input sistem';
                ...
                $this->response([
                    'status'=>true,
                    'message'=>'Data '.$jenis_perizinan->nama.' bulan '.id_to_bulan($bulan)." ".$tahun,
                    'data'=>$data
...
//    }
        ...
        try {
            ...
            if(empty($this->get('bulan'))){
                ...
                foreach($get_data_izin as $tampil){
                    ...
                    // $row['data_pos'] = json_decode($tampil->approval_pengamanan_posisi);
                    $row['data_pos'] = implode(",", array_column($this->db->where_in("id",json_decode($tampil->pos))->get('mst_pos')->result_array(), 'nama'));

                    $row['approval_1_np'] = $tampil->approval_1_np!=null ? $tampil->approval_1_np : $tampil->request_approval_1_np;
                    $row['approval_1_nama'] = $tampil->approval_1_nama!=null ? $tampil->approval_1_nama : $tampil->request_approval_1_nama;
                    $row['approval_2_np'] = $tampil->approval_2_np!=null ? $tampil->approval_2_np : $tampil->request_approval_2_np;
                    $row['approval_2_nama'] = $tampil->approval_2_nama!=null ? $tampil->approval_2_nama : $tampil->request_approval_2_nama;
    ...
    function index_post(){
        ...
        try {
            ...
            } else if(empty($this->post('approval_1_np'))){
                $this->response([
                    'status'=>false,
