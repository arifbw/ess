...
class Update extends Group_Controller {
    ...
    function index_post(){
        ...
        if($cek->num_rows()==1){
            ...
            if ($pengamanan!=null) {
                ...
                foreach ($get_posisi as $val) {
                    if($val->created==$this->post('created')){
                        $val->pos = $this->post('pos');
                        $val->nama_pos = $this->db->where('id', $this->post('pos'))->get('mst_pos')->row()->nama;
                        $val->waktu = $this->post('waktu');
                        $val->posisi = $this->post('posisi');
                    }
            ...
            $this->response([
                'status'=>true,
                'message'=>'Approval keamanan telah diupdate',
                'data'=>[]
