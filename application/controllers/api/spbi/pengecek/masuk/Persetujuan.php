...
class Persetujuan extends Group_Controller {
    ...
    function index_post(){
        if(empty($this->post('id'))){ 
            ...
            $data_pos = [
                'id'=>$new_id,
                'ess_permohonan_spbi_id'=>$this->post('id'),
            ...
            if($this->db->affected_rows()>0){
                ...
                if( count($kondisi_barang) > 0 ) $this->db->insert_batch('ess_permohonan_spbi_kondisi_barang',(array) $kondisi_barang);

                # update _pos
                $this->db->where('id',$new_id)->update('ess_permohonan_spbi_pos',['kondisi_barang_json'=>json_encode($kondisi_barang), 'barang_sesuai'=>($count_tidak_lengkap > 0 ? '2':'1')]);

                $approval_pengamanan_posisi = $this->db->where('ess_permohonan_spbi_id',$this->post('id'))->order_by('tanggal','DESC')->order_by('jam','DESC')->get('ess_permohonan_spbi_pos')->result_array();
                $this->db->where('id',$this->post('id'))->update('ess_permohonan_spbi',[
