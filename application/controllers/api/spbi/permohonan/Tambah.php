...
class Tambah extends Group_Controller {
    ...
    function index_post(){
        ...
        $last_counter = $this->db->select('MAX(no_urut) AS counter')->where("DATE_FORMAT(created_at,'%Y')", date('Y'))->where("approval_atasan_kode_unit", $this->post('approval_atasan_kode_unit'))->get('ess_permohonan_spbi')->row_array()['counter'];
        ...
        if($this->db->affected_rows()>0){
			...
			if( count($data_barang)>0 ) $this->db->insert_batch('ess_permohonan_spbi_barang',$data_barang);
