...
class Export_excel extends CI_Controller {
	...
	function get_by_header($kode=null){
		...
		$cek_header = $this->db->where('kode', $kode)->get('ess_faskar_ponsel_header');
        ...
        $filename = "Data-Pemakaian-Pulsa-$bulan_pemakaian.xlsx";
        ...
        //nama file
        header("Content-Disposition: attachment; filename=Data-Pemakaian-Pulsa-$bulan_pemakaian.xlsx");
        header('Cache-Control: max-age=0');

        $excel = PHPExcel_IOFactory::createReader('Excel2007');
        $excel = $excel->load('./asset/excel-templates-faskar/template-export-faskar-ponsel.xlsx');

		$this->db->from('ess_faskar_ponsel_detail');
        $this->db->where('deleted_at is null');
		$this->db->where('faskar_ponsel_header_id', $header->id);
		$get = $this->db->get();
		...
		$excel->getActiveSheet()->setCellValueExplicit('A1', "PEMAKAIAN PULSA BULAN ".strtoupper(Ym_to_MY($header->pemakaian_bulan)), PHPExcel_Cell_DataType::TYPE_STRING);
        ...
        foreach($get->result() as $row){
			$excel->getActiveSheet()->getStyle('E'.$awal.':I'.$awal)->getNumberFormat()->setFormatCode("#,##0");
            ...
            $excel->getActiveSheet()->setCellValue('G'.$awal, $row->roaming_ln);
