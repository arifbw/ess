...
class Detail extends CI_Controller {
	public function __construct(){
		...
		$this->folder_view = 'faskar/pam/';
		$this->folder_model = 'faskar/pam/';
		$this->folder_controller = 'faskar/pam/';
		
		$this->akses = array();
		
		$this->data["is_with_sidebar"] = true;
		
		$this->data['judul'] = "Air dari PAM";
	...
	public function data($kode){
		$cek_kode = $this->db->where('kode', $kode)->get('ess_faskar_pam_header');
		if($cek_kode->num_rows()!=1){
			$this->session->set_flashdata('failed', 'Kode tidak valid');
			redirect('faskar/pam/header');
		} else{
			$header = $cek_kode->row();
			$bulan = [
				['id'=>'01', 'value'=>'Januari'],
	...
	function action_insert(){
		...
		if(@$this->input->post('kode')){
			...
			$this->db->where('kode',$this->input->post('kode'))->update('ess_faskar_pam_detail', $data_insert);
			$this->session->set_flashdata('success',"Data pemakaian PAM a.n <b>{$data_insert['nama_karyawan']}</b> telah diupdate");
		} else{ # ini untuk kondisi input => tidak ada atribute 'kode'
			$cek = $this->db->where([
				'np_karyawan'=>$data_insert['np_karyawan'],
				'faskar_pam_header_id'=>$data_insert['faskar_pam_header_id']
			])->get('ess_faskar_pam_detail');
			if($cek->num_rows()>0){
				...
				$this->db->insert('ess_faskar_pam_detail', $data_insert);
				$this->session->set_flashdata('success',"Data pemakaian PAM a.n <b>{$data_insert['nama_karyawan']}</b> berhasil ditambahkan");
			}
		}

		redirect('faskar/pam/detail/data/'.$this->input->post('header_kode'));
	}

	function hapus(){
		$kode = $this->input->post('kode');
		$this->db->where('kode',$kode)->delete('ess_faskar_pam_detail');
		echo json_encode([
			'status'=>true,
	...
	function action_import(){
		$faskar_pam_header_id = $this->input->post('faskar_pam_header_id');
		$header_kode = $this->input->post('header_kode');
		include APPPATH.'third_party/phpexcel/PHPExcel.php';
		$config['upload_path'] = './uploads/faskar/pam/';
		...
		if(!$this->upload->do_upload('berkas')){
			...
			foreach($sheet as $row){
				if($numrow > 7 && $row['B']!='') {
					$cek = $this->db->where(array('np_karyawan'=>$row['B'], 'faskar_pam_header_id'=>$faskar_pam_header_id))->get('ess_faskar_pam_detail');
					$cek_in_batch = array_search($row['B'], array_column($data_batch, 'np_karyawan'));
					if ($cek->num_rows() == 0 && $cek_in_batch===false) { # hanya insert yg belum pernah diinput
						
						$pemakaian = $row['F']!='' ? $row['F']:0;
						$data = array(
							'kode' => $this->uuid->v4(),
							'faskar_pam_header_id' => $faskar_pam_header_id,
							'np_karyawan' => $row['B'],
						...
						# cek ke plafon
						$cek_plafon = $this->db->where('deleted_at IS NULL',null,false)->where('np_karyawan',$row['B'])->get('mst_plafon_pam');
						# cek ke mst karyawan
			...
			if($data_batch!=[]) $this->db->insert_batch('ess_faskar_pam_detail', $data_batch);
			$flash = 'success';
			$status = 200;
		...
		redirect('faskar/pam/detail/data/'.$header_kode);
	}
}