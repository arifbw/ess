...
class Detail extends CI_Controller {
	public function __construct(){
		...
		$this->folder_view = 'faskar/ponsel/';
		$this->folder_model = 'faskar/ponsel/';
		$this->folder_controller = 'faskar/ponsel/';
	...
	function action_insert(){
		...
		if(@$this->input->post('kode')){
			...
			$this->db->where('kode',$this->input->post('kode'))->update('ess_faskar_ponsel_detail', $data_insert);
			$this->session->set_flashdata('success',"Data pemakaian pulsa a.n <b>{$data_insert['nama_karyawan']}</b> telah diupdate");
		} else{ # ini untuk kondisi input => tidak ada atribute 'kode'
			$cek = $this->db->where([
				'np_karyawan'=>$data_insert['np_karyawan'],
				'no_hp'=>$data_insert['no_hp'],
				'faskar_ponsel_header_id'=>$data_insert['faskar_ponsel_header_id']
			])->get('ess_faskar_ponsel_detail');
			if($cek->num_rows()>0){
				...
				$this->db->insert('ess_faskar_ponsel_detail', $data_insert);
				$this->session->set_flashdata('success',"Data pemakaian pulsa a.n <b>{$data_insert['nama_karyawan']}</b> berhasil ditambahkan");
			}
		}

		redirect('faskar/ponsel/detail/data/'.$this->input->post('header_kode'));
	}

	function hapus(){
		$kode = $this->input->post('kode');
		$this->db->where('kode',$kode)->delete('ess_faskar_ponsel_detail');
		echo json_encode([
			'status'=>true,
	...
	function action_import(){
		$faskar_ponsel_header_id = $this->input->post('faskar_ponsel_header_id');
		$header_kode = $this->input->post('header_kode');
		include APPPATH.'third_party/phpexcel/PHPExcel.php';
		$config['upload_path'] = './uploads/faskar/ponsel/';
		...
		if(!$this->upload->do_upload('berkas')){
			...
			foreach($sheet as $row){
				if($numrow > 7 && $row['B']!='') {
					$cek = $this->db->where(array('np_karyawan'=>$row['B'], 'no_hp'=>$row['D'], 'faskar_ponsel_header_id'=>$faskar_ponsel_header_id))->get('ess_faskar_ponsel_detail');
					...
					if ($cek->num_rows() == 0 && count($cek_in_batch)==0) { # hanya insert yg belum ada datanya
						...
						$roaming_ln = $row['F']!='' ? $row['F']:0;
						$data = array(
							'kode' => $this->uuid->v4(),
							'faskar_ponsel_header_id' => $faskar_ponsel_header_id,
							'np_karyawan' => $row['B'],
							// 'nama_karyawan' => $nama_karyawan,
							'no_hp' => $row['D'],
							'pemakaian' => $pemakaian,
							'roaming_ln' => $roaming_ln,
							// 'plafon' => $plafon,
						...
						# cek ke plafon
						$cek_plafon = $this->db->where('deleted_at IS NULL',null,false)->where('np_karyawan',$row['B'])->where('no_hp',$row['D'])->get('mst_plafon_ponsel');
						# cek ke mst karyawan
						...
						if( $cek_plafon->num_rows()==1 ){
							...
							if( $row_plafon->ket=='at cost'){
								...
								$data['beban_perusahaan'] = $pemakaian + $roaming_ln;
							} else{
								$plafon = $row_plafon->plafon;
								if( $pemakaian >= $plafon ){
									$selisih = $pemakaian - $plafon;
									$beban_pegawai = abs( $roaming_ln - $selisih );
									$data['beban_pegawai'] = $beban_pegawai;
									$data['beban_perusahaan'] = $plafon + $roaming_ln;
								} else{
									$data['beban_pegawai'] = 0;
									$data['beban_perusahaan'] = $pemakaian + $roaming_ln;
								}
							...
							$data['plafon'] = 0;
							$data['beban_pegawai'] = $pemakaian + $roaming_ln;
			...
			if($data_batch!=[]) $this->db->insert_batch('ess_faskar_ponsel_detail', $data_batch);
			$flash = 'success';
			$status = 200;
		...
		redirect('faskar/ponsel/detail/data/'.$header_kode);
	}
