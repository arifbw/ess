...
class Detail extends CI_Controller {
	public function __construct(){
		...
		$this->folder_view = 'faskar/tv/';
		$this->folder_model = 'faskar/tv/';
		$this->folder_controller = 'faskar/tv/';
	...
	public function data($kode){
		$cek_kode = $this->db->where('kode', $kode)->get('ess_faskar_tv_header');
		if($cek_kode->num_rows()!=1){
			$this->session->set_flashdata('failed', 'Kode tidak valid');
			redirect('faskar/tv/header');
		} else{
			$header = $cek_kode->row();
			$bulan = [
				['id'=>'01', 'value'=>'Januari'],
	...
	public function tabel_tv() {
		$params=[];
		$params['header_id'] = $_POST['header_id'];
		$this->load->model($this->folder_model."M_tabel_tv_detail");
		$list = $this->M_tabel_tv_detail->get_datatables($params);
		$data = array();
		$no = $_POST['start'];

		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->M_tabel_tv_detail->count_all($params),
			"recordsFiltered" => $this->M_tabel_tv_detail->count_filtered($params),
			"data" => $list
		);
	...
	function action_insert(){
		...
		if(@$this->input->post('kode')){
			...
			$this->db->where('kode',$this->input->post('kode'))->update('ess_faskar_tv_detail', $data_insert);
			$this->session->set_flashdata('success',"Data pemakaian tv a.n <b>{$data_insert['nama_karyawan']}</b> telah diupdate");
		} else{ # ini untuk kondisi input => tidak ada atribute 'kode'
			$cek = $this->db->where([
				'np_karyawan'=>$data_insert['np_karyawan'],
				'no_id'=>$data_insert['no_id'],
				'faskar_tv_header_id'=>$data_insert['faskar_tv_header_id']
			])->get('ess_faskar_tv_detail');
			if($cek->num_rows()>0){
				...
				$this->db->insert('ess_faskar_tv_detail', $data_insert);
				$this->session->set_flashdata('success',"Data pemakaian tv a.n <b>{$data_insert['nama_karyawan']}</b> berhasil ditambahkan");
			}
		}

		redirect('faskar/tv/detail/data/'.$this->input->post('header_kode'));
	}

	function hapus(){
		$kode = $this->input->post('kode');
		$this->db->where('kode',$kode)->delete('ess_faskar_tv_detail');
		echo json_encode([
			'status'=>true,
	...
	function action_import(){
		$faskar_tv_header_id = $this->input->post('faskar_tv_header_id');
		$header_kode = $this->input->post('header_kode');
		include APPPATH.'third_party/phpexcel/PHPExcel.php';
		$config['upload_path'] = './uploads/faskar/tv/';
		...
		if(!$this->upload->do_upload('berkas')){
			...
			foreach($sheet as $row){
				if($numrow > 7 && $row['B']!='') {
					$cek = $this->db->where(array('np_karyawan'=>$row['B'], 'no_id'=>$row['D'], 'faskar_tv_header_id'=>$faskar_tv_header_id))->get('ess_faskar_tv_detail');
					$cek_in_batch = array_multi_search($data_batch, array('np_karyawan' => $row['B'], 'no_id' => $row['D']));
					if ($cek->num_rows() == 0 && count($cek_in_batch)==0) { # hanya insert yg belum ada datanya
						$pemakaian = $row['E']!='' ? $row['E']:0;
						$data = array(
							'kode' => $this->uuid->v4(),
							'faskar_tv_header_id' => $faskar_tv_header_id,
							'np_karyawan' => $row['B'],
						...
						# cek ke plafon
						$cek_plafon = $this->db->where('deleted_at IS NULL',null,false)->where('np_karyawan',$row['B'])->where('no_id',$row['D'])->get('mst_plafon_tv');
						# cek ke mst karyawan
			...
			if($data_batch!=[]) $this->db->insert_batch('ess_faskar_tv_detail', $data_batch);
			$flash = 'success';
			$status = 200;
		...
		redirect('faskar/tv/detail/data/'.$header_kode);
	}
