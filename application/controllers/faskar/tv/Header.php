...
class Header extends CI_Controller {
	...
	public function index(){
		...
		$this->data["ref_layanan"] = $this->db/*->where('status', '1')*/->get('mst_tv_kabel')->result_array();
	...
	public function tabel_header() {
		$this->load->model($this->folder_model."M_tabel_tv_header");
		$list = $this->M_tabel_tv_header->get_datatables();
		$data = array();
		$no = $_POST['start'];

		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->M_tabel_tv_header->count_all(),
			"recordsFiltered" => $this->M_tabel_tv_header->count_filtered(),
			"data" => $list
		);
	...
	/*function save_new_header(){
		...
		if($cek->num_rows()>0){
			...
			$this->db->insert('ess_faskar_tv_header', $data_insert);
	...
	function save_new_header(){
		...
		$cek = $this->db->get('ess_faskar_tv_header');
		if($cek->num_rows()>0){
			$this->session->set_flashdata('failed','Data sudah pernah diinput');
		} else{
			if(@$this->input->post('kode')){
				...
				$this->db->where('kode', $this->input->post('kode'))->update('ess_faskar_tv_header', $data_insert);
	...
	function ajukan(){
		...
		$this->load->view('faskar/tv/ajukan', [
			'header'=>$header
		]);
	}
	...
	function save_ajukan(){
		...
		$this->db->where('id', $this->input->post('id',true))->update('ess_faskar_tv_header', $data);
		echo json_encode([
			'status'=>true,
