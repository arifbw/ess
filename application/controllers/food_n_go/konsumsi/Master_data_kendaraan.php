<?php
	...
	class Master_data_kendaraan extends CI_Controller {
		...
		public function index(){
			...
			$this->data['content'] = $this->folder_view."master_data_kendaraan";
			
			array_push($this->data['js_sources'],"food_n_go/kendaraan/master_data_kendaraan");
		...
		private function ubah($nama,$nama_ubah,$nopol_ubah,$status_ubah){
			...
			if($cek["status"]){
				$set = array("nama" => $nama_ubah, "nopol"=>$nopol_ubah, "status"=>$status_ubah);
				
				$arr_data_lama = $this->m_master_data_kendaraan->data_jenis_kendaraan($nama);
				$log_data_lama = "";
				...
				$this->m_master_data_kendaraan->ubah($set,$nama);

				if($this->m_master_data_kendaraan->cek_hasil_jenis_kendaraan($nama_ubah,$nopol_ubah,$status_ubah)){
					$return["status"] = true;
