<?php
	...
	class Pemesanan_makan_siang extends CI_Controller {
		public function __construct(){
			...
			$this->load->model($this->folder_model."m_pemesanan_makan_siang");
		
			$this->data["is_with_sidebar"] = true;
								
			$this->data['judul'] = "Konsumsi Makan Siang";
		...
		{			
			...
			$this->data['content'] = $this->folder_view."pemesanan_makan_siang";
            ...
            $get_month = $this->db->select('tanggal_awal')->group_by('YEAR(tanggal_awal), MONTH(tanggal_awal)')->order_by('YEAR(tanggal_awal) DESC, MONTH(tanggal_awal) DESC')->get($nama_db.'.ess_pemesanan_makan_siang')->result();
            
            foreach($get_month as $row){
                $array_tahun_bulan[] = date('m-Y', strtotime($row->tanggal_awal));
            }
				
			//ambil mst dws	
			
			$array_jadwal_kerja 	= $this->m_pemesanan_makan_siang->select_mst_karyawan_aktif();
			$array_daftar_karyawan	= $this->m_pemesanan_makan_siang->select_daftar_karyawan();
			$array_daftar_unit		= $this->m_pemesanan_makan_siang->select_daftar_unit();
		...
		{			
			...
			if($_SESSION["grup"]==4) //jika Pengadministrasi Unit Kerja
			{
				...
				$np_list=array();
				$np_list=$this->m_pemesanan_makan_siang->select_np_by_kode_unit($list_kode_unit);						
		...
		{
			$this->load->model($this->folder_model."/M_tabel_pemesanan_makan_siang");
			...
			$list = $this->M_tabel_pemesanan_makan_siang->get_datatables($tampil_bulan_tahun);
			$data = array();
			$no = $_POST['start'];
			foreach ($list as $tampil) {
				...
				$row[] = tanggal_indonesia($tampil->tanggal_awal).' s/d '.tanggal_indonesia($tampil->tanggal_akhir);
				$row[] = $tampil->jenis_makanan;		
				$row[] = $tampil->diet;		
				$row[] = $tampil->gilir;
				$row[] = $tampil->jumlah;
				if($this->akses["ubah"]) {						
					$row[] = "<a class='btn btn-primary btn-xs edit_button' data-toggle='modal' data-target='#modal_ubah'>Ubah</a>";
				} else {
					$row[] = "<a class='btn btn-primary btn-xs edit_button' data-toggle='modal' data-target='#modal_ubah'>Lihat</a>";
				}
			...
			$output = array(
							"draw" => $_POST['draw'],
							"recordsTotal" => $this->M_tabel_pemesanan_makan_siang->count_all($tampil_bulan_tahun),
							"recordsFiltered" => $this->M_tabel_pemesanan_makan_siang->count_filtered($tampil_bulan_tahun),
							"data" => $data,
					);
		...
		public function action_insert_data_kehadiran()
		{
			...
			{					
				...
				$dws = $this->m_pemesanan_makan_siang->select_mst_jadwal_kerja_by_id($insert_dws_id);
				$dws_name			= $dws['dws'];
				...
				$exist	= $this->m_pemesanan_makan_siang->check_cico_exist($data_exist);
				...
				$insert = $this->m_pemesanan_makan_siang->insert_data_kehadiran($data_insert);
				...
				}else
				{
					...
					//===== Log Start =====
					$arr_data_baru = $this->m_pemesanan_makan_siang->select_kehadiran_by_id($insert,$tahun_bulan);					
					$log_data_baru = "";					
		...
		{			
			...
			{					
				...
				//===== Log Start =====
				$arr_data_lama = $this->m_pemesanan_makan_siang->select_kehadiran_by_id($id,$tahun_bulan);
				$log_data_lama = "";				
				...
				$update = $this->m_pemesanan_makan_siang->update_data_kehadiran($data_update);
				...
				}else
				{
					...
					//===== Log Start =====
					$arr_data_baru = $this->m_pemesanan_makan_siang->select_kehadiran_by_id($id,$tahun_bulan);
					$log_data_baru = "";					
					foreach($arr_data_baru as $key => $value){
						if(strcmp($key,"id")!=0){
							if(!empty($log_data_baru)){
		...
		{			
			...
			{					
				...
				$update = $this->m_pemesanan_makan_siang->update_kode_unit($data_update);
		...
		public function cetak() {
			...
			$this->load->model($this->folder_model."/M_tabel_pemesanan_makan_siangs");
			...
			if($_SESSION["grup"]==4) { //jika Pengadministrasi Unit Kerja		
				...
				$var = 1;
			$get_data = $this->M_tabel_pemesanan_makan_siangs->_get_excel($var,$tampil_bulan_tahun,$set);
		...
		public function cetak_per_unit() {
			...
			if($_SESSION["grup"]==4) { //jika Pengadministrasi Unit Kerja		
				...
				$var = 1;
			$get_data = $this->M_tabel_pemesanan_makan_siangs->_get_excel_per_unit($var,$tampil_bulan_tahun,$set);
	        error_reporting(E_ALL);
	        ini_set('display_errors', TRUE);
	        ini_set('display_startup_errors', TRUE);
