<?php
	...
	class Daftar_diet extends CI_Controller {
		public function __construct(){
			...
			$this->load->model($this->folder_model."/m_master_data_diet");
		...
		public function index(){
			...
			$this->data['content'] = $this->folder_view."diet";
			
			array_push($this->data['js_sources'],"food_n_go/master_data/daftar_diet");
			...
			if($this->input->post()){
				if(!strcmp($this->input->post("aksi"),"tambah")){
					...
					if($tambah['status']){
						$this->data['success'] = "Diet dengan nama <b>".$this->data['nama']."</b> berhasil ditambahkan.";
					...
					if($ubah["status"]){
						$this->data['success'] = "Perubahan jenis diet berhasil dilakukan.";
					}
					else{
			...
			if($this->akses["lihat"]){
				...
				$this->data["daftar_diet"] = $this->m_master_data_diet->daftar_diet();
		...
		private function tambah($nama,$status){
			...
			if($this->m_master_data_diet->cek_tambah_daftar_diet($nama)){
				$data = array(
							"nama" => $nama,
							"status" => $status
						);
				$this->m_master_data_diet->tambah($data);
				
				if($this->m_master_data_diet->cek_hasil_daftar_diet($nama,$status)){
					$return["status"] = true;
					
					$arr_data_insert = $this->m_master_data_diet->daftar_diet($nama);
					...
					$return["status"] = false;
					$return["error_info"] = "Penambahan Jenis Diet <b>Gagal</b> Dilakukan.";
				}
			}
			else{
				$return["status"] = false;
				$return["error_info"] = "Jenis Diet dengan nama <b>$nama</b> sudah ada.";
			}
			return $return;
		}
		...
		private function ubah($data_update){
			$return = array("status" => false, "error_info" => "");
			//$cek = $this->m_master_data_diet->cek_ubah_daftar_diet($nama,$nama_ubah);
			$cek = $this->m_master_data_diet->cek_ubah_daftar_diet($data_update);
			if($cek["status"]){
				$set = array("nama" => $data_update['nama_ubah'], "status"=>$data_update['status_ubah']);
				
				$arr_data_lama = $this->m_master_data_diet->daftar_diet($data_update['nama']);
				$log_data_lama = "";
				...
				$this->m_master_data_diet->ubah($set,$data_update['id']);

				if($this->m_master_data_diet->cek_hasil_daftar_diet($data_update['nama_ubah'],$data_update['status_ubah'])){
					$return["status"] = true;
					...
					$return["status"] = false;
					$return["error_info"] = "Perubahan Jenis Diet <b>Gagal</b> Dilakukan.";
				}
			}
			else{
