<?php
	...
	class Daftar_katering extends CI_Controller {
		...
		public function index(){
			$this->data['judul'] = "Daftar Penyedia Makanan";
			...
			if($this->input->post()){
				...
				if(!strcmp($this->input->post("aksi"),"tambah")){
                    ...
                    $insert['nama_penyedia'] = $this->input->post('nama_penyedia');
					...
					if($tambah['status']){
						$this->data['success'] = "Lokasi <b>".$insert['nama_penyedia']."</b> berhasil ditambahkan.";
						$this->data['panel_tambah'] = "";
						$this->data['nama_penyedia'] = "";
						$this->data['lokasi'] = "";
						$this->data['status'] = "";
					}
					else{
					...
					$this->data['nama_penyedia_ubah'] = $this->input->post("nama_penyedia");
					...
					if($ubah["status"]){
						$this->data['success'] = "Perubahan Penyedia Makanan Berhasil Dilakukan.";
					}
					else{
			...
			if($this->akses["lihat"]){
				$js_header_script = "
							...
							<script>
								$(document).ready(function() {
									$('#tabel_daftar_penyedia').DataTable({
										responsive: true
									});
									$('.select2').select2();
								});
				...
				$this->data["daftar_penyedia"] = $this->M_master_data_katering->daftar_penyedia();
				$this->data["daftar_lokasi"] = $this->M_master_data_katering->daftar_lokasi();
		...
		private function tambah($data)
		{
			...
			$this->M_master_data_katering->insert($data);
			$id_ = $this->db->insert_id();

			if($this->M_master_data_katering->cek_daftar_penyedia($id_)){
				$return["status"] = true;
				...
				$return["status"] = false;
				$return["error_info"] = "Penambahan Penyedia <b>Gagal</b> Dilakukan.";
			}
		...
		private function ubah($data_update){
			...
			$cek = $this->M_master_data_katering->cek_daftar_penyedia($data_update['id']);
			if($cek){
				$set = array(
					"nama_penyedia" => $data_update['nama_penyedia'],
				...
				$update = $this->M_master_data_katering->update($set, $data_update['id']);
				...
				if($update){
					...
					$return["status"] = false;
					$return["error_info"] = "Perubahan Penyedia <b>Gagal</b> Dilakukan.";
				}
			}
			else{
		...
		public function detail($id)
		{
			...
			$data = $this->M_master_data_katering->cek_daftar_penyedia($id);
