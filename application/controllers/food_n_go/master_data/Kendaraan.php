<?php
	...
	class Kendaraan extends CI_Controller {
		...
		public function index(){
			...
			if($this->input->post()){
				if(!strcmp($this->input->post("aksi"),"tambah")){
					...
					$this->data['id_mst_bbm'] = $this->input->post("id_mst_bbm");
					$this->data['bbm'] = $this->input->post("nama_mst_bbm");
					...
					$tambah = $this->tambah($this->data['nama'],$this->data['nopol'],$this->data['status'],$this->data['id_mst_bbm'],$this->data['id_mst_bbm']);
					...
					$this->data['id_mst_bbm'] = $this->input->post("id_mst_bbm_old");
					$this->data['id_mst_bbm_ubah'] = $this->input->post("id_mst_bbm_ubah");
					$this->data['bbm'] = $this->input->post("nama_mst_bbm_old");
					$this->data['bbm_ubah'] = $this->input->post("nama_mst_bbm_ubah");
                    ...
                    $data_update = [
                        ...
                        'id_mst_bbm'=>$this->input->post("id_mst_bbm_old"),
                        'id_mst_bbm_ubah'=>$this->input->post("id_mst_bbm_ubah"),
                        'bbm'=>$this->input->post("nama_mst_bbm_old"),
                        'bbm_ubah'=>$this->input->post("nama_mst_bbm_ubah"),
		...
		private function tambah($nama,$nopol,$status,$id_mst_bbm,$bbm){
			$return = array("status" => false, "error_info" => "");
			if($this->m_master_data_kendaraan->cek_tambah_jenis_kendaraan($nama)){
				$data = array(
							"nama" => $nama,
							"nopol" => $nopol,
							"id_mst_bbm" => $id_mst_bbm,
							"bbm" => $bbm,
							"status" => $status
						);
				...
				if($this->db->affected_rows()>0){
					...
					$arr_data_insert = $this->m_master_data_kendaraan->data_jenis_kendaraan_new($new_id);
		...
		private function ubah($data_update){
			...
			if($cek["status"]){
				$set = array("nama" => $data_update['nama_ubah'], "nopol"=>$data_update['nopol_ubah'], "status"=>$data_update['status_ubah'], "id_mst_bbm"=>$data_update['id_mst_bbm_ubah'], "bbm"=>$data_update['bbm_ubah']);
