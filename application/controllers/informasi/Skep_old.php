<?php
	...
	class Skep extends CI_Controller {
		...
		// 		$row[] = "<button class='btn btn-primary btn-xs btn-block lihat_button' data-toggle='modal' data-target='#modal_lihat' data-id='$encrypted_txt_1' onclick='tampil_rincian(this)'>Lihat</button>";
		// 		$row[] = "<button class='btn btn-primary btn-xs btn-block lihat_button' data-toggle='modal' data-target='#modal_lihat' data-id='$encrypted_txt_2' onclick='tampil_rincian(this)'>Lihat</button>";
	...
	*/
		...
		// public function ajax_get_skep($nama_file_encrypt="")
		// {
					...
					if ($_FILES['file_umum']['name'] != '') {
		                ...
		                if ($cek == 0) {
				            ...
				            if (in_array($ekstensifile, $ekstensi)) {
				                ...
				                echo "<script>alert('UPLOAD GAGAL, File yang Anda upload harus file .pdf !');window.location='".site_url('informasi/skep')."'</script>";
				            }
				        }
			        }
			        else {
			        	echo "<script>alert('UPLOAD GAGAL, File harus diisi !');window.location='".site_url('informasi/skep')."'</script>";
			        }
				}
