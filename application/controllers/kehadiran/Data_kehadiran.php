<?php
	...
	class Data_kehadiran extends CI_Controller {
		...
		public function index()
		{
			...
			$query = $this->db->select("table_name")->where('table_schema',$nama_db)->like('table_name','ess_cico_','AFTER')->group_by('table_name')->order_by('table_name','DESC')->get('information_schema.tables');
		...
		public function tabel_data_kehadiran($tampil_bulan_tahun = null)
		{
			...
			$mst_perizinan = $this->db->get('mst_perizinan')->result_array();
			...
			foreach ($list as $tampil) {
				...
				if($tampil->id_cuti)
				{
					...
					if(@$data_cuti['is_cuti_bersama']=='1') $tampil_cuti = 'Cuti Bersama (Memotong ' . $data_cuti['uraian'] .')';
					else $tampil_cuti = $data_cuti['uraian'];
				...
				}else
				{
					...
					foreach($id_perizinan as $value)
					{
						...
						$get_mst_perizinan = search_array_by_keys($mst_perizinan, ['kode_erp','kode_pamlek'], [$izin['info_type']."|".$izin['absence_type'], $izin['kode_pamlek']]);
						if($get_mst_perizinan!=null){
							$isi=$isi."".$get_mst_perizinan['nama']."<br><br>";
						}
					}
					...
					if(!$hari_libur)
					{
						if($tampil_keterangan=='')
						{
							if ((strpos($isi, 'Izin Dinas Keluar Perusahaan Non-Pendidikan') !== false) || 
								...
								|| (strpos($isi, 'Izin Datang Terlambat') !== false) # heru pds 2021-06-05 nambah ini karena SIDT yg tidak slide masuk masih muncul TM
							) 
							{								
				...
				if($tampil->wfh==1)
				{
					if($tampil->is_dinas_luar==1)
						$tampil_keterangan = "<mark>Izin Dinas Luar (WFA)</mark>"."<br><br>".$tampil_keterangan;
					else
				...
				{					
					...
					//08 07 2022, Tri WIbowo 7648 Jika time nya 00:00 kemungkinan kosong dan jika sidt maka ga usah tampil 00:00 nya
					if($tapping_1_value_time=="00:00" && $ada_sidt=='1')
					{
						$tapping_1_value_time = "";
					}
					...
					//wina tambah cek ubah kehadiran
					//$batas = strtotime("+7 day", strtotime($tampil->dws_tanggal));	
					//19 01 2022 Tri Wibowo 7648 - matikan karena belum persetujuan user
					$batas = strtotime("+90 day", strtotime($tampil->dws_tanggal));	
					
					if($this->akses["ubah kode unit"]) 
					{				
						if (date('Y-m-d', $batas) >= date('Y-m-d')) {		
							$btn_ubah = "<a class='btn btn-primary btn-xs edit_button' data-toggle='modal' data-target='#modal_ubah'
							data-id='$tampil->id'
							data-np-karyawan='$tampil->np_karyawan'
							data-nama='$tampil->nama'
							data-dws-tanggal='".date('d-m-Y', strtotime($tampil->dws_tanggal))."'
							data-batas='".date('d-m-Y', $batas)."'
		...
		//30 01 2023 - Tri Wibowo, menanggapi 5484 memakai pake NP atasan bukan seharusnya 6347
		//duplikat dari controller Pilih_approval->ajax_getNama_approval
		public function getNama_approval($np_karyawan,$np_karyawan_atasan)
		{				
		...
		{			
			...
			{					
				...
				//30 01 2023 - Tri Wibowo, menanggapi 5484 memakai pake NP atasan bukan seharusnya 6347
				//validasi Atasan
				$data_atasan = $this->getNama_approval($np_karyawan,$np_approval_array);
				if($data_atasan=="false")
				{
					$this->session->set_flashdata('warning',"Gagal, Silahkan isi No. Pokok Atasan Dengan Benar.");				
					redirect(base_url($this->folder_controller.'data_kehadiran'));	
				}
				//validasi ketika tapping keluar lebih kecil dari tapping masuk
		...
		public function cetak() {
			...
			foreach ($get_data as $tampil) {
				...
				$excel->getActiveSheet()->setCellValueExplicit('F'.$awal, $tampil->tapping_terminal_1, PHPExcel_Cell_DataType::TYPE_STRING);
				$excel->getActiveSheet()->setCellValueExplicit('H'.$awal, $tampil->tapping_terminal_2, PHPExcel_Cell_DataType::TYPE_STRING);
				...
				if($tapping_1)
	            	$excel->getActiveSheet()->setCellValueExplicit('G'.$awal, ucwords(tanggal_indonesia(substr($tapping_1,0,10))." ".substr($tapping_1,10,6)." ".$machine_id_1), PHPExcel_Cell_DataType::TYPE_STRING);
				else
				...
				if($tapping_2)
	            	$excel->getActiveSheet()->setCellValueExplicit('I'.$awal, ucwords(tanggal_indonesia(substr($tapping_2,0,10))." ".substr($tapping_2,10,6)."  ".$machine_id_2), PHPExcel_Cell_DataType::TYPE_STRING);
				else
	            	$excel->getActiveSheet()->setCellValueExplicit('I'.$awal, '', PHPExcel_Cell_DataType::TYPE_STRING);
				...
				$hari_cuti_bersama = $this->db->query("SELECT * FROM ess_cuti_bersama WHERE tanggal_cuti_bersama='$tampil->dws_tanggal' AND np_karyawan='$tampil->np_karyawan' LIMIT 1")->row_array();
				$get_cuti = $this->db->query("SELECT ess_cuti.*, mst_cuti.*
					FROM ess_cuti
					JOIN mst_cuti ON ess_cuti.absence_type = mst_cuti.kode_erp
					WHERE ess_cuti.id = '$tampil->id_cuti'
					LIMIT 1
				")->row_array();				
				//7648 Tri Wibowo, 6 Januari 2019 - ketika sudah di pembatalan maka tidak tampil
				$hari_pembatalan =  $this->db->query("SELECT * FROM ess_pembatalan_cuti WHERE is_cuti_bersama='1' AND date='$tampil->dws_tanggal' AND np_karyawan='$tampil->np_karyawan' LIMIT 1")->row_array();
				...
				if($tampil->id_cuti) {
					if($tampil_keterangan=='')
						if (!empty($get_cuti)) {
							$tampil_keterangan = $get_cuti['uraian'];
						} else {
				...
				$excel->getActiveSheet()->setCellValueExplicit('J'.$awal, $tampil_keterangan, PHPExcel_Cell_DataType::TYPE_STRING);
	            $awal += 1;	
			}
		...
		public function cetak_all_unit_date_range() {
			// debug_start_timer();
			$this->load->library('phpexcel'); 
			$this->load->model($this->folder_model."/M_tabel_data_kehadiran");
			...
			$tampil_bulan_tahun = date('Y_m', strtotime($start_date));
            
            if( in_array($_SESSION["grup"], [1,3]) ){
                $this->db->select('kode_unit');
                // $this->db->from('mst_karyawan');
                $this->db->from("ess_cico_{$tampil_bulan_tahun}");
                $this->db->group_by('kode_unit');
                $this->db->order_by('kode_unit');
                $query_daftar_unit = $this->db->get();
            } else{
			...
			if($_SESSION["grup"]==4) { //jika Pengadministrasi Unit Kerja		
				...
				$var = 1;
			$get_data = $this->M_tabel_data_kehadiran->_get_excel_per_unit_date_range($var,$tampil_bulan_tahun,$set, $param_date);
	        error_reporting(E_ALL);
	        ini_set('display_errors', TRUE);
	        ini_set('display_startup_errors', TRUE);
			...
			foreach ($get_data as $tampil) {
				...
				if($tampil->id_cuti) {
					...
					foreach($id_perizinan as $value) {
						...
						// $kode_erp = $izin['info_type']."|".$izin['absence_type'];
						// $nama_perizinan=nama_perizinan_by_kode_erp($kode_erp);
						// if($nama_perizinan) $isi=$isi."".$nama_perizinan."<br><br>";
					...
					if(!$hari_libur) {
						if($tampil_keterangan=='') {
							if ((strpos($isi, 'Izin Dinas Keluar Perusahaan Non-Pendidikan') !== false) || (strpos($isi, 'Izin Dinas Keluar Perusahaan Pendidikan / Non-Pendidikan') !== false) || (strpos($isi, 'Izin Dinas Luar Lokasi Pendidikan') !== false) || (strpos($isi, 'Izin Pribadi dengan Potongan') !== false) || (strpos($isi, 'Izin Pribadi Tanpa Potongan') !== false) || (strpos($isi, 'Izin Datang Terlambat') !== false)) {
								$tampil_keterangan =$isi;
							}
							else {	
								if($tampil->keterangan)
