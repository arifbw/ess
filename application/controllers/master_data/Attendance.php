<?php
	...
	class Attendance extends CI_Controller {
		public function __construct(){
			...
			$this->load->model($this->folder_model."/m_attendance");
		...
		public function index(){
			...
			$this->data['content'] = $this->folder_view."attendance";
			
			array_push($this->data['js_sources'],"master_data/attendance");
			...
			if($this->input->post()){
				if(!strcmp($this->input->post("aksi"),"tambah")){
					...
					if($tambah['status']){
						$this->data['success'] = "attendance dengan nama <b>".$this->data['nama']."</b> berhasil ditambahkan.";
					...
					if($ubah["status"]){
						$this->data['success'] = "Perubahan attendance berhasil dilakukan.";
					}
					else{
			...
			if($this->akses["lihat"]){
				...
				$this->data["daftar_attendance"] = $this->m_attendance->daftar_attendance();
		...
		private function tambah($nama,$keterangan,$kode_pamlek,$kode_erp,$status){
			...
			if($this->m_attendance->cek_tambah_attendance($nama)){
				$data = array(
							"nama" => $nama,
				...
				$this->m_attendance->tambah($data);
				
				if($this->m_attendance->cek_hasil_attendance($nama,$keterangan,$kode_pamlek,$kode_erp,$status)){
					$return["status"] = true;
					
					$arr_data_insert = $this->m_attendance->data_attendance($nama);
					...
					$return["status"] = false;
					$return["error_info"] = "Penambahan attendance <b>Gagal</b> Dilakukan.";
				}
			}
			else{
				$return["status"] = false;
				$return["error_info"] = "attendance dengan nama <b>$nama</b> sudah ada.";
			}
			return $return;
		}
		...
		private function ubah($nama,$nama_ubah,$keterangan_ubah,$kode_pamlek_ubah,$kode_erp_ubah,$status_ubah){
			$return = array("status" => false, "error_info" => "");
			$cek = $this->m_attendance->cek_ubah_attendance($nama,$nama_ubah);
			if($cek["status"]){
				...
				$arr_data_lama = $this->m_attendance->data_attendance($nama);
				$log_data_lama = "";
				...
				$this->m_attendance->ubah($set,$nama);

				if($this->m_attendance->cek_hasil_attendance($nama_ubah,$keterangan_ubah,$kode_pamlek_ubah,$kode_erp_ubah,$status_ubah)){
					$return["status"] = true;
					...
					$return["status"] = false;
					$return["error_info"] = "Perubahan attendance <b>Gagal</b> Dilakukan.";
				}
			}
			else{
	...
	/* Location: ./application/controllers/master_data/attendance.php */