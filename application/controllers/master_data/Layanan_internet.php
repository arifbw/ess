<?php
	...
	class Layanan_internet extends CI_Controller {
		public function __construct(){
			...
			$this->load->model($this->folder_model."/M_layanan_internet");
		...
		public function index(){
			$this->data['judul'] = "Layanan Internet";
			...
			$this->data['content'] = $this->folder_view."layanan_internet";
			...
			if($this->input->post()){
				...
				if(!strcmp($this->input->post("aksi"),"tambah")){
					...
					if($tambah['status']){
						$this->data['success'] = "Layanan internet <b>".$insert['nama']."</b> berhasil ditambahkan.";
					...
					if($ubah["status"]){
						$this->data['success'] = "Perubahan Layanan Internet Berhasil Dilakukan.";
					}
					else{
			...
			if($this->akses["lihat"]){
				$js_header_script = "
							<script>
								$(document).ready(function() {
									$('#tabel_daftar_layanan_internet').DataTable({
										responsive: true
									});
								});
							</script>";
				...
				$this->data["daftar_layanan_internet"] = $this->M_layanan_internet->daftar_layanan_internet();
		...
		private function tambah($data)
		{
			...
			$this->M_layanan_internet->insert($data);
			$id_ = $this->db->insert_id();

			if($this->M_layanan_internet->cek_daftar_layanan_internet($id_)){
				$return["status"] = true;
				...
				$return["status"] = false;
				$return["error_info"] = "Penambahan Layanan Internet <b>Gagal</b> Dilakukan.";
			}
		...
		private function ubah($data_update){
			...
			$cek = $this->M_layanan_internet->cek_daftar_layanan_internet($data_update['no']);
			if($cek){
				$set = array(
				...
				$update = $this->M_layanan_internet->update($set, $data_update['no']);
				...
				if($update){
					...
					$return["status"] = false;
					$return["error_info"] = "Perubahan Layanan Internet <b>Gagal</b> Dilakukan.";
				}
			}
			else{
		...
		public function detail($id)
		{
			...
			$data = $this->M_layanan_internet->cek_daftar_layanan_internet($id);
