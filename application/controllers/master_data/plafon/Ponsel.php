<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Ponsel extends CI_Controller {
        ...
        public function index() {
			...
			$this->data['content'] 					= $this->folder_view."ponsel";
        ...
        /*public function tabel_ponsel() {
            $this->load->model($this->folder_model."M_tabel_ponsel");
			...
			$list 	= $this->M_tabel_ponsel->get_datatables($var);
			$data = array();
			$no = $_POST['start'];
            ...
            $output = array(
                "draw" => $_POST['draw'],
                "recordsTotal" => $this->M_tabel_ponsel->count_all($var),
                "recordsFiltered" => $this->M_tabel_ponsel->count_filtered($var),
                "data" => $data,
			);
        ...
        function action_insert_ponsel() {
            ...
            if($this->akses["tambah"]) {
				...
				if ($this->form_validation->run() == FALSE) {
					...
					redirect(base_url($this->folder_controller.'ponsel'));
				}
                else {
                    ...
                    if($submit) {
                        ...
                        if ($this->input->post('edit_id', true)!='') {
                            ...
                            $this->db->set($data_insert)->where('id', $this->input->post('edit_id', true))->update('ess_laporan_ponsel');
                        } else {
                            $data_insert['created_at'] = date('Y-m-d H:i:s');
                            $data_insert['created_by'] = $_SESSION['no_pokok'];
                            $this->db->set($data_insert)->insert('ess_laporan_ponsel');
                        }
        ...
        public function view_detail()
		{
			...
			$this->load->view($this->folder_view."detail_ponsel",$data);
		}*/

		public function tabel_ponsel() {
			$this->load->model($this->folder_model."M_plafon_ponsel");
			$list = $this->M_plafon_ponsel->get_datatables();
			$data = array();
			$no = $_POST['start'];
	
			$output = array(
				"draw" => $_POST['draw'],
				"recordsTotal" => $this->M_plafon_ponsel->count_all(),
				"recordsFiltered" => $this->M_plafon_ponsel->count_filtered(),
				"data" => $list
			);
			echo json_encode($output);
		}
		...
		function action_insert(){
			...
			if(@$this->input->post('id')){
				$data_insert['updated_at'] = date('Y-m-d H:i:s');
				$data_insert['updated_by'] = $_SESSION['no_pokok'];
				$this->db->where('id',$this->input->post('id'))->update('mst_plafon_ponsel', $data_insert);
				$this->session->set_flashdata('success',"Master plafon pulsa a.n <b>{$data_insert['nama_karyawan']}</b> telah diupdate");
			} else{ # ini untuk kondisi input => tidak ada atribute 'id'
				$cek = $this->db->where([
					'np_karyawan'=>$data_insert['np_karyawan'],
					'no_hp'=>$data_insert['no_hp']
				])->get('mst_plafon_ponsel');
				if($cek->num_rows()>0){
					$row = $cek->row();
					if($row->deleted_at==null)
						...
						$this->db->where('id',$row->id)->update('mst_plafon_ponsel', $data_insert);
						$this->session->set_flashdata('success',"Master plafon pulsa a.n <b>{$data_insert['nama_karyawan']}</b> berhasil ditambahkan");
					}
				} else{
					$data_insert['created_at'] = date('Y-m-d H:i:s');
					$data_insert['created_by'] = $_SESSION['no_pokok'];
					$this->db->insert('mst_plafon_ponsel', $data_insert);
					$this->session->set_flashdata('success',"Master plafon pulsa a.n <b>{$data_insert['nama_karyawan']}</b> berhasil ditambahkan");
				}
			}

			redirect('master_data/plafon/ponsel');
		}

		function hapus(){
			$id = $this->input->post('id');
			$this->db->where('id',$id)->update('mst_plafon_ponsel', [
