<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Tv extends CI_Controller {
		...
		public function __construct(){
			...
			$this->data['judul'] = "Plafon TV Kabel";
        ...
        public function index() {
			...
			$this->data["ref_layanan"] = $this->db->get('mst_tv_kabel')->result_array();
			$this->data["navigasi_menu"] 			= menu_helper();
			$this->data['content'] 					= $this->folder_view."tv";
		...
		public function tabel_tv() {
			$this->load->model($this->folder_model."M_plafon_tv");
			$list = $this->M_plafon_tv->get_datatables();
			$data = array();
			$no = $_POST['start'];
	
			$output = array(
				"draw" => $_POST['draw'],
				"recordsTotal" => $this->M_plafon_tv->count_all(),
				"recordsFiltered" => $this->M_plafon_tv->count_filtered(),
				"data" => $list
			);
		...
		function action_insert(){
			...
			if(@$this->input->post('id')){
				...
				$this->db->where('id',$this->input->post('id'))->update('mst_plafon_tv', $data_insert);
				$this->session->set_flashdata('success',"Master plafon TV Kabel a.n <b>{$data_insert['nama_karyawan']}</b> telah diupdate");
				...
				if($cek->num_rows()>0){
					...
					if($row->deleted_at==null)
						...
						$this->db->where('np_karyawan',$this->input->post('np_karyawan'))->update('mst_plafon_tv', $data_insert);
						$this->session->set_flashdata('success',"Master plafon TV Kabel a.n <b>{$data_insert['nama_karyawan']}</b> berhasil ditambahkan");
					}
				} else{
					$data_insert['created_at'] = date('Y-m-d H:i:s');
					$data_insert['created_by'] = $_SESSION['no_pokok'];
					$this->db->insert('mst_plafon_tv', $data_insert);
					$this->session->set_flashdata('success',"Master plafon TV Kabel a.n <b>{$data_insert['nama_karyawan']}</b> berhasil ditambahkan");
				}
			}

			redirect('master_data/plafon/tv');
		}

		function hapus(){
			$id = $this->input->post('id');
			$this->db->where('id',$id)->update('mst_plafon_tv', [
