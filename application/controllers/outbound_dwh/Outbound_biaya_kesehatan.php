    ...
    function read_process($file){
        ...
        while (($row = fgetcsv($handle,0,'|'))) {
            ...
            try{
                ...
                $cekExist = $this->db
                    ->where('bill_no',addslashes(@$row[0]))
                    ->where('np_karyawan',addslashes(@$row[1]))
                    ->where('tgl_berobat',addslashes(@$row[24]))
        ...
        # heru nambahin ini, kalau sudah selesai diproses=>file didelete, 2021-05-17
        exec("rm -rf /var/www/html/ess/outbound_dwh/sikesper/dwh_hr_hcm_biaya_kesehatan/$file");
        # echo 'Done';
    }
