<?php
	...
	class Outbound_jabatan extends CI_Controller {
		 
		function __construct(){
			parent::__construct();
			//$this->load->helper('jabatan');
			$this->load->model("outbound_ess/m_outbound_jabatan");
		...
		public function generate_jabatan($jenis){
			...
			echo "Mulai ambil data jabatan dengan jenis format : $jenis\n";
			...
			jenis = 1 ==> nde
			*/
			$data = $this->m_outbound_jabatan->generate_jabatan($jenis);
		...
		private function create_file($jenis,$content){
			...
			if ( ! write_file(FCPATH . "outbound_ess/jabatan/$jenis.txt", $content)){
				echo 'Gagal membuat file txt';
			}
			else{
				echo "File outbound_ess/jabatan/$jenis.txt"."\n";
			}
