<?php
	...
	class Outbound_karyawan extends CI_Controller {
		 
		function __construct(){
			parent::__construct();
			//$this->load->helper('karyawan');
			$this->load->model("outbound_ess/m_outbound_karyawan");
		...
		public function generate_karyawan($jenis){
			...
			echo "Mulai ambil data karyawan dengan jenis format : $jenis\n";
			...
			jenis = 3 ==> nde
			*/
			$data = $this->m_outbound_karyawan->generate_karyawan($jenis);
		...
		private function create_file($jenis,$content){
			...
			if ( ! write_file(FCPATH . "outbound_ess/karyawan/$jenis.txt", $content)){
				echo 'Gagal membuat file txt';
			}
			else{
				echo "File outbound_ess/karyawan/$jenis.txt"."\n";
			}
