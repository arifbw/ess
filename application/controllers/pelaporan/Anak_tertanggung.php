<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Anak_tertanggung extends CI_Controller {
		...
		public function __construct(){
			...
			$this->data['judul'] = "Laporan Anak Tertanggung";
        ...
        public function index() {
			...
			$this->data['content'] 					= $this->folder_view."anak_tertanggung";
        ...
        public function tabel_anak_tertanggung() {
			...
			$list 	= $this->M_tabel_anak_tertanggung->get_datatables($var);
			$data = array();
			$no = $_POST['start'];
            ...
            $output = array(
                "draw" => $_POST['draw'],
                "recordsTotal" => $this->M_tabel_anak_tertanggung->count_all($var),
                "recordsFiltered" => $this->M_tabel_anak_tertanggung->count_filtered($var),
                "data" => $data,
			);
        ...
        function action_insert_anak_tertanggung() {
            ...
            if($this->akses["tambah"]) {
				...
				if (($this->input->post('edit_id',true)=='' && ($_FILES['akta_kelahiran']['tmp_name'] == '' || $_FILES['surat_keterangan']['tmp_name'] == '' || $_FILES['kk']['tmp_name'] == '')) || $this->form_validation->run() == FALSE) {
                    ...
                    if($submit) {
						...
						if ($_FILES['akta_kelahiran']['tmp_name'] != '') {
							//Surat Keterangan
							$config['upload_path'] = './uploads/pelaporan/anak_tertanggung/akta_kelahiran';
							...
							if($files['akta_kelahiran']['name']) {
								...
								$error = "File Akta Kelahiran Tidak Ditemukan";
							}
						}

						if ($_FILES['surat_keterangan']['tmp_name'] != '') {
							$config['upload_path'] = './uploads/pelaporan/anak_tertanggung/surat_keterangan';
							$config['allowed_types'] = 'pdf|jpg|png|jpeg';
							$config['max_size']	= '2148';
							$files = $_FILES;
						...
						if ($_FILES['kk']['tmp_name'] != '') {
							$config['upload_path'] = './uploads/pelaporan/anak_tertanggung/kk';
							$config['allowed_types'] = 'pdf|jpg|png|jpeg';
							$config['max_size']	= '2148';
							$files = $_FILES;
							...
							if($files['kk']['name']) {
								...
								$error = "File Kartu Keluarga Tidak Ditemukan";
							}
						}
						...
						if ($error=="") {
                            ...
                            if ($this->input->post('edit_id', true)!='') {
								$data_lama = $this->db->where('id', $this->input->post('edit_id', true))->get('ess_laporan_anak_tertanggung')->row();

                                $data_insert['updated_at'] = date('Y-m-d H:i:s');
                                $data_insert['updated_by'] = $_SESSION['no_pokok'];
                                $this->db->set($data_insert)->where('id', $this->input->post('edit_id', true))->update('ess_laporan_anak_tertanggung');
                            } else {
                                $data_insert['created_at'] = date('Y-m-d H:i:s');
                                $data_insert['created_by'] = $_SESSION['no_pokok'];
                                $this->db->set($data_insert)->insert('ess_laporan_anak_tertanggung');
                            }
                            
                            if ($this->db->affected_rows() > 0) {
								if ($this->input->post('edit_id', true)!='') {
									if (@$akta_kelahiran)
										unlink('./uploads/pelaporan/anak_tertanggung/akta_kelahiran/'.$data_lama->akta_kelahiran);
									if (@$kk)
										unlink('./uploads/pelaporan/anak_tertanggung/kk/'.$data_lama->kk);
									if (@$surat_keterangan)
										unlink('./uploads/pelaporan/anak_tertanggung/surat_keterangan/'.$data_lama->surat_keterangan);
								}
                            ...
                            redirect(base_url($this->folder_controller.'anak_tertanggung'));
						} else {
		...
		public function hapus($id=null, $np=null) {
            $this->load->model($this->folder_model."M_pelaporan");
			if(@$id != null && @$np != null) {
                $get = $this->M_pelaporan->ambil_by_id($id, 'ess_laporan_anak_tertanggung');
                $this->db->where('id', $id)->set(array('deleted_at'=>date('Y-m-d H:i:s'), 'deleted_by'=>$_SESSION['no_pokok']))->update("ess_laporan_anak_tertanggung");
				...
				if($this->db->affected_rows() > 0) {
                    ...
                    $return['status'] = true;
                    $return['msg'] = 'Laporan Anak Tertanggung Berhasil Dihapus';
                } else {
					$return['status'] = false;
                    $return['msg'] = 'Laporan Anak Tertanggung Gagal Dihapus';
                }
            } else {
				$return['status'] = false;
				$return['msg'] = 'Laporan Anak Tertanggung Gagal Dihapus';
