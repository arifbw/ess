...
class Anak_tidak_tertanggung extends CI_Controller {
	...
	public function __construct() {
		...
		$this->data['judul'] = "Laporan Anak Tidak Tertanggung";
	...
	public function index() {
		...
		$this->data['content'] 					= $this->folder_view."anak_tidak_tertanggung";
	...
	public function tabel_anak_tidak_tertanggung() {
		$this->load->model($this->folder_model."M_tabel_anak_tidak_tertanggung");
			...
			$list 	= $this->M_tabel_anak_tidak_tertanggung->get_datatables($var);
			$data = array();
			$no = $_POST['start'];
			foreach ($list as $tampil) {
				...
				if ($tampil->approval_status==null || $tampil->approval_status=='0') {
					...
					if($this->akses["tambah"]) {
						...
						$edit['alasan'] = trim($tampil->alasan);
			...
			$output = array(
				"draw" => $_POST['draw'],
				"recordsTotal" => $this->M_tabel_anak_tidak_tertanggung->count_all($var),
				"recordsFiltered" => $this->M_tabel_anak_tidak_tertanggung->count_filtered($var),
				"data" => $data,
			);
		...
		function action_insert_anak_tidak_tertanggung() {
            ...
            if($this->akses["tambah"]) {
				...
				if ($this->input->post('edit_id',true)=='' && $this->form_validation->run() == FALSE) {
                    ...
                    if($submit) {
						...
						if ($this->input->post('edit_id', true)!='') {
							...
							$this->db->set($data_insert)->where('id', $this->input->post('edit_id', true))->update('ess_laporan_anak_tidak_tertanggung');
						} else {
							$data_insert['created_at'] = date('Y-m-d H:i:s');
							$data_insert['created_by'] = $_SESSION['no_pokok'];
							$this->db->set($data_insert)->insert('ess_laporan_anak_tidak_tertanggung');
						}
						
						if ($this->db->affected_rows() > 0) {
							$this->session->set_flashdata('success',"Berhasil Update Laporan Anak Tidak Tertanggung");
						}
						else {
							$this->session->set_flashdata('warning',"Gagal Update Laporan Anak Tidak Tertanggung");
						}
						redirect(base_url($this->folder_controller.'anak_tidak_tertanggung'));
                    } else{
		...
		public function hapus($id=null, $np=null) {
            $this->load->model($this->folder_model."M_pelaporan");
			if(@$id != null && @$np != null) {
                $get = $this->M_pelaporan->ambil_by_id($id, 'ess_laporan_anak_tidak_tertanggung');
                $this->db->where('id', $id)->set(array('deleted_at'=>date('Y-m-d H:i:s'), 'deleted_by'=>$_SESSION['no_pokok']))->update("ess_laporan_anak_tidak_tertanggung");
				...
				if($this->db->affected_rows() > 0) {
                    ...
                    $return['status'] = true;
                    $return['msg'] = 'Laporan Anak Tidak Tertanggung Berhasil Dihapus';
                } else {
					$return['status'] = false;
                    $return['msg'] = 'Laporan Anak Tidak Tertanggung Gagal Dihapus';
                }
            } else {
				$return['status'] = false;
				$return['msg'] = 'Laporan Anak Tidak Tertanggung Gagal Dihapus';
