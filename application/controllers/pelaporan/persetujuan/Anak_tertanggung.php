<?php
	...
	class Anak_tertanggung extends CI_Controller {
		public function __construct(){
			...
			$this->data['judul'] = "Persetujuan Anak Tertanggung";
		...
		public function index() {
			...
			$this->data['content'] 					= $this->folder_view."persetujuan/anak_tertanggung";
		...
		public function tabel_anak_tertanggung() {
			...
			$list 	= $this->M_tabel_anak_tertanggung->get_datatables(null, 'persetujuan');
			$data = array();
			$no = $_POST['start'];
			...
			$output = array(
				"draw" => $_POST['draw'],
				"recordsTotal" => $this->M_tabel_anak_tertanggung->count_all(null, 'persetujuan'),
				"recordsFiltered" => $this->M_tabel_anak_tertanggung->count_filtered(null, 'persetujuan'),
				"data" => $data,
			);
        ...
        public function view_approve() {
			...
			if($lap['approval_status']=='3') {
				$data['sdm_status'] = "Laporan Anak Tertanggung <b>TELAH DIVERIFIKASI</b> SDM";
				$data['sdm_warna']= 'success';
			}else if($lap['approval_status']=='4') {
				$data['sdm_status'] = "Laporan Anak Tertanggung <b>TIDAK DISETUJUI</b> SDM";
				$data['sdm_warna'] = 'danger';
			}else if($lap['approval_status']=='5') {
				$data['sdm_status'] = "Laporan Anak Tertanggung <b>TELAH DISUBMIT</b> SDM ke ERP";
				$data['sdm_warna'] ='info';
			}
			$data["judul"] = ucwords(preg_replace("/_/"," ",__CLASS__));

			$this->load->view($this->folder_view."persetujuan/approve_anak_tertanggung",$data);
		}
		...
		public function save_approve() {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$pdd = $this->db->where('id', $id_)->get('ess_laporan_anak_tertanggung');
				if ($pdd->num_rows() == 1) {
					...
					$this->db->where('id', $id_)->set($set)->update('ess_laporan_anak_tertanggung');
				...
				$this->session->set_flashdata('warning', 'Persetujuan laporan Anak Tertanggung belum lengkap!');
			}
			redirect(site_url($this->folder_controller.'persetujuan/anak_tertanggung'));
		}
	}