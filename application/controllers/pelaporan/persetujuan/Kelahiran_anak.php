<?php
	...
	class Kelahiran_anak extends CI_Controller {
		public function __construct(){
			...
			$this->data['judul'] = "Persetujuan Kelahiran Anak";
		...
		public function index() {
			...
			$this->data['content'] 					= $this->folder_view."persetujuan/kelahiran_anak";
		...
		public function tabel_kelahiran_anak() {
			...
			$list 	= $this->M_tabel_kelahiran_anak->get_datatables(null, 'persetujuan');
			$data = array();
			$no = $_POST['start'];
			...
			$output = array(
				"draw" => $_POST['draw'],
				"recordsTotal" => $this->M_tabel_kelahiran_anak->count_all(null, 'persetujuan'),
				"recordsFiltered" => $this->M_tabel_kelahiran_anak->count_filtered(null, 'persetujuan'),
				"data" => $data,
			);
        ...
        public function view_approve() {
			...
			if($lap['approval_status']=='3') {
				$data['sdm_status'] = "Laporan Kelahiran Anak <b>TELAH DIVERIFIKASI</b> SDM";
				$data['sdm_warna']= 'success';
			}else if($lap['approval_status']=='4') {
				$data['sdm_status'] = "Laporan Kelahiran Anak <b>TIDAK DISETUJUI</b> SDM";
				$data['sdm_warna'] = 'danger';
			}else if($lap['approval_status']=='5') {
				$data['sdm_status'] = "Laporan Kelahiran Anak <b>TELAH DISUBMIT</b> SDM ke ERP";
				$data['sdm_warna'] ='info';
			}
			$data["judul"] = ucwords(preg_replace("/_/"," ",__CLASS__));

			$this->load->view($this->folder_view."persetujuan/approve_kelahiran_anak",$data);
		}
		...
		public function save_approve() {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$pdd = $this->db->where('id', $id_)->get('ess_laporan_kelahiran_anak');
				if ($pdd->num_rows() == 1) {
					...
					$this->db->where('id', $id_)->set($set)->update('ess_laporan_kelahiran_anak');
				...
				$this->session->set_flashdata('warning', 'Persetujuan laporan Kelahiran Anak belum lengkap!');
			}
			redirect(site_url($this->folder_controller.'persetujuan/kelahiran_anak'));
		}
	}