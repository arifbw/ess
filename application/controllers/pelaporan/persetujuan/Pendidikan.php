<?php
	...
	class Pendidikan extends CI_Controller {
		public function __construct(){
			...
			$this->data['judul'] = "Persetujuan Pendidikan";
		...
		public function index()
		{
			...
			$this->data['content'] 					= $this->folder_view."persetujuan/pendidikan";
		...
		public function tabel_pendidikan() {
			...
			$list 	= $this->M_tabel_pendidikan->get_datatables(null, 'persetujuan');
			$data = array();
			$no = $_POST['start'];
			...
			$output = array(
					"draw" => $_POST['draw'],
					"recordsTotal" => $this->M_tabel_pendidikan->count_all(null, 'persetujuan'),
					"recordsFiltered" => $this->M_tabel_pendidikan->count_filtered(null, 'persetujuan'),
					"data" => $data,
			);
		...
		{			
			...
			if($lap['approval_status']=='3') {
				$data['sdm_status'] = "Laporan pendidikan <b>TELAH DIVERIFIKASI KAUN SDM</b>";
				$data['sdm_warna']= 'success';
			}else if($lap['approval_status']=='4') {
				$data['sdm_status'] = "Laporan pendidikan <b>TIDAK DISETUJUI KAUN SDM</b>";
				$data['sdm_warna'] = 'danger';
			}else if($lap['approval_status']=='5') {
				$data['sdm_status'] = "Laporan pendidikan <b>TELAH DISUBMIT SDM</b> ke ERP";
				$data['sdm_warna'] ='info';
			}else if($lap['approval_status']=='5') {
				$data['sdm_status'] = "Laporan pendidikan <b>DITOLAK ADMIN SDM</b> ke ERP";
				$data['sdm_warna'] ='info';
			}
			$data["judul"] = ucwords(preg_replace("/_/"," ",__CLASS__));

			$this->load->view($this->folder_view."persetujuan/approve_pendidikan",$data);
		}
		...
		public function save_approve() {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$pdd = $this->db->where('id', $id_)->get('ess_laporan_pendidikan');
				if ($pdd->num_rows() == 1) {
					...
					$this->db->where('id', $id_)->set($set)->update('ess_laporan_pendidikan');

					if ($this->db->affected_rows() > 0) {
						// $izin_new = $this->db->where('id', $id_)->get('ess_laporan_pendidikan')->row_array();
				...
				$this->session->set_flashdata('warning', 'Persetujuan laporan pendidikan belum lengkap!');
			}
			redirect(site_url($this->folder_controller.'persetujuan/pendidikan'));
		}
	}
