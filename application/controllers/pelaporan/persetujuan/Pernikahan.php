<?php
	...
	class Pernikahan extends CI_Controller {
		public function __construct(){
			...
			$this->data['judul'] = "Persetujuan Pernikahan";
		...
		public function index() {
			...
			$this->data['content'] 					= $this->folder_view."persetujuan/pernikahan";
		...
		public function tabel_pernikahan() {
			...
			$list 	= $this->M_tabel_pernikahan->get_datatables(null, 'persetujuan');
			$data = array();
			$no = $_POST['start'];
			...
			$output = array(
				"draw" => $_POST['draw'],
				"recordsTotal" => $this->M_tabel_pernikahan->count_all(null, 'persetujuan'),
				"recordsFiltered" => $this->M_tabel_pernikahan->count_filtered(null, 'persetujuan'),
				"data" => $data,
			);
        ...
        public function view_approve() {
			...
			if($lap['approval_status']=='3') {
				$data['sdm_status'] = "Laporan Pernikahan <b>TELAH DIVERIFIKASI</b> SDM";
				$data['sdm_warna']= 'success';
			}else if($lap['approval_status']=='4') {
				$data['sdm_status'] = "Laporan Pernikahan <b>TIDAK DISETUJUI</b> SDM";
				$data['sdm_warna'] = 'danger';
			}else if($lap['approval_status']=='5') {
				$data['sdm_status'] = "Laporan Pernikahan <b>TELAH DISUBMIT</b> SDM ke ERP";
				$data['sdm_warna'] ='info';
			}
			$data["judul"] = ucwords(preg_replace("/_/"," ",__CLASS__));

			$this->load->view($this->folder_view."persetujuan/approve_pernikahan",$data);
		}
		...
		public function save_approve() {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$this->session->set_flashdata('warning', 'Persetujuan laporan Pernikahan belum lengkap!');
			}
			redirect(site_url($this->folder_controller.'persetujuan/pernikahan'));
		}
	}