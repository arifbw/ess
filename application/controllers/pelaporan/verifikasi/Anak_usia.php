<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Anak_usia extends CI_Controller {
		...
		public function __construct(){
			...
			$this->data['judul'] = "Verifikasi Anak 18-25 Tahun";
		...
		public function index() {
			...
			$this->data['content'] 					= $this->folder_view."verifikasi/anak_usia";
		...
		public function tabel_anak_usia() {
			...
			$list 	= $this->M_tabel_anak_usia->get_datatables(null, 'verifikasi');
			$data = array();
			$no = $_POST['start'];
            foreach ($list as $tampil) {
				...
				if($get_status=='1') {
					// $approval_status 	= "Laporan Anak 18-25 Tahun TELAH DISETUJUI Atasan pada $approval_date.";

					$btn_warna		='btn-success';
					$btn_text		='Disetujui Atasan';
				}else if($get_status=='2') {
					// $approval_status 	= "Laporan Anak 18-25 Tahun TIDAK DISETUJI Atasan pada $approval_date.<br>Alasan : ".$approval_alasan; 
				...
				if($get_status=='1') {
					...
					$sdm_status 	= "Laporan Anak 18-25 Tahun DIVERIFIKASI KAUN SDM";

					$btn_warna		='btn-success';
					$btn_text		='Verifikasi KAUN SDM';
				}else if($get_status=='4') {
					$sdm_status 	= "Laporan Anak 18-25 Tahun TIDAK DISETUJUI SDM";

					$btn_warna		='btn-danger';
					$btn_text		='Ditolak KAUN SDM';
				}else if($get_status=='5') {
					$sdm_status 	= "Laporan Anak 18-25 Tahun DISUBMIT SDM ke ERP";

					$btn_warna		='btn-primary';
					$btn_text		='SUBMIT ERP';
				}else if($get_status=='6') {
					$sdm_status 	= "Laporan Anak 18-25 Tahun DITOLAK Admin SDM";
				...
				$aksi .= '<a target="_blank" href="'.base_url('pelaporan/verifikasi/anak_usia/export_pdf/').$tampil->id.'" class="btn btn-success btn-xs"><i class="fa fa-print"></i> Cetak PDF</a>';
			...
			$output = array(
					"draw" => $_POST['draw'],
					"recordsTotal" => $this->M_tabel_anak_usia->count_all(null, 'verifikasi'),
					"recordsFiltered" => $this->M_tabel_anak_usia->count_filtered(null, 'verifikasi'),
					"data" => $data,
			);
		...
		{			
			...
			//DETAIL
			if($lap['approval_status']=='1' || $lap['approval_status']=='3' || $lap['approval_status']=='4' || $lap['approval_status']=='5' || $lap['approval_status']=='6') {
				$data['approval_status'] = "Laporan anak 18-25 tahun <b>TELAH DISETUJUI</b> Atasan pada ".datetime_indo($lap['approval_date']);
				$data['approval_warna'] ='success';
				$data['sdm_warna']= 'info';
				$data['submit_warna'] = 'info';
			}else if($lap['approval_status']=='2') {
				$data['approval_status'] = "Laporan anak 18-25 tahun <b>TIDAK DISETUJI</b> Atasan pada ".datetime_indo($lap['approval_date']); 
				$data['approval_warna'] ='danger';
			} else if($lap['approval_status']=='0' || $lap['approval_status']==null) {
				$data['approval_status'] = "Menunggu Persetujuan Atasan"; 
				$data['approval_warna'] ='info';
			...
			$this->load->view($this->folder_view."verifikasi/approve_anak_usia",$data);
		}
		...
		public function save_approve() {
		    ...
		    if ($this->input->post('alasan_submit',true)=='') {
			    $this->form_validation->set_rules('id_', 'Laporan anak 18-25 tahun', 'required');
				...
				if ($this->form_validation->run() == TRUE) {
					...
					if ($pdd->num_rows() == 1) {
						...
						$this->session->set_flashdata('warning', 'Data laporan anak 18-25 tahun tidak valid!');
					}
				} else {
					$this->session->set_flashdata('warning', 'Data verifikasi laporan anak 18-25 tahun belum lengkap!');
				}
			} else {
				$this->form_validation->set_rules('id_', 'Laporan anak 18-25 tahun', 'required');
			...
			redirect(site_url($this->folder_controller.'verifikasi/anak_usia'));
		}
		...
		public function save_erp() {
			...
			$simpan = array();
		    $this->form_validation->set_rules('id_', 'Laporan laporan anak 18-25 tahun', 'required');
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if ($pdd->num_rows() == 1) {
					...
					$return['status'] = false;
					$return['msg'] = 'Data laporan laporan anak 18-25 tahun tidak valid!';
				}
			} else {
				$return['status'] = false;
				$return['msg'] = 'Data verifikasi laporan laporan anak 18-25 tahun belum lengkap!';
			}
		...
		function export_pdf($id){
			...
			$lap = $this->db->select("*")->where('id', $id)->get('ess_laporan_anak_usia')->row_array();
			$data = array(
				'data' => $lap,
				'title' => "Laporan Anak Usia 18-25 Tahun"
			);

			//DETAIL
			if($lap['approval_status']=='1' || $lap['approval_status']=='3' || $lap['approval_status']=='4' || $lap['approval_status']=='5') {
				$data['approval_status'] = "Laporan Anak Usia 18-25 Tahun <b>TELAH DISETUJUI Atasan</b> pada <b>".datetime_indo($lap['approval_date'])."</b>";
				$data['approval_warna'] ='green';
			}else if($lap['approval_status']=='2') {
				$data['approval_status'] = "Laporan Anak Usia 18-25 Tahun <b>TIDAK DISETUJI Atasan</b> pada <b>".datetime_indo($lap['approval_date'])."</b>"; 
			...
			if($lap['approval_status']=='3' || $lap['approval_status']=='5') {
				$data['sdm_status'] = "Laporan Anak Usia 18-25 Tahun <b>TELAH DIVERIFIKASI SDM</b> pada <b>".datetime_indo($lap['sdm_verif_date'])."</b>";
				$data['sdm_warna']= 'green';
			}else if($lap['approval_status']=='4') {
				$data['sdm_status'] = "Laporan Anak Usia 18-25 Tahun <b>TIDAK DISETUJUI SDM</b> pada <b>".datetime_indo($lap['sdm_verif_date'])."</b>";
				$data['sdm_warna'] = 'red';
			}

			if($lap['approval_status']=='5') {
				$data['submit_status'] = "Laporan Anak Usia 18-25 Tahun <b>TELAH DISUBMIT SDM</b> ke ERP pada <b>".datetime_indo($lap['sdm_submit_date'])."</b>";
			}

			$html=$this->load->view($this->folder_view."verifikasi/cetak_anak_usia", $data,true);
			$mpdf->WriteHTML($html);
			$mpdf->Output();
		}
	}
