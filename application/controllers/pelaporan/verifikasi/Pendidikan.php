<?php
	...
	class Pendidikan extends CI_Controller {
		public function __construct(){
			...
			$this->data['judul'] = "Verifikasi Pendidikan";
		...
		public function index()
		{
			...
			$this->data['content'] 					= $this->folder_view."verifikasi/pendidikan";
		...
		public function tabel_pendidikan() {
			...
			$list 	= $this->M_tabel_pendidikan->get_datatables(null, 'verifikasi');
			$data = array();
			$no = $_POST['start'];
			foreach ($list as $tampil) {
				...
				if($get_status=='1') {
					$approval_status 	= "Laporan Pendidikan TELAH DISETUJUI Atasan pada ".$tampil->approval_date;

					$btn_warna		='btn-success';
					$btn_text		='Disetujui Atasan';
				}else if($get_status=='2') {
					$approval_status 	= "Laporan Pendidikan TIDAK DISETUJI Atasan pada ".$tampil->approval_date."<br>Alasan : ".$approval_alasan; 
				...
				if($get_status=='1') {
					...
					$sdm_status 	= "Laporan Pendidikan DIVERIFIKASI KAUN SDM";

					$btn_warna		='btn-success';
					$btn_text		='Verifikasi KAUN SDM';
				}else if($get_status=='4') {
					$sdm_status 	= "Laporan Pendidikan TIDAK DISETUJUI SDM";

					$btn_warna		='btn-danger';
					$btn_text		='Ditolak KAUN SDM';
				}else if($get_status=='5') {
					$sdm_status 	= "Laporan Pendidikan DISUBMIT SDM ke ERP";

					$btn_warna		='btn-primary';
					$btn_text		='SUBMIT ERP';
				}else if($get_status=='6') {
					$sdm_status 	= "Laporan Pendidikan DITOLAK Admin SDM";
				...
				$aksi .= '<a target="_blank" href="'.base_url('pelaporan/verifikasi/pendidikan/export_pdf/').$tampil->id.'" class="btn btn-success btn-xs"><i class="fa fa-print"></i> Cetak PDF</a>';
			...
			$output = array(
					"draw" => $_POST['draw'],
					"recordsTotal" => $this->M_tabel_pendidikan->count_all(null, 'verifikasi'),
					"recordsFiltered" => $this->M_tabel_pendidikan->count_filtered(null, 'verifikasi'),
					"data" => $data,
			);
		...
		{			
			...
			//DETAIL
			if($lap['approval_status']=='1' || $lap['approval_status']=='3' || $lap['approval_status']=='4' || $lap['approval_status']=='5' || $lap['approval_status']=='6') {
				$data['approval_status'] = "Laporan pendidikan <b>TELAH DISETUJUI</b> Atasan pada ".datetime_indo($lap['approval_date']);
				$data['approval_warna'] ='success';
				$data['sdm_warna']= 'info';
				$data['submit_warna'] = 'info';
			}else if($lap['approval_status']=='2') {
				$data['approval_status'] = "Laporan pendidikan <b>TIDAK DISETUJI</b> Atasan pada ".datetime_indo($lap['approval_date']); 
				$data['approval_warna'] ='danger';
			} else if($lap['approval_status']=='0' || $lap['approval_status']==null) {
				$data['approval_status'] = "Menunggu Persetujuan Atasan"; 
				$data['approval_warna'] ='info';
			...
			$this->load->view($this->folder_view."verifikasi/approve_pendidikan",$data);
		}
		...
		public function save_approve() {
		    ...
		    if ($this->input->post('alasan_submit',true)=='') {
			    $this->form_validation->set_rules('id_', 'Laporan pendidikan', 'required');
				...
				if ($this->form_validation->run() == TRUE) {
					...
					if ($pdd->num_rows() == 1) {
						...
						$this->session->set_flashdata('warning', 'Data laporan pendidikan tidak valid!');
					}
				} else {
					$this->session->set_flashdata('warning', 'Data verifikasi laporan pendidikan belum lengkap!');
				}
			} else {
				$this->form_validation->set_rules('id_', 'Laporan pendidikan', 'required');
			...
			redirect(site_url($this->folder_controller.'verifikasi/pendidikan'));
		}
		...
		public function save_erp() {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if ($pdd->num_rows() == 1) {
					...
					$return['status'] = false;
					$return['msg'] = 'Data laporan pendidikan tidak valid!';
				}
			} else {
				$return['status'] = false;
				$return['msg'] = 'Data verifikasi laporan pendidikan belum lengkap!';
			}
		...
		function export_pdf($id){
			...
			$lap = $this->db->select("*")->where('id', $id)->get('ess_laporan_pendidikan')->row_array();
			$data = array(
				'data' => $lap,
				'title' => "LAPORAN MENYELESAIKAN PENDIDIKAN"
			);
			...
			if($lap['approval_status']=='3' || $lap['approval_status']=='5') {
				$data['sdm_status'] = "Laporan Pendidikan <b>TELAH DIVERIFIKASI SDM</b> pada <b>".datetime_indo($lap['sdm_verif_date'])."</b>";
				$data['sdm_warna']= 'green';
			}else if($lap['approval_status']=='4') {
				$data['sdm_status'] = "Laporan Pendidikan <b>TIDAK DISETUJUI SDM</b> pada <b>".datetime_indo($lap['sdm_verif_date'])."</b>";
				$data['sdm_warna'] = 'red';
			}

			if($lap['approval_status']=='5') {
				$data['submit_status'] = "Laporan Pendidikan <b>TELAH DISUBMIT SDM</b> ke ERP pada <b>".datetime_indo($lap['sdm_submit_date'])."</b>";
				$data['submit_warna'] = 'green';
			}

			$html=$this->load->view($this->folder_view."verifikasi/cetak_pendidikan", $data,true);
			$mpdf->WriteHTML($html);
			$mpdf->Output();
		}
	}
