<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Perceraian extends CI_Controller {
		public function __construct(){
			...
			$this->data['judul'] = "Verifikasi Perceraian";
        ...
        public function index() {
			...
			$this->data['content'] 					= $this->folder_view."verifikasi/perceraian";
        ...
        public function tabel_perceraian() {
			...
			$list 	= $this->M_tabel_perceraian->get_datatables(null, 'verifikasi');
			$data = array();
			$no = $_POST['start'];
			foreach ($list as $tampil) {
				...
				//DETAIL
				if($get_status=='1') {
					// $approval_status 	= "Laporan Perceraian TELAH DISETUJUI Atasan pada ".$tampil->approval_date;
				...
				if($get_status=='1') {
					...
					$sdm_status 	= "Laporan Perceraian DIVERIFIKASI KAUN SDM";

					$btn_warna		='btn-success';
					$btn_text		='Verifikasi KAUN SDM';
				}else if($get_status=='4') {
					$sdm_status 	= "Laporan Perceraian TIDAK DISETUJUI SDM";

					$btn_warna		='btn-danger';
					$btn_text		='Ditolak KAUN SDM';
				}else if($get_status=='5') {
					$sdm_status 	= "Laporan Perceraian DISUBMIT SDM ke ERP";

					$btn_warna		='btn-primary';
					$btn_text		='SUBMIT ERP';
				}else if($get_status=='6') {
					$sdm_status 	= "Laporan Perceraian DITOLAK Admin SDM";
				...
				$aksi .= '<a target="_blank" href="'.base_url('pelaporan/verifikasi/perceraian/export_pdf/').$tampil->id.'" class="btn btn-success btn-xs"><i class="fa fa-print"></i> Cetak PDF</a>';
			...
			$output = array(
					"draw" => $_POST['draw'],
					"recordsTotal" => $this->M_tabel_perceraian->count_all(null, 'verifikasi'),
					"recordsFiltered" => $this->M_tabel_perceraian->count_filtered(null, 'verifikasi'),
					"data" => $data,
			);
        ...
        public function view_approve() {			
			...
			//DETAIL
			if($lap['approval_status']=='1' || $lap['approval_status']=='3' || $lap['approval_status']=='4' || $lap['approval_status']=='5' || $lap['approval_status']=='6') {
				$data['approval_status'] = "Laporan perceraian <b>TELAH DISETUJUI</b> Atasan pada ".datetime_indo($lap['approval_date']);
				$data['approval_warna'] ='success';
				$data['sdm_warna']= 'info';
				$data['submit_warna'] = 'info';
			}else if($lap['approval_status']=='2') {
				$data['approval_status'] = "Laporan perceraian <b>TIDAK DISETUJI</b> Atasan pada ".datetime_indo($lap['approval_date']); 
				$data['approval_warna'] ='danger';
			} else if($lap['approval_status']=='0' || $lap['approval_status']==null) {
				$data['approval_status'] = "Menunggu Persetujuan Atasan"; 
				$data['approval_warna'] ='info';
			...
			$this->load->view($this->folder_view."verifikasi/approve_perceraian",$data);
		}
		...
		public function save_approve() {
		    ...
		    if ($this->input->post('alasan_submit',true)=='') {
			    $this->form_validation->set_rules('id_', 'Laporan perceraian', 'required');
				...
				if ($this->form_validation->run() == TRUE) {
					...
					if ($pdd->num_rows() == 1) {
						...
						$this->session->set_flashdata('warning', 'Data laporan perceraian tidak valid!');
					}
				} else {
					$this->session->set_flashdata('warning', 'Data verifikasi laporan perceraian belum lengkap!');
				}
			} else {
				$this->form_validation->set_rules('id_', 'Laporan perceraian', 'required');
			...
			redirect(site_url($this->folder_controller.'verifikasi/perceraian'));
		}
		...
		public function save_erp() {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$pdd = $this->db->where('id', $id_)->get('ess_laporan_perceraian');
				if ($pdd->num_rows() == 1) {
					$tanggal = date('Y-m-d H:i:s');
					$status = $this->input->post('status_approval',true);
					...
					$this->db->where('id', $id_)->set($set)->update('ess_laporan_perceraian');
					...
					$return['status'] = false;
					$return['msg'] = 'Data laporan Perceraian tidak valid!';
				}
			} else {
				$return['status'] = false;
				$return['msg'] = 'Data verifikasi laporan Perceraian belum lengkap!';
			}
		...
		function export_pdf($id){
			...
			$lap = $this->db->select("*")->where('id', $id)->get('ess_laporan_perceraian')->row_array();
			$data = array(
				'data' => $lap,
				'title' => "Verifikasi Perceraian"
			);
			...
			if($lap['approval_status']=='3' || $lap['approval_status']=='5') {
				$data['sdm_status'] = "Laporan Perceraian <b>TELAH DIVERIFIKASI SDM</b> pada <b>".datetime_indo($lap['sdm_verif_date'])."</b>";
				$data['sdm_warna']= 'green';
			}else if($lap['approval_status']=='4') {
				$data['sdm_status'] = "Laporan Perceraian <b>TIDAK DISETUJUI SDM</b> pada <b>".datetime_indo($lap['sdm_verif_date'])."</b>";
				$data['sdm_warna'] = 'red';
			}

			if($lap['approval_status']=='5') {
				$data['submit_status'] = "Laporan Perceraian <b>TELAH DISUBMIT SDM</b> ke ERP pada <b>".datetime_indo($lap['sdm_submit_date'])."</b>";
			}

			$html=$this->load->view($this->folder_view."verifikasi/cetak_perceraian", $data,true);
			$mpdf->WriteHTML($html);
			$mpdf->Output();
		}
    }