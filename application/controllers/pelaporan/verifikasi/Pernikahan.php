<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Pernikahan extends CI_Controller {
		public function __construct(){
			...
			$this->data['judul'] = "Verifikasi Pernikahan";
        ...
        public function index() {
			...
			$this->data['content'] 					= $this->folder_view."verifikasi/pernikahan";
        ...
        public function tabel_pernikahan() {
			...
			$list 	= $this->M_tabel_pernikahan->get_datatables(null, 'verifikasi');
			$data = array();
			$no = $_POST['start'];
			foreach ($list as $tampil) {
				...
				//DETAIL
				if($get_status=='1') {
					$approval_status = "Laporan Pernikahan TELAH DISETUJUI Atasan pada ".$tampil->approval_date;

					$btn_warna		='btn-success';
					$btn_text		='Disetujui Atasan';
				}else if($get_status=='2') {
					// $approval_status 	= "Laporan Pernikahan TIDAK DISETUJI Atasan pada ".$tampil->approval_date."<br>Alasan : ".$approval_alasan; 
				...
				if($get_status=='1') {
					...
					$sdm_status 	= "Laporan Pernikahan DIVERIFIKASI KAUN SDM";

					$btn_warna		='btn-success';
					$btn_text		='Verifikasi KAUN SDM';
				}else if($get_status=='4') {
					$sdm_status 	= "Laporan Pernikahan TIDAK DISETUJUI SDM";

					$btn_warna		='btn-danger';
					$btn_text		='Ditolak KAUN SDM';
				}else if($get_status=='5') {
					$sdm_status 	= "Laporan Pernikahan DISUBMIT SDM ke ERP";

					$btn_warna		='btn-primary';
					$btn_text		='SUBMIT ERP';
				}else if($get_status=='6') {
					$sdm_status 	= "Laporan Pernikahan DITOLAK Admin SDM";
				...
				$aksi .= '<a target="_blank" href="'.base_url('pelaporan/verifikasi/pernikahan/export_pdf/').$tampil->id.'" class="btn btn-success btn-xs"><i class="fa fa-print"></i> Cetak PDF</a>';
			...
			$output = array(
					"draw" => $_POST['draw'],
					"recordsTotal" => $this->M_tabel_pernikahan->count_all(null, 'verifikasi'),
					"recordsFiltered" => $this->M_tabel_pernikahan->count_filtered(null, 'verifikasi'),
					"data" => $data,
			);
        ...
        public function view_approve() {			
			...
			//DETAIL
			if($lap['approval_status']=='1' || $lap['approval_status']=='3' || $lap['approval_status']=='4' || $lap['approval_status']=='5' || $lap['approval_status']=='6') {
				$data['approval_status'] = "Laporan pernikahan <b>TELAH DISETUJUI</b> Atasan pada ".datetime_indo($lap['approval_date']);
				$data['approval_warna'] ='success';
				$data['sdm_warna']= 'info';
				$data['submit_warna'] = 'info';
			}else if($lap['approval_status']=='2') {
				$data['approval_status'] = "Laporan pernikahan <b>TIDAK DISETUJI</b> Atasan pada ".datetime_indo($lap['approval_date']); 
				$data['approval_warna'] ='danger';
			} else if($lap['approval_status']=='0' || $lap['approval_status']==null) {
				$data['approval_status'] = "Menunggu Persetujuan Atasan"; 
				$data['approval_warna'] ='info';
			...
			$this->load->view($this->folder_view."verifikasi/approve_pernikahan",$data);
		}
		...
		public function save_approve() {
		    ...
		    if ($this->input->post('alasan_submit',true)=='') {
			    $this->form_validation->set_rules('id_', 'Laporan pernikahan', 'required');
				...
				if ($this->form_validation->run() == TRUE) {
					...
					if ($pdd->num_rows() == 1) {
						...
						$this->session->set_flashdata('warning', 'Data laporan pernikahan tidak valid!');
					}
				} else {
					$this->session->set_flashdata('warning', 'Data verifikasi laporan pernikahan belum lengkap!');
				}
			} else {
				$this->form_validation->set_rules('id_', 'Laporan pernikahan', 'required');
			...
			redirect(site_url($this->folder_controller.'verifikasi/pernikahan'));
		}
		...
		public function save_erp() {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				$pdd = $this->db->where('id', $id_)->get('ess_laporan_pernikahan');
				if ($pdd->num_rows() == 1) {
					$tanggal = date('Y-m-d H:i:s');
					$status = $this->input->post('status_approval',true);
					...
					$this->db->where('id', $id_)->set($set)->update('ess_laporan_pernikahan');
					...
					$return['status'] = false;
					$return['msg'] = 'Data laporan Pernikahan tidak valid!';
				}
			} else {
				$return['status'] = false;
				$return['msg'] = 'Data verifikasi laporan Pernikahan belum lengkap!';
			}
		...
		function export_pdf($id){
			...
			$lap = $this->db->select("*")->where('id', $id)->get('ess_laporan_pernikahan')->row_array();
			$data = array(
				'data' => $lap,
				'title' => "Verifikasi Pernikahan"
			);
			...
			if($lap['approval_status']=='3' || $lap['approval_status']=='5') {
				$data['sdm_status'] = "Laporan Pernikahan <b>TELAH DIVERIFIKASI SDM</b> pada <b>".datetime_indo($lap['sdm_verif_date'])."</b>";
				$data['sdm_warna']= 'green';
			}else if($lap['approval_status']=='4') {
				$data['sdm_status'] = "Laporan Pernikahan <b>TIDAK DISETUJUI SDM</b> pada <b>".datetime_indo($lap['sdm_verif_date'])."</b>";
				$data['sdm_warna'] = 'red';
			}

			if($lap['approval_status']=='5') {
				$data['submit_status'] = "Laporan Pernikahan <b>TELAH DISUBMIT SDM</b> ke ERP pada <b>".datetime_indo($lap['sdm_submit_date'])."</b>";
			}

			$html=$this->load->view($this->folder_view."verifikasi/cetak_pernikahan", $data,true);
			$mpdf->WriteHTML($html);
			$mpdf->Output();
		}
    }