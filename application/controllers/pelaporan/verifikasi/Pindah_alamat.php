<?php defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Pindah_alamat extends CI_Controller {
		public function __construct(){
			...
			$this->data['judul'] = "Verifikasi Pindah Alamat";
        ...
        public function index() {
			...
			$this->data['content'] 					= $this->folder_view."verifikasi/pindah_alamat";
        ...
        public function tabel_pindah_alamat() {
			...
			$list 	= $this->M_tabel_pindah_alamat->get_datatables(null, 'verifikasi');
			$data = array();
			$no = $_POST['start'];
			foreach ($list as $tampil) {
				...
				//DETAIL
				if($get_status=='1') {
					// $approval_status 	= "Laporan Pindah Alamat TELAH DISETUJUI Atasan pada ".$tampil->approval_date;

					$btn_warna		='btn-success';
					$btn_text		='Disetujui Atasan';
				}else if($get_status=='2') {
					// $approval_status 	= "Laporan Pindah Alamat TIDAK DISETUJI Atasan pada ".$tampil->approval_date."<br>Alasan : ".$approval_alasan; 
				...
				if($get_status=='1') {
					...
					$sdm_status 	= "Laporan Pindah Alamat DIVERIFIKASI KAUN SDM";

					$btn_warna		='btn-success';
					$btn_text		='Verifikasi KAUN SDM';
				}else if($get_status=='4') {
					$sdm_status 	= "Laporan Pindah Alamat TIDAK DISETUJUI SDM";

					$btn_warna		='btn-danger';
					$btn_text		='Ditolak KAUN SDM';
				}else if($get_status=='5') {
					$sdm_status 	= "Laporan Pindah Alamat DISUBMIT SDM ke ERP";

					$btn_warna		='btn-primary';
					$btn_text		='SUBMIT ERP';
				}else if($get_status=='6') {
					$sdm_status 	= "Laporan Pindah Alamat DITOLAK Admin SDM";
				...
				$aksi .= '<a target="_blank" href="'.base_url('pelaporan/verifikasi/pindah_alamat/export_pdf/').$tampil->id.'" class="btn btn-success btn-xs"><i class="fa fa-print"></i> Cetak PDF</a>';
			...
			$output = array(
					"draw" => $_POST['draw'],
					"recordsTotal" => $this->M_tabel_pindah_alamat->count_all(null, 'verifikasi'),
					"recordsFiltered" => $this->M_tabel_pindah_alamat->count_filtered(null, 'verifikasi'),
					"data" => $data,
			);
        ...
        public function view_approve() {			
			...
			$this->load->view($this->folder_view."verifikasi/approve_pindah_alamat",$data);
		}
		...
		public function save_approve() {
		    ...
		    if ($this->input->post('alasan_submit',true)=='') {
				...
				if ($this->form_validation->run() == TRUE) {
					...
					$this->session->set_flashdata('warning', 'Data verifikasi laporan pindah alamat belum lengkap!');
				}
			} else {
				$this->form_validation->set_rules('id_', 'Laporan pindah alamat', 'required');
			...
			redirect(site_url($this->folder_controller.'verifikasi/pindah_alamat'));
		}
		...
		public function save_erp() {
			...
			if ($this->form_validation->run() == TRUE) {
				...
				if ($pdd->num_rows() == 1) {
					...
					$return['status'] = false;
					$return['msg'] = 'Data laporan pindah alamat tidak valid!';
				}
			} else {
				$return['status'] = false;
				$return['msg'] = 'Data verifikasi laporan pindah alamat belum lengkap!';
			}
		...
		function export_pdf($id){
			...
			$lap = $this->db->select("*")->where('id', $id)->get('ess_laporan_pindah_alamat')->row_array();
			$data = array(
				'data' => $lap,
				'title' => "Verifikasi Pindah Alamat"
			);
			...
			if($lap['approval_status']=='3' || $lap['approval_status']=='5') {
				$data['sdm_status'] = "Laporan Pindah Alamat <b>TELAH DIVERIFIKASI SDM</b> pada <b>".datetime_indo($lap['sdm_verif_date'])."</b>";
				$data['sdm_warna']= 'green';
			}else if($lap['approval_status']=='4') {
				$data['sdm_status'] = "Laporan Pindah Alamat <b>TIDAK DISETUJUI SDM</b> pada <b>".datetime_indo($lap['sdm_verif_date'])."</b>";
				$data['sdm_warna'] = 'red';
			}

			if($lap['approval_status']=='5') {
				$data['submit_status'] = "Laporan Pindah Alamat <b>TELAH DISUBMIT SDM</b> ke ERP pada <b>".datetime_indo($lap['sdm_submit_date'])."</b>";
			}

			$html=$this->load->view($this->folder_view."verifikasi/cetak_pindah_alamat", $data,true);
			$mpdf->WriteHTML($html);
			$mpdf->Output();
		}
    }