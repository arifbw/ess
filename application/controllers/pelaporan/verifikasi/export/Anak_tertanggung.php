...
class Anak_tertanggung extends CI_Controller {
    ...
    function excel(){
        ...
        $get = $this->db->get('ess_laporan_anak_tertanggung')->result();

        $excel = new PHPExcel();
        $excel->setActiveSheetIndex(0);

        $excel->setActiveSheetIndex(0)->mergeCells('A1:M1');
        $excel->getActiveSheet()->setCellValueExplicit('A1', "Verifikasi Laporan Anak Tertanggung", PHPExcel_Cell_DataType::TYPE_STRING);
        ...
        foreach($get as $row){
            ...
            $excel->getActiveSheet()->setCellValueExplicit('E'.$awal, $row->tempat_lahir_anak, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('F'.$awal, tanggal_indonesia($row->tanggal_lahir_anak), PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('G'.$awal, $row->status_pekerjaan, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('H'.$awal, $row->no_surat_keterangan, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('I'.$awal, $row->no_akta_kelahiran, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('J'.$awal, $row->no_kk, PHPExcel_Cell_DataType::TYPE_STRING);
        ...
        $response =  array(
            ...
            'name' => "Data Verifikasi Pelaporan Anak Tertanggung"
        );
