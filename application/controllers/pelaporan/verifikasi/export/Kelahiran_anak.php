...
class Kelahiran_anak extends CI_Controller {
    ...
    function excel(){
        ...
        $get = $this->db->get('ess_laporan_kelahiran_anak')->result();

        $excel = new PHPExcel();
        $excel->setActiveSheetIndex(0);

        $excel->setActiveSheetIndex(0)->mergeCells('A1:M1');
        $excel->getActiveSheet()->setCellValueExplicit('A1', "Verifikasi Laporan Kelahiran Anak", PHPExcel_Cell_DataType::TYPE_STRING);
        ...
        foreach($get as $row){
            ...
            $excel->getActiveSheet()->setCellValueExplicit('J'.$awal, $row->no_dokumen_lain, PHPExcel_Cell_DataType::TYPE_STRING);
        ...
        $response =  array(
            ...
            'name' => "Data Verifikasi Pelaporan Kelahiran Anak"
        );
