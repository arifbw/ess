...
class Perceraian extends CI_Controller {
    ...
    function excel(){
        ...
        $get = $this->db->get('ess_laporan_perceraian')->result();

        $excel = new PHPExcel();
        $excel->setActiveSheetIndex(0);

        $excel->setActiveSheetIndex(0)->mergeCells('A1:Q1');
        $excel->getActiveSheet()->setCellValueExplicit('A1', "Verifikasi Laporan Perceraian", PHPExcel_Cell_DataType::TYPE_STRING);
        ...
        foreach($get as $row){
            ...
            $excel->getActiveSheet()->setCellValueExplicit('J'.$awal, $row->hari_perceraian, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('K'.$awal, tanggal_indonesia($row->tanggal_perceraian), PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('L'.$awal, $row->tempat_perceraian, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('M'.$awal, $row->no_surat_cerai, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('N'.$awal, $row->no_putusan_pengadilan, PHPExcel_Cell_DataType::TYPE_STRING);
        ...
        $response =  array(
            ...
            'name' => "Data Verifikasi Pelaporan Perceraian"
        );
