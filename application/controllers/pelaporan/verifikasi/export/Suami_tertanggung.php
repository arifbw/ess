...
class Suami_tertanggung extends CI_Controller {
    ...
    function excel(){
        ...
        $get = $this->db->get('ess_laporan_suami_tertanggung')->result();

        $excel = new PHPExcel();
        $excel->setActiveSheetIndex(0);

        $excel->setActiveSheetIndex(0)->mergeCells('A1:M1');
        $excel->getActiveSheet()->setCellValueExplicit('A1', "Verifikasi Laporan Suami Tertanggung", PHPExcel_Cell_DataType::TYPE_STRING);
        ...
        $judul = array(
            ...
            'No Surat Keterangan Tidak Mendapat Fasilitas Kesehatan',
            'Keterangan',
            'Dibuat Tanggal',
            'Status'
        );
        ...
        foreach($get as $row){
            ...
            $excel->getActiveSheet()->setCellValueExplicit('D'.$awal, $row->nama_suami, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('E'.$awal, $row->tempat_lahir_suami, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('F'.$awal, tanggal_indonesia($row->tanggal_lahir_suami), PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('G'.$awal, $row->status_pekerjaan, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('H'.$awal, $row->nama_instansi, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('I'.$awal, $row->telpon_instansi, PHPExcel_Cell_DataType::TYPE_STRING);
            $excel->getActiveSheet()->setCellValueExplicit('J'.$awal, $row->no_surat_keterangan, PHPExcel_Cell_DataType::TYPE_STRING);
        ...
        $response =  array(
            ...
            'name' => "Data Verifikasi Pelaporan Suami Tertanggung"
        );
