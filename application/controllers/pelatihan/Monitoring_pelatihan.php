<?php
	...
	class Monitoring_pelatihan extends CI_Controller {
		public function __construct(){
			...
			$this->load->model($this->folder_model."m_monitoring_pelatihan");
			
			$this->data["is_with_sidebar"] = true;
			
			$this->data['judul'] = "Monitoring Kebutuhan Pelatihan";
		...
		{				
			...
			$this->data['content'] 					= $this->folder_view."monitoring_pelatihan";
		...
		public function tabel_ess_pelatihan($bulan_tahun=null,$filter='all')
		{
			...
			$this->load->model($this->folder_model."M_tabel_monitoring_pelatihan");
			
			$list 	= $this->M_tabel_monitoring_pelatihan->get_datatables($month,$filter);
			...
			foreach ($list as $tampil) {
				...
				if(($status_1=='' || $status_1=='0' || $status_1 == null) && ($status_2!='2')) //menunggu atasan 1
				{
					$btn_warna		='btn-warning';
					$btn_text		='Menunggu Atasan 1, Disetujui Atasan 2';
					$btn_disabled 	='';
				}
				if(($status_1=='' || $status_1=='0' || $status_1 == null) && ($status_2=='' || $status_2=='0' || $status_2 == null)) //menunggu atasan 1
				{
					$btn_warna		='btn-warning';
					$btn_text		='Menunggu Atasan 1 dan 2';
					$btn_disabled 	='';
				}
				...
				if($status_1=='1' && $status_2=='1') //disetujui atasan  2
				{
					$btn_warna		='btn-success';
			...
			$output = array(
							"draw" => $_POST['draw'],
							"recordsTotal" => $this->M_tabel_monitoring_pelatihan->count_all($month,$filter),
							"recordsFiltered" => $this->M_tabel_monitoring_pelatihan->count_filtered($month,$filter),
							"data" => $data,
					);
		...
		// 	$list = $this->M_tabel_monitoring_pelatihan->get_datatables($npk, $filter);
		...
		// 		"recordsFiltered" => $this->M_tabel_monitoring_pelatihan->count_filtered($month, $filter, $np_karyawan),
		// 		"data" => $data,
		// 	);
