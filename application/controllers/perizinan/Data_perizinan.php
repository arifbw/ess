<?php
	...
	class Data_perizinan extends CI_Controller {
		...
		public function index()
		{
            ...
            $array_daftar_karyawan	= $this->M_data_perizinan->select_daftar_karyawan_and_outsource();
		...
		public function tabel_data_perizinan() {
			...
			foreach ($list as $tampil) {
                ...
                //     $row[] = "<button class='btn btn-primary btn-xs'>Sudah Cutoff Data</button>";
                ...
                // } else {
                    ...
                    if(($status_1=='1') && ($status_2!='2' || $status_2!='1')) { //disetujui atasan 1
                        if($tampil->approval_2_np==null || $tampil->approval_2_np=='') { //jika tidak ada atasan 2
                            ...
                            $btn_warna		='btn-warning';
                            $btn_text		='Disetujui Atasan 1, <br>Menunggu Atasan 2';
                    ...
                    if($status_2=='1') { //disetujui atasan  2
                        ...
                        if($status_1=='0' || $status_1==null) { //jika paralel atasan 2 belum approve
                            $btn_warna		='btn-warning';
                            $btn_text		='Disetujui Atasan 2, <br>Menunggu Atasan 1';
                            $btn_disabled 	='disabled';
                        }
                    }
                    ...
                    if($tampil->machine_id_start!=null && $tampil->machine_id_start!='' && $tampil->machine_id_start!='ess') { 
                        $btn_warna		='btn-success';
                        $btn_text		='Disetujui Pos Pengamanan';
                        $btn_disabled 	='disabled';
                    }

                    if((($tampil->machine_id_start!=null && $tampil->machine_id_start!='' && $tampil->machine_id_start!='ess') || ($tampil->machine_id_end!=null && $tampil->machine_id_end!='' && $tampil->machine_id_end!='ess')) && (get_perizinan_name($tampil->kode_pamlek)->nama=='Izin Datang Terlambat')) { 
                        $row[] = "<button class='btn btn-success btn-xs status_button' data-toggle='modal' data-target='#modal_status' 
                        ...
                        data-approval-2-status='$approval_2_status'>Disetujui</button>";
                    } else {
                        ...
                        data-approval-2-nama='$approval_2_nama'
                        data-approval-1-status='$approval_1_status'
                        data-approval-2-status='$approval_2_status'>$btn_text</button>";
                    }
                // }
                # END: status

                # action
                $aksi = "<button class='btn btn-default btn-xs status_button' data-toggle='modal' data-target='#modal_status'
                        data-np-karyawan='$np_karyawan'
                        data-nama='$nama'
                        data-pamlek='$kode_pamlek'
                        data-created-at='$created_at'
                        ...
                        data-approval-2-nama='$approval_2_nama'
                        data-approval-1-status='$approval_1_status'
                        data-approval-2-status='$approval_2_status'>Detail</button>";

                $sudah_cutoff = sudah_cutoff($tampil->start_date ?: $tampil->end_date);
                if($sudah_cutoff) { //jika sudah lewat masa cutoff
                    $aksi .= "&nbsp;<button class='btn btn-primary btn-xs' data-toggle='tooltip' title='Tidak bisa melakukan aksi setelah Cutoff data di ERP pada $sudah_cutoff' disabled>Submit ERP</button>";
                }
				$row[] = $aksi;
        ...
        function action_insert_perizinan(){
            ...
            if($submit){
				...
				$approval_2_np		= $this->input->post('approval_2_np');
				$approval_2_nama	= $this->input->post('approval_2_input');
