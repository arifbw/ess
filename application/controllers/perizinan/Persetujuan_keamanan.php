<?php
	...
	class Persetujuan_keamanan extends CI_Controller {
		...
		public function tabel_persetujuan_keamanan() {
            ...
            $filter_pos   = $this->uri->segment(12);
...
//				}
				...
				$last_pengamanan = null;

				if (is_array($pengamanan_posisi) && count($pengamanan_posisi) > 0) {
					$last_pengamanan = $pengamanan_posisi[count($pengamanan_posisi) - 1];
				}

				if ($tampil->approval_pengamanan_posisi != null && $tampil->approval_pengamanan_posisi != '') {
					if ($last_pengamanan !== null) {
						$btn_warna = 'btn-warning';
...
//                echo json_encode($data_insert); exit();
        ...
        function update_cico($data_lempar){
            ...
            $tanggal_proses = ($data_lempar['date_start']==null || $data_lempar['date_start']=='') ? $data_lempar['date_end'] : $data_lempar['date_start']; # 2022-04-04
            
            while($tanggal_proses <= $data_lempar['date_end']){ # 2022-04-04
                $tahun_bulan = str_replace('-','_',substr("$tanggal_proses", 0, 7)); # 2022-04-04
                ...
                $tanggal_proses = date ("Y-m-d", strtotime("+1 day", strtotime($tanggal_proses))); # 2022-04-04
            } # 2022-04-04
        }
		...
		{			
			...
			$arr_pos = json_decode($izin["pos"]);
			$pos = $this->db->select("mst_pos.*, '1' AS ket")->where('status', '1')->where("find_in_set('".$_SESSION['no_pokok']."', no_pokok)")->get('mst_pos')->result();
			$data["pos"] = $pos;
			// ->where_in('id', $arr_pos)
			...
			$data["status_approval_2_status"] = $approval_2_status;
			$data["status_approval_2_keterangan"] = $izin['approval_2_keterangan'];
			$data_pengamanan = ($izin["approval_pengamanan_posisi"]==null) ? array() : json_decode($izin["approval_pengamanan_posisi"]);
			$array_to_sort=[];
			foreach ($data_pengamanan as $key) {
				$array_to_sort[] = $key->waktu;
			}
			array_multisort($array_to_sort, SORT_ASC, $data_pengamanan);
			$data["pengamanan"] = $data_pengamanan;
			$data["judul"] = ucwords(preg_replace("/_/"," ",__CLASS__));
		...
		public function save_approve($jenis=null) {
			...
			// echo $this->db->last_query();exit;
				...
				if ($izin->num_rows() == 1) {
					...
					//17 03 2022, Tri Wibowo 7648, di akui ketika di setujui pamsiknilmat
					$bulan = substr($tgl, 0, 4).'_'.substr($tgl, 5, 2);
					$tabel = 'ess_perizinan_'.$bulan;
					...
					if ($check_id_perizinan=='' || $check_id_perizinan==null) {
                    	...
                    	$this->update_cico($parameter_perizinan);
						// if ($_SESSION['no_pokok']!='7648') {
                    	// } else {
                    		// var_dump($parameter_perizinan);exit;
                    	// }
					}
			...
			//17 03 2022, Tri Wibowo - 7648, Jika sudah di approve keamanan maka diakui
        ...
        public function cetak() {
			...
			$excel->getActiveSheet()->setCellValueExplicit('G'.'3', 'Approval Atasan 1', PHPExcel_Cell_DataType::TYPE_STRING);
				 
			$excel->getActiveSheet()->setCellValueExplicit('H'.'3', 'Approval Atasan 2', PHPExcel_Cell_DataType::TYPE_STRING);
			...
			// 	// Increment $awal for date cells
			// 	$awal += 1; 
			
			// 	// Use a separate counter for approval status cells
			// 	$excel->getActiveSheet()->setCellValueExplicit('G'.$awal, strtoupper($tampil->approval_1_status), PHPExcel_Cell_DataType::TYPE_STRING);
			// 	$excel->getActiveSheet()->setCellValueExplicit('H'.$awal, strtoupper($tampil->approval_2_status), PHPExcel_Cell_DataType::TYPE_STRING);
			
			// 	// Increment $awal for approval status cells
			// 	$awal += 1;
			// }
        ...
        function update_approval() {
            ...
            foreach( $pengamanan_posisi as $row ){
                ...
                $new_text .= '
                            &nbsp;&nbsp;<a title="Edit" data-no="'.$no.'" data-created="'.$row->created.'" data-pos="'.$row->pos.'" data-nama_pos="'.$row->nama_pos.'" data-waktu="'.$row->waktu.'" data-posisi="'.$row->posisi.'" onclick="edit_approval(this)"><i class="fa fa-pencil"></i></a>';
                if($row->status=='1'){
        ...
        # save approval ajax
        public function save_approve_ajax($jenis=null) {
            $response = [];
			$simpan = array();
			$set_date_real = array();
			$id_ = $this->input->post('id_perizinan');
			...
			if ($jenis=='batal') {
				if ($izin->num_rows() == 1) {
                    ...
                    $response['status'] = true;
                    $response['message'] = 'Berhasil Membatalkan Perizinan';
				} else {
                    $response['status'] = false;
                    $response['message'] = 'Data perizinan tidak valid!';
				}
			} else {
				if ($izin->num_rows() == 1) {
					$kode_pamlek = $izin->row()->kode_pamlek;
					$pengamanan = $izin->row()->approval_pengamanan_posisi;
					if ($pengamanan!=null) {
						$get_posisi =  json_decode($pengamanan);
						foreach ($get_posisi as $val) {
					...
					if ($this->db->affected_rows() > 0) {
                        ...
                        if (trim($check_id_perizinan)=='' || $check_id_perizinan==null) {
							...
							$this->db->where('id',$check_id_perizinan)->update($tabel,[
								'start_date'=>$izin_new["start_date"],
								'end_date'=>$izin_new["end_date"],
								'start_time'=>$izin_new["start_time"],
								'end_time'=>$izin_new["end_time"],
						...
						$response['status'] = true;
						$response['message'] = 'Berhasil Memberikan Approval';
						$response['new_pengamanan_posisi'] = $simpan;
						$response['kode_pamlek'] = $kode_pamlek;
					}
					else {
                        $response['status'] = false;
						$response['message'] = 'Gagal Memberikan Approval! Cek Koneksi Anda.';
					}
				} else {
                    $response['status'] = false;
                    $response['message'] = 'Data perizinan tidak valid!';
		...
		function batalkan_perizinan_ajax(){
			$response = [];
			$set = [];
			$id_ = $this->input->post('id_perizinan');
			$tgl = $this->input->post('tgl');
