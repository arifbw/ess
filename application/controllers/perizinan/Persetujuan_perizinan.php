<?php
	...
	class Persetujuan_perizinan extends CI_Controller {
		public function __construct(){
			...
			$this->data['judul'] = "Persetujuan Perizinan";
		...
		public function index()
		{
			...
			$perizinan_belum_approve_atasan = $this->db->select('a.id, a.np_karyawan, a.nama, a.info_type, a.absence_type, a.kode_pamlek, a.start_date, a.end_date, a.approval_1_np, a.approval_1_nama, a.approval_1_status, a.approval_2_np, a.approval_2_nama, a.approval_2_status, a.id_perizinan, a.approval_pengamanan_posisi, b.nama AS nama_izin')
				...
				->group_start()
					->group_start()
						->where('a.approval_1_np',$_SESSION["no_pokok"])
					...
					->group_end()
					->or_group_start()
						->where('a.approval_2_np',$_SESSION["no_pokok"])
						->where('a.approval_2_status IS NULL',null,false)
						->group_start()
							->where('a.approval_1_status IS NULL',null,false)
            ...
            $this->load->model($this->folder_model."M_persetujuan_perizinan");
			//echo __FILE__ . __LINE__;die(var_dump($this->akses));
            $izin = $this->M_persetujuan_perizinan->get_mst_perizinan()->result();
            $array_daftar_karyawan	= $this->M_persetujuan_perizinan->select_daftar_karyawan();
            
			$this->data["akses"] 					= $this->akses;
			$this->data["navigasi_menu"] 			= menu_helper();
			$this->data['content'] 					= $this->folder_view."persetujuan_perizinan";
            ...
            $this->data['array_tahun_bulan'] 	= $this->M_persetujuan_perizinan->get_tabel_perizinan_from_schema()->result();
			
			$this->load->view('template',$this->data);
		}	
		
		public function tabel_persetujuan_perizinan() {
            $jenis = array();
			...
			*/
			...
			$this->load->model($this->folder_model."M_tabel_persetujuan_perizinan");
...
//			}
			...
			$list 	= $this->M_tabel_persetujuan_perizinan->get_datatables($var,$params,@$jenis);	
			...
			foreach ($list as $tampil) {
                ...
                if ($this->akses["persetujuan"]) {
					...
					//17 03 2022 Tri Wibowo 7648, admin pamsiknilmat master data dapat melakukan release
					//jika admin pamsiknilmat maka tidak ada cutoff, masih bisa approve
					if($_SESSION["grup"]=='15')
					{
						$sudah_cutoff=null;

						//11 10 2022 jika admin pamsiknilmat btn disable nya ngikut apakah perizinan nya sudah di approve semua atasan atau belum	
						if($kode_pamlek=='0' || $kode_pamlek=='G' || $kode_pamlek=='SIPK') //jika idt, poliklinik, sipk maka hanya atasan 1
						{
							if($status_1=='1')
							{
								$btn_disabled = 'disabled';
					...
					//end of 17 03 2022 Tri Wibowo 7648
					...
					{	
						$aksi = "<button class='btn btn-primary btn-xs persetujuan_button' data-toggle='modal' data-target='#modal_persetujuan' data-id=".$tampil->id." data-tgl='".$tampil->start_date."' $btn_disabled>Persetujuan</button>";
			...
			$output = array(
							"draw" => $_POST['draw'],
							"recordsTotal" => $this->M_tabel_persetujuan_perizinan->count_all($var, $params, @$jenis),
							"recordsFiltered" => $this->M_tabel_persetujuan_perizinan->count_filtered($var, $params, @$jenis),
							"data" => $data,
					);
		...
		{			
			...
			if($_SESSION["grup"]==4) //jika Pengadministrasi Unit Kerja
			{
				...
				$np_list=array();
				$np_list=$this->M_persetujuan_perizinan->select_np_by_kode_unit($list_kode_unit);						
        ...
        /*function action_insert_perizinan(){
            ...
            if($submit){
                ...
                if($np_karyawan=='' || $np_karyawan==null){
					...
					redirect(base_url($this->folder_controller.'persetujuan_perizinan'));
...
//                echo json_encode($data_insert); exit();
        ...
        public function hapus($id=null, $np=null, $tanggal_end=null, $tanggal_start=null) {
			...
			if(@$id != null && @$np != null && @$tanggal_start != null && @$tanggal_end != null) {
                ...
                $get = $this->M_persetujuan_perizinan->ambil_perizinan_id($id, 'ess_perizinan_'.$tahun_bulan_perizinan);
			...
			redirect(base_url('perizinan/persetujuan_perizinan'));
		}
		...
		{			
			...
			$izin = $this->db->select("a.*, (CASE WHEN start_date is not null then start_date else end_date end) as ordere")->where('id', $id_)->get('ess_request_perizinan a')->row_array();
			...
			$data['approval_1']		= trim($izin['approval_1_np']);
			$data['approval_2']		= trim($izin['approval_2_np']);	
			$data['status_1']		= trim($izin['approval_1_status']);
			$data['status_2']		= trim($izin['approval_2_status']);
			...
			$this->load->view($this->folder_view."approve_perizinan",$data);
		}
		...
		public function save_approve() {
			...
			if ($izin->num_rows() == 1) {
				if( $izin->row()->np_batal!=null ){
					$this->session->set_flashdata('warning', 'Perizinan sudah dibatalkan');
					redirect(site_url($this->folder_controller.'persetujuan_perizinan'));
					exit();
				}
				$tanggal = date('Y-m-d H:i:s');
				$status_1 = $this->input->post('status_1',true);
				$status_2 = $this->input->post('status_2',true);
				// echo $status_2;exit;
				if ($status_1!='') {
					$set['approval_1_updated_at'] = $tanggal;
					$set['approval_1_status'] = $status_1;
					if($status_1=='2') {
						$set['approval_1_keterangan'] = $this->input->post('alasan_1');
					}
				}
				if ($status_2!='') {
					...
					if($status_2=='2') {
						$set['approval_2_keterangan'] = $this->input->post('alasan_2');
					}
				}

				$this->db->where('id', $id_)->set($set)->update('ess_request_perizinan');
				if ($this->db->affected_rows() > 0) {
					
					//set tabel asli ess_perizinan_bulan
					$izin_new = $this->db->where('id', $id_)->get('ess_request_perizinan')->row_array();
//					if ($izin_new['approval_1_status']=='1' && ($izin_new['approval_2_status']=='1' || $izin_new['approval_2_status']==null)) {
                    # 2021-03-04, bowo/heru. ada izin yg membutuhkan atasan 2 dan ada izin yg tidak membutuhkan atasan 2
                    # jika (ada atasan 2, status atasan 1=1 dan status Atasan 2=1) OR (jika tidak ada atasan 2, status atasa 1=1)
					
					$check_id_perizinan = $izin_new["id_perizinan"];
					
					if ( (($izin_new['approval_2_np']!=null && $izin_new['approval_1_status']=='1' && $izin_new['approval_2_status']=='1') || ($izin_new['approval_2_np']==null && $izin_new['approval_1_status']=='1' )) && (trim($check_id_perizinan)=='' || $check_id_perizinan==null) ) {
						unset($izin_new["id"]);
						...
						$this->db->set($izin_new)->insert($tabel);
						$id_perizinan = $this->db->insert_id();
						$this->db->where('id', $id_)->set('id_perizinan', $id_perizinan)->update('ess_request_perizinan');
                    	...
                    	$this->update_cico($parameter_perizinan);
					} else if( trim($check_id_perizinan)!='' && $check_id_perizinan!=null ){
						# update
						$this->db->where('id',$check_id_perizinan)->update($tabel,[
							'approval_1_status'=>$izin_new["approval_1_status"],
							'approval_1_keterangan'=>$izin_new["approval_1_keterangan"],
							'approval_1_updated_at'=>$izin_new["approval_1_updated_at"],
							'approval_2_status'=>$izin_new["approval_2_status"],
							'approval_2_keterangan'=>$izin_new["approval_2_keterangan"],
							'approval_2_updated_at'=>$izin_new["approval_2_updated_at"]
						]);

						$parameter_perizinan = [
							'id_row_baru'=>$check_id_perizinan,
							'np_karyawan'=>$izin_new['np_karyawan'],
							'date_start'=>$izin_new['start_date'],
							'date_end'=>$izin_new['end_date']
						];
					...
					// $this->session->set_flashdata('success', 'Berhasil Memberikan Approval '.$tabel.' '.$id_.' '.$id_perizinan.' '.var_dump($parameter_perizinan));
				}
				else {
        ...
        public function cetak() {
			...
			if($_SESSION["grup"]==4) { //jika Pengadministrasi Unit Kerja
				...
				$var = 1;
			$get_data = $this->M_tabel_persetujuan_perizinan->_get_excel($var,$target_table,$set,@$jenis);
	        ...
	        header("Content-Disposition: attachment; filename=persetujuan_perizinan.xlsx");
	        header('Cache-Control: max-age=0');

	        $excel = PHPExcel_IOFactory::createReader('Excel2007');
	        $excel = $excel->load('./asset/Template_persetujuan_perizinan.xlsx');
