...
class Pemeriksaan_keluar_danposko extends CI_Controller {
	public function __construct(){
		...
		$this->data['judul'] = "Pemeriksaan SPBE Keluar oleh Dan / Wadan Posko";
	...
	public function index(){
		...
		$this->data['content'] = $this->folder_view."pemeriksaan_keluar_danposko";

		$this->load->view('template',$this->data);
	}

	function get_data_persetujuan(){
		$filter = ['otoritas'=>'danposko'];
	...
	function simpan_approval(){
		...
		$danposko_status = $this->input->post('danposko_status');

		if( $spbe->approval_pengamanan_keluar=='1' && $danposko_status=='2' ){
			$status = false;
			$message = 'Pembatalan hanya lewat Petugas Pos karena barang sudah diapprove Petugas Pos';
		} else{
			$data_insert = [
				'danposko_np'=>$this->session->userdata('no_pokok'),
				'danposko_nama'=>$this->session->userdata('nama'),
				'danposko_jabatan'=>$this->session->userdata('nama_jabatan'),
				'danposko_status'=>$this->input->post('danposko_status'),
				'danposko_keterangan'=>$this->input->post('danposko_status')=='1' ? null:$this->input->post('danposko_keterangan'),
				'danposko_tanggal'=>$this->input->post('danposko_tanggal'),
				'danposko_jam'=>$this->input->post('danposko_jam'),
