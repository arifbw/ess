...
class Pengecekan_masuk_petugas_pamsiknilmat extends CI_Controller {
	public function __construct(){
		...
		$this->data['judul'] = "Pemeriksaan SPBE Masuk oleh Petugas Pamsiknilmat";
	...
	function simpan_approval(){
		$ess_permohonan_spbe_id = $this->input->post('ess_permohonan_spbe_id');
		$new_id = $this->uuid->v4();
		$data_pos = [
			'id'=>$new_id,
			'ess_permohonan_spbe_id'=>$ess_permohonan_spbe_id,
		...
		if($this->db->affected_rows()>0){
			...
			if( count($kondisi_barang) > 0 ) $this->db->insert_batch('ess_permohonan_spbe_kondisi_barang',$kondisi_barang);

			# update _pos
			$this->db->where('id',$new_id)->update('ess_permohonan_spbe_pos',['kondisi_barang_json'=>json_encode($kondisi_barang)]);

			$approval_pengamanan_posisi = $this->db->where('ess_permohonan_spbe_id',$ess_permohonan_spbe_id)->order_by('tanggal','DESC')->order_by('jam','DESC')->get('ess_permohonan_spbe_pos')->result_array();
			$this->db->where('id',$ess_permohonan_spbe_id)->update('ess_permohonan_spbe',[
