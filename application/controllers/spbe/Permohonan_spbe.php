...
class Permohonan_spbe extends CI_Controller {
	...
	function get_data_permohonan(){
		...
		$data = $this->M_tabel_permohonan_spbe->get_datatables($filter);
		$output = array(
			"draw" => $_POST['draw'],
			"recordsTotal" => $this->M_tabel_permohonan_spbe->count_all($filter),
			"recordsFiltered" => $this->M_tabel_permohonan_spbe->count_filtered($filter),
			"data" => $data,
		);
	...
	public function tambah(){
		$array_daftar_karyawan	= $this->M_permohonan_spbe->select_daftar_karyawan([
			'grup'=>$_SESSION["grup"],
		...
		$this->data["akses"] = $this->akses;
		$this->data["navigasi_menu"] = menu_helper();
		$this->data['content'] = $this->folder_view."tambah_permohonan_spbe";
	...
	function simpan(){
		...
		$data_insert = [];
		$array_detail = ['np_karyawan','nama','nama_jabatan','kode_unit','nama_unit','milik','maksud','dikirim_ke','keluar_tanggal','approval_kasek_np','approval_kasek_nama','approval_kasek_jabatan','approval_kasek_kode_unit','approval_kasek_nama_unit','approval_kasek_nama_unit_singkat','approval_atasan_np','approval_atasan_nama','approval_atasan_jabatan','approval_atasan_kode_unit','approval_atasan_nama_unit','approval_atasan_nama_unit_singkat','barang_kembali'];
		...
		$barang_kembali = $this->input->post('barang_kembali');
		...
		$last_counter = $this->db->select('MAX(no_urut) AS counter')->where("DATE_FORMAT(created_at,'%Y')", date('Y'))->where("approval_atasan_kode_unit", $this->input->post('approval_atasan_kode_unit'))->get('ess_permohonan_spbe')->row_array()['counter'];
		...
		if($this->db->affected_rows()>0){
			...
			$this->db->insert_batch('ess_permohonan_spbe_barang',$data_barang);
	...
	function get_kasek(){
		// $list = $this->db->query("SELECT no_pokok, nama, nama_jabatan, kode_unit, nama_unit, nama_unit_singkat FROM `mst_karyawan`
		// WHERE grup_jabatan='KASEK' OR SUBSTR(kode_jabatan,-3,3)='600'")->result_array();

		// $this->db->select('no_pokok, nama, nama_jabatan, kode_unit, nama_unit, nama_unit_singkat');
		// $this->db->from('mst_karyawan');
		// $this->db->where("grup_jabatan = 'KASEK' OR SUBSTRING(kode_jabatan, -3) = '600'");
		// $query = $this->db->get();
		// $list = $query->result();
		$this->load->model('m_approval');
		$np_karyawan = $this->input->post('np_karyawan');
