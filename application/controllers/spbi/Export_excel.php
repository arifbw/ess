...
class Export_excel extends CI_Controller {
    ...
    function generate(){
        ...
        header("Content-Disposition: attachment; filename=Data-Permohonan-SPBI-$filename.xlsx");
        header('Cache-Control: max-age=0');

        $excel = PHPExcel_IOFactory::createReader('Excel2007');
        $excel = $excel->load('./asset/excel-templates-spbi-spbe/template-export-spbi.xlsx');

        # proses isi data
        if( @$this->input->get('id',true) ) $this->db->where('uuid', $this->input->get('id',true) );
        $data = $this->db->from('ess_permohonan_spbi')
        ...
        foreach($data as $row){
            ...
            $excel->getActiveSheet()->setCellValueExplicit('F'.$awal, $proses, PHPExcel_Cell_DataType::TYPE_STRING);
            ...
            if( !in_array($row->pos_keluar,[null,'null']) ){
                ...
                $excel->getActiveSheet()->setCellValueExplicit('G'.$awal, $text_pos_keluar, PHPExcel_Cell_DataType::TYPE_STRING);
            }
            ...
            if( !in_array($row->pos_masuk,[null,'null']) ){
                ...
                $excel->getActiveSheet()->setCellValueExplicit('H'.$awal, $text_pos_masuk, PHPExcel_Cell_DataType::TYPE_STRING);
            }

            # Atasan
            $excel->getActiveSheet()->setCellValueExplicit('I'.$awal, $row->approval_atasan_np .' - '. $row->approval_atasan_nama, PHPExcel_Cell_DataType::TYPE_STRING);
            if( $row->approval_atasan_status!=null ){
                $excel->getActiveSheet()->setCellValueExplicit('J'.$awal, ($row->approval_atasan_status=='1' ? 'Setuju':'Tolak'), PHPExcel_Cell_DataType::TYPE_STRING);
            }
            if( $row->approval_atasan_updated_at!=null ){
                $excel->getActiveSheet()->setCellValueExplicit('K'.$awal, datetime_indo($row->approval_atasan_updated_at), PHPExcel_Cell_DataType::TYPE_STRING);
            }

            # pengecek
            $excel->getActiveSheet()->setCellValueExplicit('L'.$awal, $row->pengecek1_np .' - '. $row->pengecek1_nama, PHPExcel_Cell_DataType::TYPE_STRING);
            if( $row->pengecek1_status!=null ){
                $excel->getActiveSheet()->setCellValueExplicit('M'.$awal, ($row->pengecek1_status=='1' ? 'Setuju':'Tolak'), PHPExcel_Cell_DataType::TYPE_STRING);
            }
            if( $row->pengecek1_tanggal!=null && $row->pengecek1_jam!=null ){
                $datetime = $row->pengecek1_tanggal.' '.$row->pengecek1_jam;
                $excel->getActiveSheet()->setCellValueExplicit('N'.$awal, datetime_indo($datetime), PHPExcel_Cell_DataType::TYPE_STRING);
            }

            # pengawas
            $excel->getActiveSheet()->setCellValueExplicit('O'.$awal, $row->konfirmasi_pengguna_np .' - '. $row->konfirmasi_pengguna_nama, PHPExcel_Cell_DataType::TYPE_STRING);
            if( $row->konfirmasi_pengguna!=null ){
                $excel->getActiveSheet()->setCellValueExplicit('P'.$awal, ($row->konfirmasi_pengguna=='1' ? 'Setuju':'Tolak'), PHPExcel_Cell_DataType::TYPE_STRING);
            }
            if( $row->konfirmasi_pengguna_tanggal!=null && $row->konfirmasi_pengguna_jam!=null ){
                ...
                $excel->getActiveSheet()->setCellValueExplicit('Q'.$awal, datetime_indo($datetime), PHPExcel_Cell_DataType::TYPE_STRING);
            }

            # danposko
            $excel->getActiveSheet()->setCellValueExplicit('R'.$awal, $row->danposko_np .' - '. $row->danposko_nama, PHPExcel_Cell_DataType::TYPE_STRING);
            if( $row->danposko_status!=null ){
                $excel->getActiveSheet()->setCellValueExplicit('S'.$awal, ($row->danposko_status=='1' ? 'Setuju':'Tolak'), PHPExcel_Cell_DataType::TYPE_STRING);
            }
            if( $row->danposko_tanggal!=null && $row->danposko_jam!=null ){
                $datetime = $row->danposko_tanggal.' '.$row->danposko_jam;
                $excel->getActiveSheet()->setCellValueExplicit('T'.$awal, datetime_indo($datetime), PHPExcel_Cell_DataType::TYPE_STRING);
            }
            ...
            if( !in_array($row->approval_pengamanan_posisi, [null,'null']) ){
                ...
                $excel->getActiveSheet()->setCellValueExplicit('U'.$awal, $text_keluar, PHPExcel_Cell_DataType::TYPE_STRING);
                ...
                $excel->getActiveSheet()->setCellValueExplicit('V'.$awal, $text_masuk, PHPExcel_Cell_DataType::TYPE_STRING);
            }

            # pembawa
            $excel->getActiveSheet()->setCellValueExplicit('W'.$awal, $row->konfirmasi_pengguna_np .' - '. $row->konfirmasi_pengguna_nama, PHPExcel_Cell_DataType::TYPE_STRING);
            if( $row->konfirmasi_pembawa_status!=null ){
                $excel->getActiveSheet()->setCellValueExplicit('X'.$awal, ($row->konfirmasi_pembawa_status=='1' ? 'Setuju':'Tolak'), PHPExcel_Cell_DataType::TYPE_STRING);
            }
            if( $row->konfirmasi_pembawa_tanggal!=null && $row->konfirmasi_pembawa_jam!=null ){
                ...
                $excel->getActiveSheet()->setCellValueExplicit('Y'.$awal, datetime_indo($datetime), PHPExcel_Cell_DataType::TYPE_STRING);
            }
