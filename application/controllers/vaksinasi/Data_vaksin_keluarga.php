...
class Data_vaksin_keluarga extends CI_Controller {
	...
	function save(){
		...
		if( $this->input->post('ubah_status_vaksin')=='2' ){
			...
			$input['tanggal_vaksin_1'] = null;
			$input['tanggal_vaksin_2'] = null;
			$input['alasan'] = $this->input->post('ubah_alasan');
		} else{
			// $input['mst_klinik_id'] = null;
			$input['alasan'] = null;
			$input['tanggal_vaksin_1'] = $this->input->post('ubah_tanggal_vaksin_1');
			if($this->input->post('ubah_tanggal_vaksin_2')!='') $input['tanggal_vaksin_2'] = $this->input->post('ubah_tanggal_vaksin_2');
		}
	...
	function export(){
		...
		, mst_klinik.alamat as lokasi_klinik')
			...
			//->where("8 - MONTH(ess_kesehatan_keluarga_tertanggung.tanggal_lahir) >=",0);
		if( $jenis!='all' ){
			switch ($jenis) {
				case "1":
