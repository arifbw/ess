...
class M_tabel_permohonan extends CI_Model {
	...
	{		 
		...
		$this->db->select('ess_cuti.alasan_sdm');
		$this->db->select('ess_cuti.approval_alasan_1');
		$this->db->select('ess_cuti.approval_alasan_2');
		...
		$this->db->join(
			'mst_cuti',
			'(mst_cuti.id = ess_cuti.mst_cuti_id OR (ess_cuti.mst_cuti_id IS NULL AND mst_cuti.kode_erp = ess_cuti.absence_type))',
			'left'
		);
