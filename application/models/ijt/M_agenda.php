...
class M_agenda extends CI_Model
{
	...
	var $column_order = array(null, 'a.kegiatan'); //field yang ada di table event
	var $column_search = array('a.kegiatan'); //field yang diizin untuk pencarian 
	var $order = array('a.created_at' => 'desc'); // default order 

	private function _get_datatables_query()
	{
		if ($this->session->userdata('grup') != '5') { // SEMUA
			$this->db->select('a.*,b.nama_jabatan, GROUP_CONCAT(c.np) AS applyer');
			// $this->db->select('a.*,b.nama_jabatan');
			$this->db->from('ijt_event a');
			$this->db->join('ijt_data b', 'b.id = a.job_id and b.deleted_at is null', 'left');
			$this->db->join("ijt_event_undangan c", "a.id = c.event_id and c.deleted_at is null", 'left');
			$this->db->where('a.deleted_at is null');
			$this->db->group_by('a.id');
			// $this->db->where('b.deleted_at is null');
			$this->db->order_by("CASE WHEN tanggal >= CURDATE() THEN 1 ELSE 2 END", "ASC", FALSE);
			$this->db->order_by("CASE WHEN tanggal >= CURDATE() THEN tanggal END", "ASC", FALSE);
			$this->db->order_by("CASE WHEN tanggal < CURDATE() THEN tanggal END", "DESC", FALSE);
		} else {  // PENGGUNA
			$this->db->select('a.*,c.status,b.nama_jabatan');
			$this->db->from('ijt_event a');
			$this->db->join('ijt_data b', 'b.id = a.job_id and b.deleted_at is null', 'left');
			$this->db->join('ijt_konfirmasi_kehadiran c', 'c.id_agenda = a.id and c.deleted_at is null', 'left');
			$this->db->join('ijt_apply d', 'd.job_id = b.id and d.deleted_at is null', 'left');
			$this->db->join('ijt_verval e', 'e.apply_id = d.id and e.deleted_at is null', 'left');
			$this->db->join("ijt_event_undangan f", "a.id = f.event_id and f.np = d.np and f.deleted_at is null");
			$this->db->where('d.np', $this->session->userdata('no_pokok'));
			$this->db->where('e.is_verval', '1');
			$this->db->where('a.deleted_at is null');
			// $this->db->where('b.deleted_at is null');
			// $this->db->where('c.deleted_at is null');
			// $this->db->where('d.deleted_at is null');
			// $this->db->where('e.deleted_at is null');
			$this->db->where('e.jenis_verval', 'administrasi');
			$this->db->order_by("CASE WHEN tanggal >= CURDATE() THEN 1 ELSE 2 END", "ASC", FALSE);
			$this->db->order_by("CASE WHEN tanggal >= CURDATE() THEN tanggal END", "ASC", FALSE);
			$this->db->order_by("CASE WHEN tanggal < CURDATE() THEN tanggal END", "DESC", FALSE);
		}
