...
class M_data_keterlambatan extends CI_Model
{
	...
	private function _get_datatable_keterlambatan_query($kode_unit = null, $np_karyawan, $periode, $keterangan = null, $exp = null)
	{
		// die('sii');
		...
		# ambil row cico yg BERPOTENSI DIA TERLAMBAT
		...
		$a .= " ::limit::";
		# END ambil row cico yg BERPOTENSI DIA TERLAMBAT
		...
		# heru PDS ganti line ini, 2021-01-07 10:57, yg dibandingkan hanya sampai menit
		//$this->db->join("($c) c","FIND_IN_SET(c.id,a.id_perizinan)>0 AND a.datang=c.waktu_izin","LEFT");

		//05 04 2022, Tri Wibowo 7648, Kenapa harus ada validasi waktu datang dan waktu izin ? ada beberapa SIDT yang tidak tampil di keterangan makanya saya edit
		//$this->db->join("($c) c","FIND_IN_SET(c.id,a.id_perizinan)>0 AND date_format(a.datang,'%Y-%m-%d %H:%i')=date_format(c.waktu_izin,'%Y-%m-%d %H:%i')","LEFT");
		$this->db->join("($c) c", "FIND_IN_SET(c.id,a.id_perizinan)>0", "LEFT");
		//end of 05 04 2022, Tri Wibowo 7648

		# END heru PDS ganti line ini, 2021-01-07 10:57
		...
		# heru PDS menambahkan WHERE ini, 2021-01-07 10:57
		$this->db->group_start();
		$this->db->group_start();
		$this->db->where('a.id_cuti is null', null, false);
		$this->db->or_where('a.id_cuti', '');
		$this->db->group_end();
		$this->db->group_start();
		$this->db->where('a.id_sppd is null', null, false);
		$this->db->or_where('a.id_sppd', '');
		$this->db->group_end();
		$this->db->group_end();
		# END heru PDS menambahkan WHERE ini, 2021-01-07 10:57
	...
	function get_datatable_keterlambatan($kode_unit, $np_karyawan, $periode, $keterangan, $exp = null)
	{
		...
		$this->_get_datatable_keterlambatan_query($kode_unit, $np_karyawan, $periode, $keterangan, $exp);
	...
	function count_filtered($kode_unit, $np_karyawan, $periode, $keterangan)
	{
		//dipindah kebawah karena tabel nya dinamis
		$column_search = array("a.np_karyawan", "a.nama", "b.description"); //set column field database for datatable_pamlek 

		$this->_get_datatable_keterlambatan_query($kode_unit, $np_karyawan, $periode, $keterangan);
		if ($_POST['search']['value'] != "") {
			$i = 0;
			foreach ($column_search as $item) // loop column 
			{
		...
		$sql = str_replace("::limit::", "", $sql);
		if ($keterangan == "all")
			$count_filtered = count($this->db->query('select * from (' . $sql . ') a where a.jadwal!="OFF"')->result_array());
		else if ($keterangan == "izin")
			$count_filtered = count($this->db->query('select * from (' . $sql . ') a where a.keterangan like "Izin Datang Terlambat%" and a.jadwal!="OFF"')->result_array());
		else
			$count_filtered = count($this->db->query('select * from (' . $sql . ') a where a.keterangan not like "Izin Datang Terlambat%" and a.jadwal!="OFF"')->result_array());

		return $count_filtered;
	}

	public function count_all($kode_unit, $np_karyawan, $periode, $keterangan)
	{
		$this->_get_datatable_keterlambatan_query($kode_unit, $np_karyawan, $periode, $keterangan);
		...
		if ($keterangan == "all")
			...
			$count_all = count($this->db->query('select * from (' . $sql . ') a where a.keterangan like "Izin Datang Terlambat%" and a.jadwal!="OFF"')->result_array());
		else
	...
	function export_data_keterlambatan($kode_unit, $np_karyawan, $periode, $keterangan, $exp = null)
	{
		$this->_get_datatable_keterlambatan_query($kode_unit, $np_karyawan, $periode, $keterangan, $exp);
		$sql = $this->db->get_compiled_select();
		$sql = str_replace("::limit::", "", $sql);
	...
	public function get_rekap_keterlambatan($kode_unit, $np_karyawan, $periode)
	{
		...
		$sql = str_replace("::limit::", "", $sql);
		// echo $sql.'<br><br>';
		$query = $this->db->query('select *, count(*) as jml from (' . $sql . ') a where a.keterangan!="Izin Datang Terlambat|||" group by np_karyawan');
