...
class M_data_rekap_keterlambatan extends CI_Model
{
    ...
    //   a.kode_unit,
    //   a.tanggal,
    //   b.description AS jadwal,
    //   DATE_FORMAT(a.jadwal_masuk, '%Y-%m-%d %H:%i') AS jadwal_masuk,
    //   DATE_FORMAT(a.datang, '%Y-%m-%d %H:%i') AS datang,
    //   CONCAT(
    //     GROUP_CONCAT(
    //       IFNULL(d.nama, '')
    //       ORDER BY
    ...
    //     '|',
    //     CASE
    //       WHEN DATE_FORMAT(a.tapping_fix_1, '%Y-%m-%d %H:%i') != DATE_FORMAT(a.tapping_time_1, '%Y-%m-%d %H:%i') THEN CONCAT(
    //         'koreksi kehadiran, semula ',
    //         IFNULL(a.tapping_time_1, '')
    //       )
    //       ELSE ''
    //     END
    //   ) AS keterangan
    ...
    //           a11.nama_unit,
    //           a11.kode_unit,
    //           a11.dws_tanggal AS tanggal,
    //           IFNULL(a11.dws_name_fix, a11.dws_name) AS jadwal,
    //           CONCAT(
    //             IFNULL(a11.dws_in_tanggal_fix, a11.dws_in_tanggal),
    //             ' ',
    //             IFNULL(a11.dws_in_fix, a11.dws_in)
    //           ) AS jadwal_masuk,
    //           IFNULL(a11.tapping_fix_1, a11.tapping_time_1) AS datang,
    //           a11.tapping_fix_1,
    //           a11.tapping_time_1,
    //           a11.tapping_terminal_1,
    //           a11.id_perizinan,
    //           a11.id_cuti,
    //           a11.id_sppd
    //         FROM
    //           ess_cico_2024_06 a11
    //         WHERE
    //           a11.dws_name != 'OFF'
    //           AND a11.kode_unit LIKE '%' ESCAPE '!'
    //           AND a11.np_karyawan = '7648'
    //       ) a1
    //     WHERE
    //       DATE_FORMAT(a1.datang, '%Y-%m-%d %H:%i') > DATE_FORMAT(a1.jadwal_masuk, '%Y-%m-%d %H:%i')
    //       OR DATE_FORMAT(a1.tapping_time_1, '%Y-%m-%d %H:%i') > DATE_FORMAT(a1.jadwal_masuk, '%Y-%m-%d %H:%i')
    //   ) a
    //   LEFT JOIN mst_jadwal_kerja b ON a.jadwal = b.dws
    //   AND b.dws_variant = ''
    //   LEFT JOIN (
    //     SELECT
    //       c1.id,
    //       CONCAT(c1.end_date, ' ', c1.end_time) AS waktu_izin,
    //       c1.kode_pamlek
    //     FROM
    //       ess_perizinan c1
    //     WHERE
    //       c1.kode_unit LIKE '%' ESCAPE '!'
    //       AND c1.np_karyawan = '7648'
    //   ) c ON FIND_IN_SET(c.id, a.id_perizinan) > 0
    //   AND DATE_FORMAT(a.datang, '%Y-%m-%d %H:%i') = DATE_FORMAT(c.waktu_izin, '%Y-%m-%d %H:%i')
    //   LEFT JOIN mst_perizinan d ON d.kode_pamlek = c.kode_pamlek
    //   LEFT JOIN (
    //     SELECT
    //       e1.id,
    //       e2.uraian
    //     FROM
    //       ess_cuti e1
    //       JOIN mst_cuti e2 ON e1.absence_type = e2.kode_erp
    //     WHERE
    //       e1.start_date <= LAST_DAY('2024-06-01')
    //       AND e1.end_date >= '2024-06-01'
    //       AND e1.kode_unit LIKE '%' ESCAPE '!'
    //       AND e1.np_karyawan = '7648'
    //   ) e ON a.id_cuti = e.id
    //   LEFT JOIN (
    //     SELECT
    //       f1.id,
    ...
    //       ess_sppd f1
    //     WHERE
    //       f1.tgl_berangkat <= LAST_DAY('2024-06-01')
    //       AND f1.tgl_pulang >= '2024-06-01'
    //       AND f1.kode_unit LIKE '%' ESCAPE '!'
    //       AND f1.np_karyawan = '7648'
    //   ) f ON a.id_sppd = f.id
    // WHERE
    //   (
    //     a.id_cuti IS NULL
