...
class M_tabel_persetujuan_lembur extends CI_Model {

	var $table = 'ess_lembur_transaksi';
	var $column_order = array(null, 'no_pokok', 'ess_lembur_transaksi.nama', 'tgl_dws', 'input_mulai', 'input_selesai', 'waktu_diakui', 'approval_pimpinan_status', ''); //set column field database for datatable orderable
	...
	private function _get_datatables_query($approve, $tgl='', $np=null)
	{
	...
	function get_datatables($approve, $tgl='', $np=null) {
		$this->_get_datatables_query($approve, $tgl, $np);
	...
	function count_filtered($approve, $tgl='', $np=null) {
		$this->_get_datatables_query($approve, $tgl, $np);
		$query = $this->db->get();
		return $query->num_rows();
	}

	public function count_all($approve, $tgl='', $np=null) {
		//jika Pengadministrasi Unit Kerja
		// if($_SESSION["grup"]==4) {
		// 	$ada_data=0;
		// 	$var=array();
		...
		if ($approve == '3') {
			...
			$this->db->where('((approval_pimpinan_status="0" or approval_pimpinan_status is null) AND ((waktu_mulai_fix is not null or waktu_mulai_fix != "") AND (waktu_selesai_fix is not null or waktu_selesai_fix != "")))');
		}
		else if ($approve != 'all') {
			$this->db->where('approval_pimpinan_status', $approve);
		}
