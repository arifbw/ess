...
class M_bypass_master_data_organisasi extends CI_Model {
	...
	private function _get_datatable_skeps_query($np=0,$nomor=0){
		...
		if($nomor!='0')
			$this->db->where("nomor_skep", $this->encrypt->decode($nomor));
	...
	function get_datatable_skep($np=0,$nomor=0){
		$this->_get_datatable_skeps_query($np, $nomor);
		if($_POST['length'] != -1)
	...
	function count_filtered($np=0,$nomor=0){
		$this->_get_datatable_skeps_query($np, $nomor);
		$query = $this->db->get();
		return $query->num_rows();
	}

	public function count_all($np=0,$nomor=0){		
		$this->db->select("*");	
		$this->db->from($this->table_skep);
	...
	public function daftar_nomor(){
		$this->db->select('nomor_skep as nomor');
		$this->db->from($this->table_skep);
		$this->db->group_by('nomor_skep');
