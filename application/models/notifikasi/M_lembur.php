...
class M_lembur extends CI_Model {
	...
	public function get_lembur_belum_persetujuan($tanggal_cutoff){
		...
		$this->db->join($this->table_jadwal_kerja." g","ifnull(f.dws,e.dws)=g.dws AND IFNULL(f.dws_variant,'')=g.dws_variant","left");
		
		//23 09 2021 - 7648 Tri Wibowo harus ada waktu mulai fix (diakui)
		$this->db->where("a.waktu_mulai_fix IS NOT NULL");
		$this->db->where("a.waktu_mulai_fix !=''");
