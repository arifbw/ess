...
class M_tabel_persetujuan_lembur_manual extends CI_Model {

	var $table = 'ess_lembur_transaksi';
	var $column_order = array(null, 'no_pokok', 'nama', 'tgl_mulai', 'tgl_selesai', 'jam_mulai', 'jam_selesai'); //set column field database for datatable orderable
	var $column_search = array('ess_lembur_transaksi.no_pokok', 'nama', 'tgl_mulai', 'tgl_selesai', 'jam_mulai', 'jam_selesai'); //set column field database for datatable searchable 
	...
	private function _get_datatables_query($approve, $tahun_bulan=null)
	{
	...
	function get_datatables($approve, $tahun_bulan=null) {
		$this->_get_datatables_query($approve, $tahun_bulan);
	...
	function count_filtered($approve, $tahun_bulan=null) {
		$this->_get_datatables_query($approve, $tahun_bulan);
		$query = $this->db->get();
		return $query->num_rows();
	}

	public function count_all($approve, $tahun_bulan=null) {
		//jika Pengadministrasi Unit Kerja
		if($_SESSION["grup"]==4) {
			$ada_data=0;
			$var=array();
