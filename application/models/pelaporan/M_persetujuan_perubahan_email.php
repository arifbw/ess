...
class M_persetujuan_perubahan_email extends CI_Model {

    var $table = 'laporan_perubahan_email';
    var $column_order = array(null, 'np_karyawan','nama_karyawan', 'email_lama', 'email_baru', 'np_atasan', 'nama_atasan', 'keterangan', 'status_approve'); // Kolom untuk pengurutan
    var $column_search = array('np_karyawan', 'nama_karyawan', 'email_lama', 'email_baru', 'np_atasan', 'nama_atasan', 'keterangan', 'status_approve'); // Kolom yang bisa dicari
    ...
    public function insert_data($data) {
        ...
        // Lakukan penyisipan data ke dalam tabel 'laporan_perubahan_email'
        $this->db->insert('laporan_perubahan_email', $data);
    ...
    public function delete_data($id) {
        ...
        $this->db->where('id', $id);
        $this->db->delete('laporan_perubahan_email');
    ...
    public function approve($id) {
        // Persiapkan data yang ingin diubah
        $data = array(
            'status_approve' => 'Disetujui'
        );
    
        // Ubah nilai kolom status menjadi 'Disetujui' berdasarkan ID
        $this->db->where('id', $id);
        $this->db->update('laporan_perubahan_email', $data);
    
        // Mengembalikan TRUE jika data berhasil diupdate, dan FALSE jika tidak
        return $this->db->affected_rows() > 0;
    }
