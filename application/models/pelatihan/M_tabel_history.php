<?php
defined('BASEPATH') OR exit('No direct script access allowed');

class M_tabel_history extends CI_Model {

	var $table = 'ess_sppd';
	var $column_order = array(null, 'np_karyawan','nama','nama_jabatan','perihal','tgl_berangkat'); //set column field database for datatable orderable
	var $column_search = array('np_karyawan','nama','nama_jabatan','perihal','tgl_berangkat'); //set column field database for datatable searchable 
	var $order = array('tgl_berangkat' => 'desc'); // default order 
	
	public function __construct(){
		parent::__construct();
		//Do your magic here
	}
	
	private function _get_datatables_query($var=null, $month=null)
	{		 
		$this->db->select('ess_sppd.id');
		$this->db->select('ess_sppd.np_karyawan');
		$this->db->select('ess_sppd.nama');
		$this->db->select('ess_sppd.nama_jabatan');
		$this->db->select('ess_sppd.perihal');
		$this->db->select('ess_sppd.tgl_berangkat');
		$this->db->from($this->table);	
		$this->db->where('YEAR(tgl_berangkat) >=', 2024);
		$this->db->where_in('catatan', 'Perjalanan Dinas Tugas Belajar');
		
		
		if($_SESSION["grup"]==4) //jika Pengadministrasi Unit Kerja
		{
			$this->db->where_in('kode_unit', $var);								
		}else
		if($_SESSION["grup"]==5) //jika Pengguna
		{
			$this->db->where_in('np_karyawan', $var);	
		}else
		{
		}

		if(@$month!=0){
            $this->db->where("DATE_FORMAT(tgl_berangkat,'%Y-%m')", $month);
        }
				
		$i = 0;
	
		foreach ($this->column_search as $item) // loop column 
		{
			if($_POST['search']['value']) // if datatable send POST for search
			{
				
				if($i===0) // first loop
				{
					$this->db->group_start(); // open bracket. query Where with OR clause better with bracket. because maybe can combine with other WHERE with AND.
					$this->db->like($item, $_POST['search']['value']);
				}
				else
				{
					$this->db->or_like($item, $_POST['search']['value']);
				}

				if(count($this->column_search) - 1 == $i) //last loop
					$this->db->group_end(); //close bracket
			}
			$i++;
		}
		
		if(isset($_POST['order'])) // here order processing
		{
			$this->db->order_by($this->column_order[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
		} 
		else if(isset($this->order))
		{
			$order = $this->order;
			$this->db->order_by(key($order), $order[key($order)]);
		}
	}

	private function _get_datatables_query2($var=null, $month=null)
	{		 
		$this->db->select('ess_sppd.id');
		$this->db->select('ess_sppd.np_karyawan');
		$this->db->select('ess_sppd.nama');
		$this->db->select('ess_sppd.nama_jabatan');
		$this->db->select('ess_sppd.perihal');
		$this->db->select('ess_sppd.tgl_berangkat');
		$this->db->from($this->table);	
		$this->db->where("YEAR(tgl_berangkat) < 2024");
		$this->db->not_like('perihal', 'rapat');
		
		
		if($_SESSION["grup"]==4) //jika Pengadministrasi Unit Kerja
		{
			$this->db->where_in('kode_unit', $var);								
		}else
		if($_SESSION["grup"]==5) //jika Pengguna
		{
			$this->db->where_in('np_karyawan', $var);	
		}else
		{
		}

		if(@$month!=0){
            $this->db->where("DATE_FORMAT(tgl_berangkat,'%Y-%m')", $month);
        }
				
		$i = 0;
	
		foreach ($this->column_search as $item) // loop column 
		{
			if($_POST['search']['value']) // if datatable send POST for search
			{
				
				if($i===0) // first loop
				{
					$this->db->group_start(); // open bracket. query Where with OR clause better with bracket. because maybe can combine with other WHERE with AND.
					$this->db->like($item, $_POST['search']['value']);
				}
				else
				{
					$this->db->or_like($item, $_POST['search']['value']);
				}

				if(count($this->column_search) - 1 == $i) //last loop
					$this->db->group_end(); //close bracket
			}
			$i++;
		}
		
		if(isset($_POST['order'])) // here order processing
		{
			$this->db->order_by($this->column_order[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
		} 
		else if(isset($this->order))
		{
			$order = $this->order;
			$this->db->order_by(key($order), $order[key($order)]);
		}
	}

	function get_datatables($var=null, $month=null)
	{
		$this->_get_datatables_query($var,$month);
		
		if($_POST['length'] != -1)
		// $this->db->limit($_POST['length'], $_POST['start']);
		$query = $this->db->get()->result();
		
		$this->_get_datatables_query2($var,$month);
		
		if($_POST['length'] != -1)
		// $this->db->limit($_POST['length'], $_POST['start']);
		$query2 = $this->db->get()->result();
		
		$combined_results = array_merge($query, $query2);
		
		if(isset($_POST['order'])) // here order processing
		{
			usort($combined_results, function ($a, $b) {
				if ($_POST['order']['0']['column'] == 3){
					if ($_POST['order']['0']['dir'] == 'asc'){
						return strcasecmp($a->nama_jabatan, $b->nama_jabatan);
					} else {
						return strcasecmp($b->nama_jabatan, $a->nama_jabatan);
					}
				} else if ($_POST['order']['0']['column'] == 4){
					if ($_POST['order']['0']['dir'] == 'asc'){
						return strcasecmp($a->perihal, $b->perihal);
					} else {
						return strcasecmp($b->perihal, $a->perihal);
					}
				}  else if ($_POST['order']['0']['column'] == 5){
					if (strtotime($a->tgl_berangkat) == strtotime($b->tgl_berangkat)) {
						return 0;
					}
					if ($_POST['order']['0']['dir'] == 'asc'){
						return (strtotime($a->tgl_berangkat) > strtotime($b->tgl_berangkat)) ? -1 : 1;
					} else {
						return (strtotime($a->tgl_berangkat) < strtotime($b->tgl_berangkat)) ? -1 : 1;
					}
				}
			});
		} 

		return $combined_results;
	}

	function count_filtered($var=null, $month=null)
	{
		$this->_get_datatables_query($var, $month);
		$query = $this->db->get();

		$this->_get_datatables_query2($var, $month);
		$query2 = $this->db->get();

		$combined_results = array_merge($query->result_array(), $query2->result_array());
		return count($combined_results);
	}

	public function count_all($var=null, $month=null)
	{
		$this->db->from($this->table);
		
		if($_SESSION["grup"]==4) //jika Pengadministrasi Unit Kerja
		{
			$this->db->where_in('kode_unit', $var);								
		}else
		if($_SESSION["grup"]==5) //jika Pengguna
		{
			$this->db->where_in('np_karyawan', $var);	
		}else
		{
		}	

		if(@$month!=0){
            $this->db->where("DATE_FORMAT(tgl_berangkat,'%Y-%m')", $month);
        }
			
		return $this->db->count_all_results();
	}
	
	
}

/* End of file m_perencanaan_jadwal_kerja.php */
/* Location: ./application/models/kehadiran/m_perencanaan_jadwal_kerja.php */