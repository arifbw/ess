...
class M_tabel_persetujuan extends CI_Model {
    ...
    var $column_order = array(null, 'np_karyawan', 'pelatihan', 'tanggal_pelatihan', 'skala_prioritas', 'np_karyawan', 'np_karyawan','created_at'); //set column field database for datatable orderable
    var $column_search = array('np_karyawan','pelatihan','tanggal_pelatihan'); //set column field database for datatable searchable 
    ...
    function approve_all($data) {
        // Start a transaction to ensure data consistency
        $this->db->trans_start();

        // Update status_1 and approval_1_date where approval_1 matches and status_1 is 0
        $this->db->where('approval_1', $data['approval_pimpinan_np']);
        $this->db->where('status_1', '0');
        $this->db->update('ess_diklat_kebutuhan_pelatihan', [
            'status_1' => $data['approval_pimpinan_status'],
            'approval_1_date' => date('Y-m-d H:i:s')
        ]);

        // Update status_2 and approval_2_date where approval_2 matches and status_2 is 0
        $this->db->where('approval_2', $data['approval_pimpinan_np']);
        $this->db->where('status_2', '0');
        $this->db->update('ess_diklat_kebutuhan_pelatihan', [
            'status_2' => $data['approval_pimpinan_status'],
