...
class M_data_perizinan extends CI_Model {
    ...
    function get_mst_perizinan(){
        $not_in = ['AB','ATU','TK','TM', 'D'];
		$this->db->select('nama, kode_pamlek, kode_erp, status')
         ->where_not_in('kode_pamlek', $not_in)
		 ->where('status', 1)
         ->order_by('kode_pamlek');
		$query = $this->db->get('mst_perizinan');
        return $query;
    }
	...
	function select_daftar_karyawan_and_outsource()
	{		
		$query1 = $this->db->select('nama, no_pokok')
				->from('mst_karyawan')
				->get_compiled_select();

		$query2 = $this->db->select('nama, np_karyawan as no_pokok')
				->from('ess_karyawan_outsource')
				->get_compiled_select();
	...
	public function ambil_perizinan_id($id, $table){
