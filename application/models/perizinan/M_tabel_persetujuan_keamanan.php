...
class M_tabel_persetujuan_keamanan extends CI_Model {
	...
	private function _get_datatables_query($var, $params, $jenis=null, $pos='') {
        if($pos!=''){
			$get_pos = $this->db->where('id', $pos)->get('mst_pos')->row()->no_pokok;
            $this->db->where("(a.pos like '%\"".$pos."\"%')");
            // $this->db->where("(a.pos like '%\"".$pos."\"%') and (find_in_set('".$_SESSION['no_pokok']."', '".$get_pos."'))");
        }
        /* else {
			$get_pos = $this->db->where('status', '1')->where("find_in_set('".$_SESSION['no_pokok']."', no_pokok)")->get('mst_pos')->result_array();
			$pos_in = array_column('id', $get_pos);
        ...
        // $this->db->select("a.*, (CASE WHEN start_date is not null then start_date else end_date end) as ordere")->from($params['table_name']." a");
		...
		// $this->db->where('(approval_1_status!="2" AND approval_2_status!="2")');
        
        # tambahan untuk filter date range, 2021-02-24
        $this->db->group_start();
        $this->db->where("(a.start_date BETWEEN '".$params['startDate']."' AND '".$params['endDate']."')");
        $this->db->or_where("(a.end_date BETWEEN '".$params['startDate']."' AND '".$params['endDate']."')");
        $this->db->group_end();
	...
	function get_datatables($var, $params, $jenis=null, $pos='')
	{
		$this->_get_datatables_query($var, $params, @$jenis, $pos);
	...
	function count_filtered($var, $params, $jenis=null, $pos='')
	{
		$this->_get_datatables_query($var, $params, @$jenis);
		$query = $this->db->get();
		return $query->num_rows();
	}

	public function count_all($var, $params, $jenis=null, $pos='')
	{
        $this->_get_datatables_query($var, $params, @$jenis);
