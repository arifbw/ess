...
class M_tabel_persetujuan_perizinan extends CI_Model {
	...
	private function _get_datatables_query($var, $params, $jenis=null) {
		...
		} else if($_SESSION["grup"]==5) //jika Pengguna
		{
			...
			$this->db->where('((approval_1_np="'.$var.'" AND approval_1_status is null) OR (approval_2_np="'.$var.'" AND approval_2_status is null))');
		} else
		{
		}
		//persetujuan
        ...
        $table = $params['bulan'];
        $tahun = substr($table, 14, 4);
        $bulan = substr($table, 19, 2);
        if($tahun!='' && $bulan!=''){
            $this->db->group_start();
            $this->db->where("(DATE_FORMAT(start_date,'%Y-%m')='$tahun-$bulan')");
            $this->db->or_where("(DATE_FORMAT(end_date,'%Y-%m')='$tahun-$bulan')");
            $this->db->group_end();
        }
