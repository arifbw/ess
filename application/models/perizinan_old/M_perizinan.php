    ...
    function update_cico($data){
        ...
        $get_perizinan = $this->db->query("SELECT GROUP_CONCAT(id) AS id_perizinan 
										...
										(
                                            (kode_pamlek!='0' AND
                                            (
                                                (CONCAT(start_date,' ',start_time) BETWEEN DATE_SUB(CONCAT('".$data['date_time_in']."'), INTERVAL 1 HOUR) AND DATE_ADD(CONCAT('".$data['date_time_out']."'), INTERVAL 1 HOUR)) OR 
				                                (CONCAT(end_date,' ',end_time) BETWEEN DATE_SUB(CONCAT('".$data['date_time_in']."'), INTERVAL 1 HOUR) AND DATE_ADD(CONCAT('".$data['date_time_out']."'), INTERVAL 1 HOUR))
                                            ))
                                            OR
                                            (kode_pamlek='0' AND
                                            (
