...
class M_agenda extends CI_Model
{
	...
	private function _get_datatables_query()
	{
		...
		/*if (!in_array($this->session->userdata('grup'), [5, 4])) {
			...
			$this->db->where("
					CASE 
						WHEN a.np_tergabung != 'all' THEN FIND_IN_SET('$no_pokok', a.np_tergabung)>0
						ELSE 1 = 1 
					END
				");
		}*/
		...
		if(@$this->session->userdata('grup')){
			if(in_array($this->session->userdata('grup'), [30])){
				// no filter
			} else{
				$this->db->group_start();
				$this->db->where('a.np_tergabung', 'all');
				$this->db->or_where('a.np_tergabung is null', null, false);
				$this->db->or_where("FIND_IN_SET('{$no_pokok}', a.np_tergabung)");
				$this->db->group_end();
			}
		} else{
