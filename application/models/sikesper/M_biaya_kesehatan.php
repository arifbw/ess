...
class M_biaya_kesehatan extends CI_Model {
	...
	var $column_order = array(null,'np_karyawan','nama_pegawai','nama_unit_singkat','bill_no','nama_vendor',null,null);
	...
	private function _get_datatable_query($np, $vendor, $filter_status, $tahun) {
		...
		{
			if($_POST['search']['value']) // if datatable send GET for search
			{
	...
	function get_datatable($np, $vendor=null, $filter_status, $tahun){
	...
	function count_filtered($np, $vendor=null, $filter_status, $tahun){
		$this->_get_datatable_query($np, $vendor, $filter_status, $tahun);
		$query = $this->db->get();
		return $query->num_rows();
	}

	public function count_all($np, $vendor=null, $filter_status, $tahun){		
		$this->_get_datatable_query($np, $vendor, $filter_status, $tahun);
