...
class M_pemeriksaan_spbe_keluar extends CI_Model {
	...
	private function _get_datatables_query($params) {
		...
		switch ($params['otoritas']) {
			case 'petugas pamsiknilmat':
				$this->db->order_by('pengecek1_status', 'ASC');
				break;
			case 'komandan posko':
				$this->db->order_by('danposko_status', 'ASC');
				break;
			case 'pengguna':
				$this->db->where('konfirmasi_pengguna_np',$params['np']);
				$this->db->order_by('konfirmasi_pengguna', 'ASC');
				break;
			default:
				# code...
				break;
