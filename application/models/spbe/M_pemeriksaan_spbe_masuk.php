...
class M_pemeriksaan_spbe_masuk extends CI_Model {
    
	var $table = "ess_permohonan_spbe";
	...
	private function _get_datatables_query($params) {
        ...
        $this->db->where('approval_atasan_status','1');
        $this->db->where('barang_kembali','1');
        $this->db->where('approval_pengamanan_keluar','1');

		switch ($params['otoritas']) {
			case 'petugas pamsiknilmat':
				$this->db->order_by('approval_pengamanan_masuk', 'ASC');
				break;
			case 'pengguna':
				$this->db->where('konfirmasi_pengguna_np',$params['np']);
				$this->db->order_by('konfirmasi_pembawa_status', 'ASC');
				break;
			default:
				# code...
				break;
