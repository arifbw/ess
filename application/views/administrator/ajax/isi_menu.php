			...
			echo "<div class='well well-sm'>";
				echo "<div class='row'>";
					echo "<div class='col-md-1'>";
						echo "<i class='fa ".$daftar_menu[$i]["icon"]." fa-fw'></i>";
					echo "</div>";
					echo "<div class='col-md-3'>";
						echo "<input type='hidden' id='isian_menu_lama_$i' value='".$daftar_menu[$i]["id_modul"]."'/>";
						echo "<select class='form-control' onchange='ubah(this)' id='isian_menu_$i' $disabled>";
							$optgroup = "";
							for($j=0;$j<count($pilihan_modul);$j++){
								if((int)$daftar_menu[$i]["id_modul"]==(int)$pilihan_modul[$j]["id"]){
								...
								if(strcmp($optgroup,$pilihan_modul[$j]["nama_kelompok_modul"])!=0){
									$optgroup=$pilihan_modul[$j]["nama_kelompok_modul"];
									echo "<optgroup label='$optgroup'>";
								}
								
								echo "<option value=".$pilihan_modul[$j]["id"]." $selected>".$pilihan_modul[$j]["nama"]."</option>";
								
								if($j==count($pilihan_modul)-1 or strcmp($optgroup,$pilihan_modul[$j+1]["nama_kelompok_modul"])!=0){
									echo "</optgroup>";
								}
							}
						echo "</select>";
					...
					echo "<div class='col-md-6'>";
						echo "URL : <a href='".base_url($daftar_menu[$i]["url"])."'>".base_url($daftar_menu[$i]["url"])."</a>";
					echo "</div>";
