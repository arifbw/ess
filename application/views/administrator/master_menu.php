							...
							<table width="100%" class="table table-striped table-bordered table-hover" id="tabel_grup_pengguna">
								<thead>
									<tr>
										...
										<?php
											if($this->akses["ubah"] or $this->akses["isi"] or $this->akses["lihat_log"]){
								...
								<tbody>
									<?php
										for($i=0;$i<count($daftar_master_menu);$i++){
											if($i%2==0){
												$class = "even";
											}
											else{
											...
											echo "<tr class='$class'>";
												echo "<td>".$daftar_master_menu[$i]["nama"]."</td>";
												echo "<td>";
													if((int)$daftar_master_menu[$i]["status"]==1){
														echo "Aktif";
													}
													else if((int)$daftar_master_menu[$i]["status"]==0){
														echo "Non Aktif";
													}
												echo "</td>";
												if($this->akses["ubah"] or $this->akses["isi"] or $this->akses["lihat log"]){
													echo "<td class='text-center'>";
														...
														if($this->akses["isi"]){
															echo "<a class='btn btn-primary btn-xs' href='".base_url($url_isi_menu."/".$daftar_master_menu[$i]["nama"])."'>Isi</a> ";
														}
														if($akses["lihat log"]){
															echo "<button class='btn btn-primary btn-xs' onclick='lihat_log(\"".$daftar_master_menu[$i]["nama"]."\",".$daftar_master_menu[$i]["id"].")'>Lihat Log</button>";
														}
													echo "</td>";
												}
											echo "</tr>";
