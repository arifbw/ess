	...
	<div class="container-fluid">
				...
				<div class="panel panel-default">
					...
					<div id="collapseOne" class="panel-collapse collapse">
						<div class="panel-body">
							<form role="form" action="<?php echo base_url('faskar/internet/detail/action_insert'); ?>" id="formulir_tambah" method="post" enctype="multipart/form-data">
								<input type="hidden" name="faskar_internet_header_id" value="<?= $header->id?>">
								...
								<div class="row form-group">
									...
									<div class="col-lg-6">
										<input class="form-control" type="text" name="np_karyawan" id="np_karyawan" minlength="4" onchange="prosesNp()" required>
									</div>														
								</div>
		...
		<div class="row">
			<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
				<a href="<?= base_url('faskar/internet/header')?>" class="btn btn-default btn-md"><i class="fa fa-arrow-left"></i> Kembali</a>
			</div>
		...
		<div class="row table-responsive">
			<div class="col-lg-12">
				<table width="100%" class="table table-striped table-bordered table-hover" id="tabel_internet">
					<thead>
						<tr>
		...
		<div class="modal fade" id="modal-upload" aria-labelledby="label_modal_upload" aria-hidden="true" data-backdrop="static" data-keyboard="false">
		 	<div class="modal-dialog modal-dialog-centered" role="document">
		    	<div class="modal-content">
					...
					<form role="form" action="<?php echo base_url('faskar/internet/detail/action_import'); ?>" id="formulir_upload" method="post" enctype="multipart/form-data">
		      		<div class="modal-body">
						...
						<div class="alert alert-info">
							...
							<a class="btn btn-success" href="<?= base_url('file/template/template-upload-faskar-internet.xlsx')?>" target="_blank"><b><i class="fa fa-file-excel-o"></i> Download contoh file</b></a>
						</div>
						<div class="row">
							<div class="form-group">
								<div class="col-lg-3">
...
<script type="text/javascript">
	...
	const initAll = async() =>{
		...
		$('#page-header-title').html(`Pemakaian internet ${header_lokasi} Bulan ${moment(header_pemakaian_bulan).format('MMMM YYYY')}`);
		$('#upload-title').html(`Import Data Pemakaian internet ${header_lokasi} Bulan ${moment(header_pemakaian_bulan).format('MMMM YYYY')}`);
		tableServerside();
	}

	const tableServerside = async () => {
		let header_id = '<?= $header->id?>';
		table = $('#tabel_internet').DataTable({ 
			"iDisplayLength": 10,
			"language": {
			...
			"ordering": false,
			"ajax": {				
				"url"	: "<?php echo site_url("faskar/internet/detail/tabel_internet")?>",					 
				"type"	: "POST",
	...
	$("#btn-cancel-form").on('click', function(e){
		...
		$('#formulir_tambah').find('[name=faskar_internet_header_id]').val('<?= $header->id?>');
	...
	const hapus = async (data) =>{
		...
		if (result) {
			$.ajax({
				type: "POST",
				url: `<?= base_url('faskar/internet/detail/hapus')?>`,
				data: {kode: data.kode},
				dataType: 'json',
	...
	const edit = async (data) => {
		...
		let fields = ['np_karyawan', 'nama_karyawan', 'no_hp', 'pemakaian', 'keterangan'];
	...
	const getAllPlafon = async()=>{
		let data = await $.ajax({
			...
			data: { table_name: 'mst_plafon_internet' },
			dataType: 'json',
		})
		console.log(data);
