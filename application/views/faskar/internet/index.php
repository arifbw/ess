				...
				<div class="panel panel-default">
					<div class="panel-heading">
						<h4 class="panel-title">
							<a data-toggle="collapse" data-parent="#accordion" href="#collapseOne">Tambah Periode</a>
						</h4>
					</div>
					<div id="collapseOne" class="panel-collapse collapse">
						<div class="panel-body">
							<form role="form" action="<?php echo base_url('faskar/internet/header/save_new_header'); ?>" id="formulir_tambah" method="post" enctype="multipart/form-data">
								<div class="row" id="">
									...
									<div class="form-group">
										...
										<div class="col-lg-6">
											<select class="form-control" name="layanan" id="layanan" style="width: 100%" required>
												<option value="Satelindo">SATELINDO</option>
											</select>
										</div>														
									</div>
								</div>
...
<script type="text/javascript">
	...
	const tableServerside = async () => {
		table = $('#tabel_header').DataTable({ 
			...
			"ordering": false,
			"ajax": {				
				"url"	: "<?php echo site_url("faskar/internet/header/tabel_header")?>",					 
				"type"	: "POST",
				"data"	: {}
			},
			"columnDefs": [
			...
			columns: [
				...
				},
				{
					render: (data, type, row) => {
						...
						if( row.submit_date===null && parseInt(row.jumlah_data) > 0 ){
							const ajukan = $('<button/>', {
								html: 'Ajukan',
						...
						let urlDetail = '<?= base_url('faskar/internet/detail/data')?>';
						const detail = $('<a/>', {
							html: 'Detail',
	...
	const ajukan = (data) => {
		...
		$.ajax({
			type: "POST",
			url: `<?= base_url('faskar/internet/header/ajukan')?>`,
			data: data,
			dataType: 'html',
