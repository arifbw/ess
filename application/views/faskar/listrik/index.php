				...
				<div class="panel panel-default">
					...
					<div id="collapseOne" class="panel-collapse collapse">
						<div class="panel-body">
							<form role="form" action="<?php echo base_url('faskar/listrik/header/save_new_header'); ?>" id="formulir_tambah" method="post" enctype="multipart/form-data">
...
<script type="text/javascript">
	...
	const tableServerside = async () => {
		table = $('#tabel_header').DataTable({ 
			...
			"ordering": false,
			"ajax": {				
				"url"	: "<?php echo site_url("faskar/listrik/header/tabel_header")?>",					 
				"type"	: "POST",
				"data"	: {}
			},
			"columnDefs": [
			...
			columns: [
				...
				},
				{
					render: (data, type, row) => {
						...
						let urlDetail = '<?= base_url('faskar/listrik/detail/data')?>';
						const detail = $('<a/>', {
							html: 'Detail',
	...
	const ajukan = (data) => {
		...
		$.ajax({
			type: "POST",
			url: `<?= base_url('faskar/listrik/header/ajukan')?>`,
			data: data,
			dataType: 'html',
	...
	const view_info = (data) =>{
		data['judul'] = 'Status Pemakaian Listrik';
		$("#modal-content-ajukan").html('Loading...');
        $("#modal-ajukan").modal('show');
		$.ajax({
			type: "POST",
	...
	// 	$.ajax({
	// 		type: "POST",
	// 		url: `<?= base_url('faskar/listrik/detail')?>`,
	// 		data: data,
	// 		dataType: 'html',
	// 	}).then(function(response){
