...
<script type="text/javascript">
	...
	const tableServerside = async () => {
		table = $('#tabel_header').DataTable({ 
			...
			"ordering": false,
			"ajax": {				
				"url"	: "<?php echo site_url("faskar/persetujuan/internet/header/tabel_header")?>",					 
				"type"	: "POST",
				"data"	: {}
			},
			"columnDefs": [
			...
			columns: [
				...
				},
				{
					render: (data, type, row) => {
						let label='';
						let url = '<?= base_url('faskar/persetujuan/internet/detail/data')?>';
						const detail = $('<a/>', {
							html: 'Detail',
	...
	const approval = (data) =>{
		...
		$.ajax({
			type: "POST",
			url: `<?= base_url('faskar/persetujuan/internet/header/approval')?>`,
			data: data,
			dataType: 'html',
