	...
	<div class="container-fluid">
				...
				<div class="panel panel-default">
					...
					<div id="collapseOne" class="panel-collapse collapse">
						<div class="panel-body">
							<form role="form" action="<?php echo base_url('faskar/tv/detail/action_insert'); ?>" id="formulir_tambah" method="post" enctype="multipart/form-data">
								<input type="hidden" name="faskar_tv_header_id" value="<?= $header->id?>">
								...
								<div class="row form-group">
									<div class="col-lg-3">
										<label>No. ID <span style="color: red">*</span></label>
									</div>
									<div class="col-lg-6">
										<input class="form-control" type="text" name="no_id" id="no_id" onchange="getPlafonById();" required>
									</div>														
								</div>
								...
								<div class="row form-group">
									...
									<div class="col-lg-6">
										<input class="form-control" type="number" name="plafon" id="plafon" required readonly>
		...
		<div class="row">
			<div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
				<a href="<?= base_url('faskar/tv/header')?>" class="btn btn-default btn-md"><i class="fa fa-arrow-left"></i> Kembali</a>
			</div>
		...
		<div class="modal fade" id="modal-upload" aria-labelledby="label_modal_upload" aria-hidden="true" data-backdrop="static" data-keyboard="false">
		 	<div class="modal-dialog modal-dialog-centered" role="document">
		    	<div class="modal-content">
					...
					<form role="form" action="<?php echo base_url('faskar/tv/detail/action_import'); ?>" id="formulir_upload" method="post" enctype="multipart/form-data">
		      		<div class="modal-body">
						...
						<div class="alert alert-info">
							...
							<a class="btn btn-success" href="<?= base_url('file/template/template-upload-faskar-tv.xlsx')?>" target="_blank"><b><i class="fa fa-file-excel-o"></i> Download contoh file</b></a>
						</div>
						<div class="row">
...
<script type="text/javascript">
	...
	const initAll = async() =>{
		...
		$('#page-header-title').html(`Pemakaian TV Kabel ${header_lokasi} Bulan ${moment(header_pemakaian_bulan).format('MMMM YYYY')}`);
		$('#upload-title').html(`Import Data Pemakaian TV Kabel ${header_lokasi} Bulan ${moment(header_pemakaian_bulan).format('MMMM YYYY')}`);
		tableServerside();
	}
	...
	const tableServerside = async () => {
		...
		table = $('#tabel_tv').DataTable({ 
			...
			"ordering": false,
			"ajax": {				
				"url"	: "<?php echo site_url("faskar/tv/detail/tabel_tv")?>",					 
				"type"	: "POST",
			...
			columns: [
				...
				},
				{
					render: (data, type, row) => {
						let plafon;
						if(row.plafon!=null) plafon = 'Rp ' + parseInt(row.plafon).toLocaleString();
						else plafon = '-'
						return plafon;
					}
				},
	...
	$("#btn-cancel-form").on('click', function(e){
		...
		$('#formulir_tambah').find('[name=faskar_tv_header_id]').val('<?= $header->id?>');
	...
	const hapus = async (data) =>{
		...
		if (result) {
			$.ajax({
				type: "POST",
				url: `<?= base_url('faskar/tv/detail/hapus')?>`,
				data: {kode: data.kode},
				dataType: 'json',
	...
	const edit = async (data) => {
		...
		let fields = ['np_karyawan', 'nama_karyawan', 'no_id', 'pemakaian', 'plafon', 'keterangan'];
		for (const i of fields) {
			$('#formulir_tambah').find(`[name=${i}]`).val(`${data[i]}`);
		}

		let find = await _.find(allPlafon, (o)=>{ return o.np_karyawan==$('#np_karyawan').val() && o.no_id==$('#no_id').val(); });
