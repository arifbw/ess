				...
				<div class="panel panel-default">
					...
					<div id="collapseOne" class="panel-collapse collapse">
						<div class="panel-body">
							<form role="form" action="<?php echo base_url('faskar/tv/header/save_new_header'); ?>" id="formulir_tambah" method="post" enctype="multipart/form-data">
								<div class="row" id="">
								...
								<div class="row" id="">
									<div class="form-group">
										<div class="col-lg-3">
											<label>Layanan <span style="color: red">*</span></label>
										</div>
										<div class="col-lg-6">
											<select class="form-control" name="layanan" id="layanan" style="width: 100%" required>
												<?php foreach($ref_layanan as $row){?>
												<option value="<?= $row['nama']?>" <?= $row['status']=='0' ?' disabled':''?>><?= $row['nama']?></option>
												<?php } ?>
											</select>
										</div>														
									</div>
...
<script type="text/javascript">
	var table;
	var mst_tv_kabel = <?= json_encode($ref_layanan)?>;
	...
	const tableServerside = async () => {
		table = $('#tabel_header').DataTable({ 
			...
			"ordering": false,
			"ajax": {				
				"url"	: "<?php echo site_url("faskar/tv/header/tabel_header")?>",					 
				"type"	: "POST",
				"data"	: {}
			},
			"columnDefs": [
			...
			columns: [
				...
				},
				{
					render: (data, type, row) => {
						...
						let urlDetail = '<?= base_url('faskar/tv/detail/data')?>';
						const detail = $('<a/>', {
							html: 'Detail',
	...
	const ajukan = (data) => {
		...
		$.ajax({
			type: "POST",
			url: `<?= base_url('faskar/tv/header/ajukan')?>`,
			data: data,
			dataType: 'html',
	...
	const edit = async (data) => {
		...
		$('#formulir_tambah').find(`[name=lokasi]`).val(`${data.lokasi}`);
		$('#formulir_tambah').find(`[name=layanan]`).val(`${data.layanan}`);
