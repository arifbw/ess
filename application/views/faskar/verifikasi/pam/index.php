...
<script type="text/javascript">
	...
	const tableServerside = async () => {
		table = $('#tabel_header').DataTable({ 
			...
			"ordering": false,
			"ajax": {				
				"url"	: "<?php echo site_url("faskar/verifikasi/pam/header/tabel_header")?>",					 
				"type"	: "POST",
				"data"	: {}
			},
			"columnDefs": [
			...
			columns: [
				...
				},
				{
					render: (data, type, row) => {
						let label='';
						let url = '<?= base_url('faskar/verifikasi/pam/detail/data')?>';
						const detail = $('<a/>', {
							html: 'Detail',
						...
						const ekspor = $('<a/>', {
							...
							href: `<?= base_url('faskar/export/pam/Export_excel/get_by_header/')?>` + row.kode,
							target: '_blank'
						})
	...
	const approval = (data) =>{
		...
		$.ajax({
			type: "POST",
			url: `<?= base_url('faskar/verifikasi/pam/header/approval')?>`,
			data: data,
			dataType: 'html',
