				...
				<div class="panel panel-default">
					...
					<div id="collapseOne" class="panel-collapse collapse <?php echo $panel_tambah;?>">
						<div class="panel-body">
							<form role="form" action="<?= site_url('food_n_go/master_data/jenis_lembur/index'); ?>" id="formulir_tambah" method="post">
								...
								<div class="form-group">
									<div class="row">
										...
										<div class="col-lg-7">
											<div class='input-group date' id='batas-waktu'>
							                    <input type='text' class="form-control" name="batas_waktu" value="<?= @$data_agenda->batas_waktu; ?>" />
							                    <span class="input-group-addon">
							                        <span class="glyphicon glyphicon-time"></span>
							                    </span>
							                </div>
										</div>
										<div id="warning-batas" class="col-lg-3 text-danger"></div>
									</div>
								</div>
								<div class="form-group">
									<div class="row">
										...
										<div class="col-lg-7">
											<input class="form-control" name="warning" placeholder="Masukkan Peringatan" required>
										</div>
										<div id="warning-peringatan" class="col-lg-3 text-danger"></div>
									</div>
								</div>
								<div class="form-group">
									<div class="row">
		...
		<div class="row">
			<table width="100%" class="table table-striped table-bordered table-hover" id="tabel_daftar_lokasi">
				...
				<tbody>
					<?php 
						$start = 1;
						foreach($daftar_jenis as $val){ 
					?>
					<tr>
						<td><?= $start++; ?></td>
						<td><?= $val->nama; ?></td>
						<td><?= $val->batas_waktu; ?></td>
						<td><?= $val->warning; ?></td>
		        		...
		        		<form role="form" action="<?= site_url('food_n_go/master_data/jenis_lembur/index'); ?>" id="formulir_tambah" method="post">
							...
							<div class="form-group">
								<div class="row">
									...
									<div class="col-lg-7">
										<input id="nama-jenis" class="form-control" name="nama" placeholder="Masukkan Nama Jenis" required>
									</div>
									<div id="warning_nama" class="col-lg-3 text-danger"></div>
								</div>
							</div>
							<div class="form-group">
								<div class="row">
									...
									<div class="col-lg-7">
										<div class='input-group date' id='batas-waktu-ubah'>
						                    <input type='text' class="form-control ubah-batas" name="batas_waktu" required />
						                    <span class="input-group-addon">
						                        <span class="glyphicon glyphicon-time"></span>
						                    </span>
						                </div>
							...
							<div class="form-group">
								<div class="row">
									...
									<div class="col-lg-7">
										<input class="form-control ubah-peringatan" name="warning" placeholder="Masukkan Peringatan" required>
									</div>
									<div id="warning-peringatan-ubah" class="col-lg-3 text-danger"></div>
								</div>
							</div>
							<div class="form-group">
								<div class="row">
							...
							<input type="hidden" id="no-jenis" name="no">
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$('#batas-waktu, #batas-waktu-ubah').datetimepicker({
            format: 'HH:mm'
		});
	    ...
	    $(document).on('click', '.ubah', function(){
	    	...
	    	$.ajax({
	    		url: "<?php echo site_url('food_n_go/master_data/jenis_lembur/detail/'); ?>"+id,
	    		success: function(data) {
	    			$('#nama-jenis').val(data.result.nama);
	    			$('.ubah-batas').val(data.result.batas_waktu);
	    			$('#status-lama').val(data.result.status);
	    			$('#no-jenis').val(data.result.id);
	    			$('.ubah-peringatan').val(data.result.warning);
