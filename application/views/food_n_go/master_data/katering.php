...
<div id="page-wrapper">
	<div class="container-fluid">
				...
				<div class="panel panel-default">
					...
					<div id="collapseOne" class="panel-collapse collapse <?php echo $panel_tambah;?>">
						<div class="panel-body">
							<form role="form" action="<?= site_url('food_n_go/master_data/daftar_katering/index'); ?>" id="formulir_tambah" method="post">
								...
								<div class="form-group">
									<div class="row">
										...
										<div class="col-lg-7">
											<input class="form-control" name="nama_penyedia" placeholder="Masukkan Nama Penyedia" required>
										</div>
									...
									<div class="row">
										...
										<div class="col-lg-7">
											<select name="lokasi" class="form-control select2" style="width: 100%;">
												<?php foreach ($daftar_lokasi as $value) { ?>
		...
		?>
		<div class="row">
			<table width="100%" class="table table-striped table-bordered table-hover" id="tabel_daftar_penyedia">
				<thead>
					<tr>
				...
				<tbody>
					<?php 
						$start = 1;
						foreach($daftar_penyedia as $val){ 
					?>
					<tr>
						<td><?= $start++; ?></td>
		        		...
		        		<form role="form" action="<?= site_url('food_n_go/master_data/daftar_katering/index'); ?>" id="formulir_tambah" method="post">
							...
							<div class="form-group">
								<div class="row">
									...
									<div class="col-lg-7">
										<input id="nama-penyedia" class="form-control" name="nama_penyedia" placeholder="Masukkan Nama Penyedia" required>
									</div>
									<div id="warning_nama" class="col-lg-3 text-danger"></div>
								</div>
							</div>
							<div class="form-group">
								<div class="row">
									...
									<div class="col-lg-7">
										<select id="ubah-lokasi" name="lokasi" class="form-control select2" style="width: 100%;">
											<?php foreach ($daftar_lokasi as $value) { ?>
												<option value="<?= $value->id ?>"><?= $value->nama; ?></option>
											<?php } ?>
										</select>
							...
							<input id="no-penyedia" type="hidden" name="id" />
							<div class="row">
								<div class="col-lg-12 text-center">
									<button type="submit" class="btn btn-primary" onclick="return cek_simpan_tambah()">Simpan</button>
								</div>
...
<script type="text/javascript">
	$(document).ready(function() {
	    $(document).on('click', '.ubah', function(){
	    	...
	    	$.ajax({
	    		url: "<?php echo site_url('food_n_go/master_data/daftar_katering/detail/'); ?>"+id,
	    		success: function(data) {
	    			$('#nama-penyedia').val(data.result.nama_penyedia);
	    			$('#status-lama').val(data.result.status);
	    			$('#no-penyedia').val(data.result.id);

	    			$('#ubah-lokasi').val(data.result.lokasi);
	    			$('#ubah-lokasi').select2().trigger('click');
