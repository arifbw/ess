		...
		<div id="page-wrapper">
			...
			?>
					...
					<form action="<?= site_url('informasi/data_keterlambatan_new/export') ?>" method="POST" target="_blank">
						<div class="row">
							<div class='col-lg-4'>
		...
		<script type="text/javascript">	
			...
			async function initial_detail_value(){
				np = gup("np");
				tahun = gup("tahun");
				kode_unit = gup("kode_unit");

				if(kode_unit) $('#unit_kerja').val(kode_unit).trigger("change");
				/* if(tahun){
					$("#insert_date_awal").val(`${tahun}-01`);
					$("#insert_date_akhir").val(`${tahun}-12`);
				} */
				if (tahun) {
					let currentYear = new Date().getFullYear();
					let currentMonth = new Date().getMonth() + 1; // getMonth() returns 0-based index
					let akhirBulan = (tahun == currentYear) ? `${tahun}-${String(currentMonth).padStart(2, '0')}` : `${tahun}-12`;

					$("#insert_date_awal").val(`${tahun}-01`);
					$("#insert_date_akhir").val(akhirBulan);
				}
				if(np){
					await sleep(1000);
					$("#karyawan").val(np).trigger("change");
					await sleep(500);
			...
			function table_serverside(){
				...
				table = $('#tabel_data_keterlambatan').DataTable({ 
					...
					"ajax": {
						...
						"data": function(d){
							d.kode_unit = document.getElementById("unit_kerja").value;
							d.np_karyawan =  document.getElementById("karyawan").value;
							d.periode_awal = document.getElementById("insert_date_awal").value;
							d.periode_akhir = document.getElementById("insert_date_akhir").value;
						}
					},
