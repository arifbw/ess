...
<div id="page-wrapper">
	...
	<?php
		...
		if($this->akses["lihat"]){
	?>
			<form action="<?= site_url('informasi/data_keterlambatan/export') ?>" method="POST" target="_blank">
				<div class="row">
					<div class='col-lg-8 col-md-8'>
						...
						<?php
							...
							// heru PDS comment ini, sto ambil sesuai periode, 2021-01-09
							if($np!=""){
								for($i=0;$i<count($daftar_akses_unit_kerja);$i++){
									if(strcmp($daftar_akses_unit_kerja[$i]["kode_unit"],$kode_unit)==0){
						...
						<div class='row'>
							...
							<div class='col-lg-5 col-md-5'>
								...
								<select class="form-control select2" name="keterangan" id="keterangan">
									...
									<option value="izin">Izin Datang Terlambat</option>
								</select>
							</div>
						</div>
					</div>
...
<script>
	var if_boleh_pilih_karyawan = <?php echo $akses["pilih seluruh karyawan"] ? "true" : "false"; ?>;
	var if_boleh_pilih_unit = <?php echo count($this->data["daftar_akses_unit_kerja"]) > 1 ? "true" : "false"; ?>;
	...
	function pilihan_karyawan(){
		if(!if_boleh_pilih_karyawan) return false;
		...
		$.ajax({
			...
			url: document.getElementById("base_url").value+"informasi/data_keterlambatan/daftar_karyawan_revisi/",
			dataType: "json",
			data: {
				"unit_kerja" : unit_kerja, 
				// heru PDS tambahkan parameter ini, 2021-01-11
				insert_date_awal: insert_date_awal, insert_date_akhir: insert_date_akhir
			},
	...
	const getUnitByPeriode=()=>{
		...
		$.ajax({
			type: "POST",
			url: `${url}informasi/data_keterlambatan/getUnitByPeriode`,
			dataType: "html",
			data: {
				insert_date_awal : insert_date_awal, insert_date_akhir: insert_date_akhir
			},
