							...
							<div class="col-lg-2">
								<section class="panel">
									<div class="panel panel-default">
										<div class="panel-heading form-inline">
											<div class="form-group">
												<button type="submit" class="btn btn-info btn-sm btn-save">Save</button>
											</div>
										</div>
									</div>
								</section>
										...
										<table class="table table-striped table-hover table-bordered" id="editable-sample" width="100%">
											<thead style="position:relative">
												<tr>
													...
													<th style="width:10%"> Jenis&nbsp;Lembur</th>
													<th style="width:10%"> Alasan&nbsp;Lembur</th>
...
<script type="text/javascript">
	...
	function addNewRow() {
		...
		for (i = 0; i < baris; i++) {
			...
			var newRow = "<tr id=\"tableRow" + lastIndexTable + "\">" +
				...
				"	<td><select class=\"form-control select2 input-sm no_pokok\" onchange=\"changeNpDanTanggal(" + lastIndexTable + ");\" id=\"no_pokok" + lastIndexTable + "\" name=\"no_pokok[]\" required><?= $list_np ?></select><span id=\"error-no_pokok" + lastIndexTable + "\" style=\"font-size: smaller; color: red;\"></span></td>		" +
				// "	<td><input type=\"text\" class=\"form-control input-sm nama\" id=\"nama"+lastIndexTable+"\" name=\"nama[]\" readonly=readonly></td>                 		"+
				"	<td><input type=\"date\" class=\"form-control input-sm tgl_dws\" onchange=\"changeNpDanTanggal(" + lastIndexTable + ");change_tgl(" + lastIndexTable + ");\" id=\"tgl_dws" + lastIndexTable + "\" name=\"tgl_dws[]\" required><span id=\"error-tgl_dws" + lastIndexTable + "\" style=\"font-size: smaller; color: red;\"></span></td>" +
				...
				"	<td><label class=\"jam\" id=\"jam" + lastIndexTable + "\"></label><br><span id=\"error-jam" + lastIndexTable + "\" style=\"font-size: smaller; color: red;\"></span></td>" +
				//M724 - Adnin Diba Purnomo (5 April 2023) untuk dropdown jenis kategori lembur
				//"	<td><input type=\"text\" class=\"form-control input-sm alasan\" id=\"alasan"+lastIndexTable+"\" name=\"alasan[]\" required></td>"  
				"<td><select class=\"form-control select2 alasan\" onchange=\"checkLembur(" + lastIndexTable + ");\" id=\"alasan" + lastIndexTable + "\" name=\"alasan[]\" required><?= $kategori_lembur ?></select></td>     		" +
				"	<td><input type=\"text\" class=\"form-control input-sm keterangan\" id=\"keterangan" + lastIndexTable + "\" name=\"keterangan[]\" required></td>" +
	...
	function selectHeaderCopy() {
		...
		if (coloumnHeader == 'NP') {
			...
			$('.select2').select2();
		} else if (coloumnHeader == 'Jenis Alasan') {
			//M724 - Adnin Diba Purnomo (5 April 2023) untuk copy header alasan lembur
			//$('#rowCopyValue').append('<input type=\"text\" class=\"form-control input-sm\" id=\"copyValue\">');
			$('#rowCopyValue').append('<select class=\"form-control select2 input-sm alasan\"  id=\"copyValue"\"><?= $kategori_lembur ?></select>');
			//M724 - Adnin Diba Purnomo (5 April 2023)
			$('.select2').select2();
		} else if (coloumnHeader == 'Keterangan') {
			$('#rowCopyValue').append('<input type=\"text\" class=\"form-control keterangan input-sm\" id=\"copyValue\">');
		}
	...
	function copyToRow() {
		...
		if (coloumnHeader == 'NP') {
			...
			//7648 Tri Wibowo 06 04 2023 - Copy All Row untuk alasan yg di rubah menjadi dropdown
			/*
			...
			*/
			...
			//end of  7648 Tri Wibowo 06 04 2023


		} else if (coloumnHeader == 'Keterangan') {
			$('.keterangan').each(function(index, item) {
				this.value = copyValue;
			});
		}
		/*
		...
		if (no_pokok != "" && tgl_mulai != "" && jam_mulai != "" && tgl_selesai != "" && jam_selesai != "") {
			$.ajax({
				...
				success: function(res) {
					res = JSON.parse(res);
					if (res.status == true) {
						$(`#jam${id}`).css('border-color', '');
						$(`#error-jam${id}`).text('');
						$('#jam' + id).text(res.msg);
						getPilihanAtasanLembur(id);
						$('.btn-save').removeClass('hidden');
					} else {
						$(`#jam${id}`).css('border-color', 'red');
						$(`#error-jam${id}`).text(res.msg);
						$('#jam' + id).text(res.total);
	...
	//M724 - Adnin Diba Purnomo (5 April 2023) untuk get Alasan
	function getAlasan(id) {
		var alasan = $('#alasan' + id).val();
		$.ajax({
			type: "POST",
			dataType: "html",
			url: "<?php echo base_url('lembur/pengajuan_lembur/ajax_getAlasan'); ?>",
			data: "vnp_karyawan=" + alasan,
			success: function(msg) {
				if (msg != '') {
					//console.log(msg);
					//$("select[id=np_approver"+id+"] option[value="+msg+"]").attr('selected','selected');
					$("#alasan" + id).val(msg).trigger("change");
				} else if ($("#alasan" + id).children().length > 0) {
					$("#alasan" + id).get(0).selectedIndex = 0;
					$("#alasan" + id).trigger("change");
				} else {
					alert('Alasan tidak ditemukan!');
				}
			}
		});
	}
	...
	function changeNpDanTanggal(id) {
		getWaktu(id);
		checkLembur(id);
		getPilihanAtasanLembur(id)
	}

	function checkLembur(id) {
		var no_pokok = $('#no_pokok' + id).val();
		var tgl_dws = $('#tgl_dws' + id).val();
		var jenis = $('#alasan' + id).val() || '';
		var index = 1;
		var arrCheck = [];
		var arrNp = $("select[name='no_pokok[]']")
			.map(function() {
				return $(this).val();
			}).get();
		var arrTgl = $("input[name='tgl_dws[]']")
			.map(function() {
				return $(this).val();
			}).get();
		var arrJenis = $("[name='alasan[]']")
			.map(function() {
				return $(this).val();
			}).get();

		if (no_pokok != '' && tgl_dws != '') {
			for (let index = 0; index < arrNp.length; index++) {
				var currentNp = arrNp[index];
				var currentTgl = arrTgl[index];
				var currentJenis = arrJenis[index];
				var find = arrCheck.filter(arr => arr.no_pokok === currentNp && arr.tgl_dws === currentTgl && arr.jenis === currentJenis);

				if (find.length > 0) {
					console.log('true');
					$(`#tableRow${index + 1}`).css('border', '2px solid red');
					$('.btn-save').addClass('hidden');
				} else {
					console.log('false');
					$(`#tableRow${index + 1}`).css('border', '');
					$('.btn-save').removeClass('hidden');
				}
			...
			$.ajax({
				type: "POST",
				dataType: "html",
				url: "<?php echo base_url('lembur/pengajuan_lembur/ajax_checkLembur'); ?>",
				data: {
					no_pokok: no_pokok,
					tgl_dws: tgl_dws
				},
				success: function(res) {
					res = JSON.parse(res);
					if (res.status == true) {
						$(`#no_pokok${id}`).css('border-color', '');
						$(`#tgl_dws${id}`).css('border-color', '');
						$(`#error-no_pokok${id}`).text('');
						$(`#error-tgl_dws${id}`).text('');
						$('.btn-save').removeClass('hidden');
					} else {
						if (res.field == 'all') {
							$(`#no_pokok${id}`).css('border-color', 'red');
							$(`#tgl_dws${id}`).css('border-color', 'red');
							$(`#error-no_pokok${id}`).text(res.msg);
							$(`#error-tgl_dws${id}`).text(res.msg);
						} else {
							$(`#${res.field + id}`).css('border-color', 'red');
							$(`#error-${res.field + id}`).text(res.msg);
						}
						$('.btn-save').addClass('hidden');
					}
				}
			...
			$.ajax({
				...
				success: function(msg) {
					if (msg != '') {
						...
						var tipe_atasan = get_data.tipe_atasan;
						...
						for (var i = 0; i < arr_atasan.length; i++) {
							...
							if (get_unit.substr(0, kode_unit.length) == kode_unit) {
								// console.log(arr_atasan[i]);

								switch (tipe_atasan) {
									case "kadiv":
										if(arr_atasan[i]["nama_jabatan"].includes("Kepala Divisi")){
											$("#np_approver" + id).val(arr_atasan[i]["no_pokok"]).trigger("change");
										}
										break;
									case "kasek":
										if(arr_atasan[i]["nama_jabatan"].includes("Kepala Seksi")){
											$("#np_approver" + id).val(arr_atasan[i]["no_pokok"]).trigger("change");
										}
										break;
									case "kadep":
										if(arr_atasan[i]["nama_jabatan"].includes("Kepala Departemen")){
											$("#np_approver" + id).val(arr_atasan[i]["no_pokok"]).trigger("change");
										}
										break;
									default:
