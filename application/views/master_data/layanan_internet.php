...
<div id="page-wrapper">
	<div class="container-fluid">
				...
				<div class="panel panel-default">
					...
					<div id="collapseOne" class="panel-collapse collapse <?php echo $panel_tambah;?>">
						<div class="panel-body">
							<form role="form" action="<?= site_url('master_data/layanan_internet/index'); ?>" id="formulir_tambah" method="post">
								<input type="hidden" name="aksi" value="tambah"/>
								<div class="form-group">
									<div class="row">
										<div class="col-lg-3">
											<label>Nama Layanan Internet</label>
										</div>
										<div class="col-lg-7">
											<input class="form-control" name="nama" placeholder="Masukkan Nama layanan_internet" required>
										</div>
		...
		?>
		<div class="row">
			<table width="100%" class="table table-striped table-bordered table-hover" id="tabel_daftar_layanan_internet">
				<thead>
					<tr>
						<th class='text-center'>#</th>
						<th class='text-center'>Layanan Internet</th>
				...
				<tbody>
					<?php 
						$start = 1;
						foreach($daftar_layanan_internet as $val){ 
					?>
					<tr>
						<td><?= $start++; ?></td>
						<td><?= $val->nama; ?></td>
		        		...
		        		<form role="form" action="<?= site_url('master_data/layanan_internet/index'); ?>" id="formulir_tambah" method="post">
							...
							<div class="form-group">
								<div class="row">
									...
									<div class="col-lg-7">
										<input id="nama-layanan_internet" class="form-control" name="nama" placeholder="Masukkan Nama layanan_internet" required>
									</div>
									<div id="warning_nama" class="col-lg-3 text-danger"></div>
								</div>
							</div>
							...
							<input type="hidden" id="no-layanan_internet" name="no">
...
<script type="text/javascript">
	$(document).ready(function() {
	    $(document).on('click', '.ubah', function(){
	    	...
	    	$.ajax({
	    		url: "<?php echo site_url('master_data/layanan_internet/detail/'); ?>"+id,
	    		success: function(data) {
	    			$('#nama-layanan_internet').val(data.result.nama);
	    			$('#status-lama').val(data.result.status);
	    			$('#no-layanan_internet').val(data.result.id);
