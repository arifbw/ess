                    ...
                    <div class="panel panel-default">
                        ...
                        <div id="collapseOne" class="panel-collapse collapse">
                            <div class="panel-body">
                                <form role="form" action="<?= base_url(); ?>master_data/plafon/tv/action_insert" id="formulir_tambah" method="post" enctype="multipart/form-data">
                                    <div class="form-group row">
                                        <div class="col-lg-2">
                                            <label>NP Karyawan</label>
                                        </div>
                                        <div class="col-lg-7">
                                            <select class="form-control select2" name="np_karyawan" id="np_karyawan" style="width: 100%" onchange="findDataKaryawan();" required>
                                                <option></option>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-lg-7">
                                            <input class="form-control" type="text" name="no_hp" id="no_hp" placeholder="Masukkan No HP" required>
                                        </div>														
                                    </div>
                    ...
                    <table width="100%" class="table table-striped table-bordered table-hover" id="data-table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center no-sort">Tanggal Pensiun</th>
                                <th class="text-center no-sort">No HP</th>
...
<script type="text/javascript">
    ...
    const tableServerside = async () => {
		table = $('#data-table').DataTable({
			...
			"ordering": false,
			"ajax": {				
				"url"	: "<?php echo site_url("master_data/plafon/tv/tabel_tv")?>",					 
				"type"	: "POST",
				"data"	: {}
			},
			"columnDefs": [
    ...
    const edit = async (data) => {
		...
		let fields = ['np_karyawan', 'nama_karyawan', 'nama_jabatan', 'tanggal_pensiun', 'no_hp', 'plafon'];
    ...
    const hapus = async (data) =>{
		...
		if (result) {
			$.ajax({
				type: "POST",
				url: `<?= base_url('master_data/plafon/tv/hapus')?>`,
				data: {id: data.id},
				dataType: 'json',
    ...
    const findDataKaryawan = async ()=>{
        ...
        let findData = await _.find(allKaryawan, function(o) { return o.no_pokok === `${np_karyawan}`; });
        if (typeof findData!='undefined'){
            ...
            $("#nama_karyawan").val('');
            $("#nama_jabatan").val('');
            console.log('Data Karyawan not found');
        }
    }
</script>