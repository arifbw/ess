...
<div id="page-wrapper">
	<div class="container-fluid">
				...
				<div class="panel panel-default">
					...
					<div id="collapseOne" class="panel-collapse collapse <?php echo $panel_tambah;?>">
						<div class="panel-body">
							<form role="form" action="<?= site_url('master_data/tv_kabel/index'); ?>" id="formulir_tambah" method="post">
								<input type="hidden" name="aksi" value="tambah"/>
								<div class="form-group">
									<div class="row">
										<div class="col-lg-2">
											<label>Nama Layanan TV Kabel</label>
										</div>
										<div class="col-lg-7">
											<input class="form-control" name="nama" placeholder="Masukkan Nama Layanan TV Kabel" required>
										</div>
		...
		?>
		<div class="row">
			<table width="100%" class="table table-striped table-bordered table-hover" id="tabel_daftar_tv_kabel">
				<thead>
					<tr>
						<th class='text-center'>#</th>
						<th class='text-center'>Layanan TV Kabel</th>
				...
				<tbody>
					<?php 
						$start = 1;
						foreach($daftar_tv_kabel as $val){ 
					?>
					<tr>
						<td><?= $start++; ?></td>
						<td><?= $val->nama; ?></td>
		        		...
		        		<form role="form" action="<?= site_url('master_data/tv_kabel/index'); ?>" id="formulir_tambah" method="post">
							...
							<div class="form-group">
								<div class="row">
									...
									<div class="col-lg-7">
										<input id="nama-tv_kabel" class="form-control" name="nama" placeholder="Masukkan Nama tv_kabel" required>
									</div>
									<div id="warning_nama" class="col-lg-3 text-danger"></div>
								</div>
							</div>
							...
							<input type="hidden" id="no-tv_kabel" name="no">
...
<script type="text/javascript">
	$(document).ready(function() {
	    $(document).on('click', '.ubah', function(){
	    	...
	    	$.ajax({
	    		url: "<?php echo site_url('master_data/tv_kabel/detail/'); ?>"+id,
	    		success: function(data) {
	    			$('#nama-tv_kabel').val(data.result.nama);
	    			$('#status-lama').val(data.result.status);
	    			$('#no-tv_kabel').val(data.result.id);
