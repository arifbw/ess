					...
					<div class="row">
						<div class="col-lg-3">
							<select class='form-control' id='filter_bulan'>
								...
								<option value='<?= $ls['bln'] ?>' ><?= id_to_bulan(substr($ls['bln'], -2)).' '.substr($ls['bln'], 0, 4) ?></option>
								<?php } ?>
							</select>
						</div>
					...
					<table width="100%" class="table table-striped table-bordered table-hover" id="tabel_ess_lembur_sdm">
						<thead>
							<tr>
								...
								<th class='text-center'>Tanggal Approve SDM</th>
		...
		<script type="text/javascript">	
			function cetak_excel() {
				...
				window.location = "<?= base_url('osdm/laporan_bantuan_cubes/cetak/')?>"+filter_bulan;
			}
		  ...
		  //       });	
			...
			{					
				lembur_table = $('#tabel_ess_lembur_sdm').DataTable({ 					
					...
					"ajax": {
						 "url"	: "<?php echo site_url("osdm/laporan_bantuan_cubes/tabel_laporan_bantuan_cubes/")?>",
						 "data"	: {tahun_bulan: $('#filter_bulan').val()},						 
						 "type"	: "POST"
					},
