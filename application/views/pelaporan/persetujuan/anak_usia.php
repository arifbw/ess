				...
				<?php if($this->akses["lihat"]): ?>			
					...
					<div class="form-group">	
						<div class="row table-responsive">
							<table width="100%" class="table table-striped table-bordered table-hover" id="daftar_anak_usia">
								<thead>
									<tr>
		...
		<script type="text/javascript">
			...
			$(document).ready(function() {
				...
				$('#daftar_anak_usia').DataTable().destroy();				
				table_serverside();
			});
			...
			function table_serverside() {
				...
				table = $('#daftar_anak_usia').DataTable({ 
					"iDisplayLength": 10,
					"language": {
					...
					"ajax": {
						"url": "<?= site_url("pelaporan/persetujuan/anak_usia/tabel_anak_usia/")?>",
						"type": "POST",
					},
		...
		<script>
			$(document).on('click','.detail_button',function(e){
				...
				$.post('<?= site_url("pelaporan/persetujuan/anak_usia/view_detail") ?>',
			...
			$(document).on('click','.persetujuan_button',function(e){
				...
				$.post('<?= site_url("pelaporan/persetujuan/anak_usia/view_approve") ?>',
					{id_:$(this).attr('data-id')},
					function(e){
						$(".set-approve").html(e);
					}
