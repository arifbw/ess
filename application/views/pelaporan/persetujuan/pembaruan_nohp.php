...
<script>
    $(document).ready(function() {
            ...
            $('#modal-inactive').on('show.bs.modal', function(event) {
                ...
                var nohpLama = button.closest('tr').find('td:eq(3)').text(); // Mengambil teks dari kolom nohp lama
                var nohpBaru = button.closest('tr').find('td:eq(4)').text(); // Mengambil teks dari kolom nohp baru
                var modal = $(this);

                modal.find('#message-inactive').html('<b>Nama:</b> ' + nama_karyawan +'<br><b>NP Karyawan:</b> ' + np_karyawan +'<br><b>No HP Lama:</b> ' + nohpLama + '<br><b>No HP Baru:</b> ' + nohpBaru);


                modal.find('#inactive-action').on('click', function(e) {
                    e.preventDefault(); // Mencegah aksi default dari link

                    $.post('<?= base_url() ?>pelaporan/persetujuan/pembaruan_nohp/approve/' + idToDelete, function(response) {
                        if (response === 'success') {
                ...
                modal.find('#inactive-action_1').on('click', function(e) {
                    ...
                    $.post('<?= base_url() ?>pelaporan/persetujuan/pembaruan_nohp/tolak/' + idToDelete, function(response) {
                        if (response === 'success') {
        ...
        $('#laporan_perubahan_nohp_table').DataTable({
            ...
            "serverSide": true,
            "ajax": {
                "url": "<?php echo base_url('pelaporan/persetujuan/pembaruan_nohp/tabel_laporan_perubahan_nohp'); ?>",
                "type": "POST"
            },
            "columns": [{
                    "data": "0"
