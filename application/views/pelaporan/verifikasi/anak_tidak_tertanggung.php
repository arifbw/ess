		...
		<?php if($this->akses["lihat"]): ?>	
			...
			<div class="form-group">	
				<div class="row table-responsive">
					<table width="100%" class="table table-striped table-bordered table-hover" id="daftar_anak_tidak_tertanggung">
						<thead>
							<tr>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('#daftar_anak_tidak_tertanggung').DataTable().destroy();				
		table_serverside();
	});
	...
	function table_serverside() {
		...
		table = $('#daftar_anak_tidak_tertanggung').DataTable({ 
			iDisplayLength: 10,
			language: {
			...
			ajax: {
				url: "<?= site_url("pelaporan/verifikasi/anak_tidak_tertanggung/tabel_anak_tidak_tertanggung/")?>",
				data:{
					status: $("#status").val()
				},
				type: "POST",
	...
	function export_excel(){
		...
		$.ajax({
			url: '<?= base_url('pelaporan/verifikasi/export/anak_tidak_tertanggung/excel') ?>',
			type: 'GET',
			dataType: 'JSON',
			data: filter,
...
<script>
	$(document).on('click','.detail_button',function(e){
		...
		$.post('<?= site_url("pelaporan/verifikasi/anak_tidak_tertanggung/view_detail") ?>',
	...
	$(document).on('click','.persetujuan_button',function(e){
		...
		$.post('<?= site_url("pelaporan/verifikasi/anak_tidak_tertanggung/view_approve") ?>',
			{id_:$(this).attr('data-id')},
			function(e){
				$(".set-approve").html(e);
			}
