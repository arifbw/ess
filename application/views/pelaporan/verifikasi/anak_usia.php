		...
		<?php if($this->akses["lihat"]): ?>			
			...
			<div class="form-group">	
				<div class="row table-responsive">
					<table width="100%" class="table table-striped table-bordered table-hover" id="anak_usia">
						<thead>
							<tr>
...
<script type="text/javascript">
	...
	function table_serverside() {
		...
		table = $('#anak_usia').DataTable({ 
			...
			ajax: {
				url: "<?= site_url("pelaporan/verifikasi/anak_usia/tabel_anak_usia/")?>",
				data:{
					status: $("#status").val()
				},
				type: "POST",
	...
	function export_excel(){
		...
		$.ajax({
			url: '<?= base_url('pelaporan/verifikasi/export/anak_usia/excel') ?>',
			type: 'GET',
			dataType: 'JSON',
			data: filter,
...
<script>
	$(document).on('click','.detail_button',function(e){
		...
		$.post('<?= site_url("pelaporan/verifikasi/anak_usia/view_detail") ?>',
	...
	$(document).on('click','.persetujuan_button',function(e){
		...
		$.post('<?= site_url("pelaporan/verifikasi/anak_usia/view_approve") ?>',
			{id_:$(this).attr('data-id')},
			function(e){
				$(".set-approve").html(e);
			}
