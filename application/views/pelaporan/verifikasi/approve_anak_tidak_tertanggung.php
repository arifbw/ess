			...
			<form role="form" action="<?php echo base_url(); ?>pelaporan/verifikasi/anak_tidak_tertanggung/save_approve/" id="formulir_tambah" method="post">	
				<div class="row">
					<div class="col-md-12">
						<table>
							<tr>
            ...
            $(document).on('click','#set_submit_erp',function(e){
	            ...
	            Swal.fire({
				  	...
				  	if (result.isConfirmed) {
			            $.post('<?php echo site_url("pelaporan/verifikasi/anak_tidak_tertanggung/save_erp") ?>',
			                {id_: '<?= $detail['id'] ?>'},
			                function(get){
			                	ret = JSON.parse(get);
			                	if (ret.status==true) {
