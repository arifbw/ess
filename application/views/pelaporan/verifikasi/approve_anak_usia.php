			...
			<form role="form" action="<?php echo base_url(); ?>pelaporan/verifikasi/anak_usia/save_approve/" id="formulir_tambah" method="post">	
				<div class="row">
					<div class="col-md-12">
						<table>
							<tr>
							...
							<tr>
								<td style="padding-top:20px" colspan="3">
									<b>
										<!-- <a class="btn btn-primary" target="_blank" href="<?= base_url('uploads/pelaporan/anak_usia/surat_keterangan/'.$detail['surat_keterangan']) ?>">Unduh Surat Keterangan</a>
										<a class="btn btn-primary" target="_blank" href="<?= base_url('uploads/pelaporan/anak_usia/kk/'.$detail['kk']) ?>">Unduh Kartu Keluarga</a>
										<a class="btn btn-primary" target="_blank" href="<?= base_url('uploads/pelaporan/anak_usia/akta_kelahiran/'.$detail['akta_kelahiran']) ?>">Unduh Akta Kelahiran</a> -->
				...
				<div id="set_approve">
					<div class="alert alert-<?= $sdm_warna ?>">
						<strong><a class="text-<?= $sdm_warna ?>">Verifikasi Laporan Anak Tertanggung</a></strong><br>
            ...
            $(document).on('click','#set_submit_erp',function(e){
	            ...
	            Swal.fire({
				  	...
				  	if (result.isConfirmed) {
			            $.post('<?php echo site_url("pelaporan/verifikasi/anak_usia/save_erp") ?>',
			                {id_: '<?= $detail['id'] ?>'},
			                function(get){
			                	ret = JSON.parse(get);
			                	if (ret.status==true) {
