			...
			<form role="form" action="<?= base_url(); ?>pelaporan/verifikasi/pindah_alamat/save_approve/" id="formulir_tambah" method="post">	
				<div class="row">
					<div class="col-md-12">
						<table>
							<tr>
							...
							<tr>
								<td style="padding-top:20px" colspan="3">
									<b><a class="btn btn-primary" target="_blank" href="<?= base_url('uploads/pelaporan/pindah_alamat/ktp/'.$detail['ktp']) ?>">Unduh KTP</a></b>
								</td>
								<td></td>
							</tr>
						</table>
			...
			$(document).on('click','#set_submit_erp',function(e){
				...
				Swal.fire({
					...
					if (result.isConfirmed) {
						$.post('<?= site_url("pelaporan/verifikasi/pindah_alamat/save_erp") ?>',
							{id_: '<?= $detail['id'] ?>'},
							function(get){
								ret = JSON.parse(get);
								if (ret.status==true) {
