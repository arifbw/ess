					...
					<table width="100%" class="table table-striped table-bordered table-hover" id="daftar_perceraian">
						<thead>
							<tr>
								...
								<th class="text-center no-sort">Nama Pasangan</th>
...
<script type="text/javascript">
	...
	function table_serverside() {
		...
		table = $('#daftar_perceraian').DataTable({
			...
			ajax: {
				url: "<?= site_url("pelaporan/verifikasi/perceraian/tabel_perceraian/")?>",
				data:{
					status: $("#status").val()
				},
				type: "POST",
	...
	function export_excel(){
		...
		$.ajax({
			url: '<?= base_url('pelaporan/verifikasi/export/perceraian/excel') ?>',
			type: 'GET',
			dataType: 'JSON',
			data: filter,
...
<script>
	$(document).on('click','.detail_button',function(e){
		...
		$.post('<?= site_url("pelaporan/verifikasi/perceraian/view_detail") ?>',
	...
	$(document).on('click','.persetujuan_button',function(e){
		...
		$.post('<?= site_url("pelaporan/verifikasi/perceraian/view_approve") ?>',
			{id_:$(this).attr('data-id')},
			function(e){
				$(".set-approve").html(e);
			}
