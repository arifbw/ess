		...
		<div id="page-wrapper">
			<div class="container-fluid">
				...
				<?php if( count($perizinan_belum_approve_atasan)>0 && $this->akses["persetujuan"] && $_SESSION['grup']=='5' ){?>
				<div class="row">
					<div class="alert alert-warning wrap-collabsible">
						<input id="collapsible" class="toggle" type="checkbox">
						<label for="collapsible" class="lbl-toggle">Permohonan yang <b>belum diapprove</b> Atasan <span class="badge badge-danger"><?php echo count($perizinan_belum_approve_atasan) ?></span></label>
							...
							<div class="content-inner">
								...
								<?php
									foreach ($perizinan_belum_approve_atasan as $key) {
										...
										$tanggal_check = $key->start_date ?: $key->end_date;
										$sudah_cutoff = sudah_cutoff($tanggal_check);
										if( $sudah_cutoff ){
											
										} else{
											// Benerin logic front-end perstujuan (Bewe)
											// echo "<br><span class='persetujuan_button' data-toggle='modal' data-target='#modal_persetujuan' data-id='".$key->id."' data-tgl='".($key->start_date!=null ? $key->start_date: $key->end_date)."'>{$kalimat}</span>";
											echo "<br><span>{$kalimat}</span>";
										}
									}
								?>
								</p>
					...
					<div class="alert alert-info">
						<i class="fa fa-info-circle"></i> Untuk memberikan <strong>persetujuan perizinan</strong>, silakan klik <strong>Tombol Persetujuan</strong> yang tersedia pada kolom aksi di tabel berikut. Pastikan Anda memeriksa setiap detail perizinan dengan teliti sebelum memberikan persetujuan.
					</div>
				</div>
				<?php } ?>
	...
	<!--                                    <input type="hidden" id="get_jenis">-->
		...
		<script type="text/javascript">
            ...
            $(document).on('click','.persetujuan_button',function(e){
				...
				$(".get-approve").html(`
					<div style="min-height: 300px; display: flex; align-items: center; justify-content: center; gap: 10px;">
						<i class="fa fa-spinner infinite-rotate"></i> Mohon Tunggu Sejenak Tampilan Persetujuan sedang dalam proses ...
					</div>
				`);
			...
			function sendRequest(that) {
				$.ajax({
					url: '<?php echo site_url("perizinan/persetujuan_perizinan/view_approve") ?>',
					type: 'POST',
					data: {
						id_perizinan: $(that).attr('data-id'),
						tgl: $(that).attr('data-tgl')
					},
					success: function(e) {
						retryCount = 0;
						$(".get-approve").html(e); // Populate the result on success
					},
					timeout: 1500, // Set timeout to 3 seconds
					error: function(xhr, textStatus) {
						if (textStatus === "timeout" && retryCount < maxRetries) {
							// $(".get-approve").html("Request Timeout, Mencoba Ulang : " + `(${retryCount}/${maxRetries})`);

							/* $(".get-approve").html(`
								<div style="min-height: 300px; display: flex; align-items: center; justify-content: center; gap: 10px;">
									<i class="fa fa-spinner infinite-rotate"></i> Mohon tunggu sejenak ... (Retry : ${retryCount}/${maxRetries})
								</div>
							`); */
							retryCount++; // Increment retry counter
							sendRequest(that); // Retry the request
						} else {
							retryCount = 0;
							$(".get-approve").html("Mohon tunggu sejenak tampilan persetujuan sedang dalam proses, Selanjutnya anda dapat melakukan persetujuan kembali"); // Display error
