<link href="<?php echo base_url('asset/select2') ?>/select2.min.css" rel="stylesheet" />
        	...
        	<div class="container-fluid">
        				...
        				<div class="panel panel-default">
        					...
        					<div id="collapseOne" class="panel-collapse collapse">
        						<div class="panel-body">
        							<form role="form" action="<?php echo base_url(); ?>perizinan/permohonan_perizinan/action_insert_perizinan_ajax" id="formulir_tambah" method="post" onsubmit="return false;">
        								<div class="row">
        								...
        								<?php
										$bulan_lalu = $data_tanggal	= date('Y-m', strtotime('-1 months', strtotime(date('Y-m-d')))) . "-01";
										...
										// robi j971 menampilkan kembali backdate
										?>
        					...
        					<form id="forms">
        						<div class="col-lg-6">
        							...
        							<?php foreach ($jenis_izin as $row) { ?>
        								<div class="checkbox">
        									<label>
        										<input name='izin_<?= $row->kode_pamlek ?>' id='izin_<?= $row->kode_pamlek ?>' class='filter_jenis' type="checkbox" value="1" onclick='table_serverside()'> <?= $row->nama ?>
        									</label>
        								</div>
        							<?php } ?>
        						</div>
        								...
        								<form method="post" target="_blank" action="<?php echo base_url('perizinan/permohonan_perizinan_x/cetak') ?>">
        									<div class="modal-content">
        										...
        										<div class="modal-body">
        											...
        											<input type="hidden" name="range_tgl" value="" id="range_tgl" />
				...
				?>
        			...
        			<div class="modal fade" id="modal_ubah" role="dialog" aria-labelledby="label_modal_ubah" aria-hidden="true">
        				<div class="modal-dialog">
        					<div class="modal-content">
        						<form method="post" action="<?php echo base_url(); ?>perizinan/permohonan_perizinan/permohonan_perizinan">
        							<div class="modal-header">
        								<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
        ...
        <script type="text/javascript">
        	...
        	function table_serverside() {
        		...
        		table = $('#permohonan_perizinan').DataTable({
        			...
        			"ajax": {
        				"url": "<?php echo base_url("perizinan/permohonan_perizinan/permohonan_perizinan/") ?>" + bulan_tahun + '/' + izin_0 + '/' + izin_C + '/' + izin_E + '/' + izin_F + '/' + izin_G + '/' + izin_H + '/' + izin_SIPK,
        				"type": "POST",
        				data: {
        	...
        	function otoritas() {
        		...
        		document.getElementById('range_tgl').value = $('#date_range').val();
        ...
        <script>
        	...
        	function hapus(id, np, tanggal_end, tanggal_start) {
        		Swal.fire({
        			title: 'Konfirmasi',
        			text: "Apakah Anda yakin ingin menghapus perizinan ini?",
        			icon: 'warning',
        			...
        			if (result.isConfirmed) {
        				$.ajax({
        					url: "<?= base_url('perizinan/permohonan_perizinan/hapus_ajax'); ?>",
        					type: "POST",
        					dataType: "json",
        					data: {
        						id: id,
        	...
        	function take_action_date() {
        		...
        		if (add_absence_type == '0|2001|5000') {
        			...
        			$('#start_date').val('');
        			$('#start_time').val('');
        			$('#end_date').data("DateTimePicker").minDate(getMinDate());
        		} else {
        	...
        	function getNamaAtasan1() {
        		var absence_type = $('#add_absence_type').val();
        		...
        		if (np_atasan == " ") {
        			...
        			var tanggal = (absence_type == '0|2001|5000' ? $('#end_date').val() : $('#start_date').val());
        			if (tanggal == '') {
        	...
        	$('#add_absence_type').on('change', (e) => {
        		...
        		if (e.target.value == 'SIPK|2001|5060') alasan_set_to_option();
        	...
        	$('#formulir_tambah').on('submit', function(e) {
        		...
        		Swal.fire({
        			title: 'Konfirmasi',
        			text: "Pastikan isian Anda sudah benar",
        			icon: 'warning',
        			allowOutsideClick: false,
        			showCancelButton: true,
        			...
        			if (result.isConfirmed) {
        				$.ajax({
        					...
        					$('#add_absence_type').trigger('change');
        					$('#add_pos').trigger('change');
        					if (res.status == true) {
        						toastr.success(res.message, '', {
        							"positionClass": "toastr-middle",
        						});
        	...
        	function getMinDate(x = null) {
        		// Get today's date
        		const today = x ?? moment();

        		let minDate;
        		if (today.date() <= 10) {
        			// If today is on or before the 10th, set minDate to the 11th of the previous month
        			minDate = moment(today).subtract(1, 'months').date(11);
        		} else {
        			// If today is after the 10th, set minDate to the 11th of the current month
        			minDate = moment(today).date(11);
        		}
