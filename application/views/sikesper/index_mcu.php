		...
		<div id="page-wrapper">
			<div class="container-fluid">
							...
							<div id="collapseOne" class="panel-collapse collapse <?php echo $panel_tambah;?>">
								<div class="panel-body">
									<div class="text-center">
										<div class="row">
											<div class="col-lg-12">
												<a href="<?= base_url('file/template/IMPORT_MCU.xlsx') ?>" class="btn btn-primary">Download File Template</a>
												<br><br><br>
											</div>
										</div>
				...
				<div class="row">
					...
					<div class='col-lg-4'>
						<select class="form-control select2" name="tahun" id="tahun" onchange="ambil_tanggal()">
							<option value='0'>Pilih Semua Tahun</option>
							<?php for($i=0;$i<count($tahun);$i++){
								echo "<option value='".$tahun[$i]["tahun"]."'>".$tahun[$i]["tahun"]."</option>";
							} ?>
						</select>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class='col-lg-4'>
						<select class="form-control select2" name="vendor" id="vendor" onchange="refresh_table_serverside()">
							<option value='0'>Pilih Semua Vendor</option>
							<?php for($i=0;$i<count($daftar_vendor);$i++){
								echo "<option value='".$daftar_vendor[$i]["vendor"]."'>".$daftar_vendor[$i]["vendor"]."</option>";
							} ?>
						</select>
					</div>

					<?php //if ($akses["pilih seluruh karyawan"]) { ?>
					<div class='col-lg-2 text-right'>Tanggal Upload</div>
					<div class='col-lg-4'>
						<select class="form-control select2" name="tgl" id="tgl" onchange="refresh_table_serverside()">
							<!--<option value='0'>Semua Tanggal Upload</option>-->
							<?php /*for($i=0;$i<count($tanggal_upload);$i++){
								...
								echo "<option value='".$tanggal_upload[$i]["tanggal"]."' $selected>".$tanggal_upload[$i]["tanggal"]."</option>";
							}*/ ?>
						</select>
					</div>
					<?php if ($akses['hapus']) { ?>
							...
							<table width="100%" class="table table-striped table-bordered table-hover" id="tabel_mcu">
								<thead>
									<tr>
										...
										<th class='text-center no-sort' style="width:15%">TANGGAL UPLOAD</th>
										<th class='text-center no-sort'>TANGGAL MCU</th>
		...
		<script>		
			function table_serverside(){
				...
				table = $('#tabel_mcu').DataTable({ 
					...
					"ajax": {
						"url": "<?php echo site_url("sikesper/hasil_mcu/tabel_mcu/")?>"+karyawan+"/"+tgl+"/"+vendor+"/"+tahun,
						"type": "POST"
					},
			...
			$(document).on('click', '.delete', function(e) {
				...
				Swal.fire({
				  ...
				  if (result.value) {
				  	$.ajax({
				  		url: "<?= base_url('sikesper/hasil_mcu/hapus/') ?>",
            ...
            function ambil_tanggal(){
                let param_tahun = $('#tahun').find(':selected').val();
                $('#tgl').html('<option value="0">Semua Tanggal Upload</option>');
                $.ajax({
					url: "<?= base_url('sikesper/hasil_mcu/ambil_tanggal') ?>",
					type: "POST",
					dataType: "html",
					data: {tahun: param_tahun},
