...
$('#modal-approval').on('hidden.bs.modal', function () {
    ...
    $('#form-approval').find('[name=konfirmasi_pengguna]').trigger('change');
});

$('#form-approval').find('[name=konfirmasi_pengguna]').on('change', (e)=>{
    switch (e.target.value) {
        case '1':
            $('#form-approval').find('.div-keterangan').hide();
            $('#form-approval').find('[name=konfirmasi_pengguna_keterangan]').prop('required',false);
            break;
        default:
            $('#form-approval').find('.div-keterangan').show();
            $('#form-approval').find('[name=konfirmasi_pengguna_keterangan]').prop('required',true);
            break;
    }
})
...
const do_submit = async()=>{
    ...
    $.ajax({
        type: "POST",
        url: `${BASE_URL}spbe/konfirmasi_keluar_pengawal/simpan_approval`,
        data: data,
    }).then( (response)=>{
