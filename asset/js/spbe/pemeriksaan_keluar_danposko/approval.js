...
$('#modal-approval').on('hidden.bs.modal', function () {
    ...
    $('#form-approval').find('[name=danposko_status]').trigger('change');
});

$('#form-approval').find('[name=danposko_status]').on('change', (e)=>{
    switch (e.target.value) {
        case '1':
            $('#form-approval').find('.div-keterangan').hide();
            $('#form-approval').find('[name=danposko_keterangan]').prop('required',false);
            break;
        default:
            $('#form-approval').find('.div-keterangan').show();
            $('#form-approval').find('[name=danposko_keterangan]').prop('required',true);
            break;
    }
})
...
const do_submit = async()=>{
    ...
    $.ajax({
        type: "POST",
        url: `${BASE_URL}spbe/pemeriksaan_keluar_danposko/simpan_approval`,
        data: data,
    }).then( (response)=>{
