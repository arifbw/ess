const show_detail = async(e)=>{
    let data = await get_detail_spbi(e.id);
    $('#modal-detail').modal('show');
    for (const [key,value] of Object.entries(data)) {
        if(['barang','pos_keluar','pos_masuk','approval_atasan_status','pengecek1_status','konfirmasi_pengguna','danposko_status','approval_pengamanan_posisi','konfirmasi_pembawa_status'].includes(key)===false) $('#modal-detail').find(`.${key}`).text(value);
    }
...
const get_detail_spbi = async(id)=>{
    let req = await $.ajax({
        type: "GET",
        url: `${BASE_URL}spbi/spbi_proses/detail_spbi/${id}`,
        data: {},
    });
    return req;
}
