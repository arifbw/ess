...
function table_serverside() {
    ...
    table_spbi = $('#table-data').DataTable({ 
        destroy: true,
        iDisplayLength: 10,
        language: {
        ...
        ordering: false,
        ajax: {
            url: `${BASE_URL}spbi/pemeriksaan_keluar_admin_pamsiknilmat/get_data_persetujuan`,
            type: "POST",
                ...
                render: function ( data, type, row, meta ) {
                    ...
                    if( row.approval_pengamanan_keluar==null ){
                        let batal = $('<button/>', {
                            html: 'Batalkan',
                            class: 'btn btn-danger',
                            onclick: `action_batal(${JSON.stringify(row)})`
                        });
                        btn += batal.prop('outerHTML');
                    }
...
const get_mst_pos = async()=>{
    ...
    let req = await $.ajax({
        type: "GET",
        url: `${BASE_URL}spbi/spbi_proses/mst_pos`,
        data: {}
    });
...
const set_option_pos = async()=>{
    let pos = ref_mst_pos; // await list_pos_by_np();
...
// batal
const action_batal = async(data)=>{
    Swal.fire({
        title: 'Konfirmasi',
        html: `Anda akan membatalkan permohonan dengan nomor: <br><b>${data.nomor_surat}</b>?`,
        icon: 'warning',
...
const do_batalkan = async(data)=>{
    $.ajax({
        type: "POST",
        url: `${BASE_URL}spbi/spbi_proses/batalkan`,
        data: { id: data.id },
    }).then( (response)=>{
