...
const do_submit = async()=>{
    ...
    for (let i = 0; i < $('#modal-approval').find(`.barang`).length; i++) {
        ...
        const row = {
            ess_permohonan_spbi_barang_id: groupInput.find('.ess_permohonan_spbi_barang_id').val(),
    ...
    $.ajax({
        type: "POST",
        url: `${BASE_URL}spbi/pengecekan_masuk_petugas_pamsiknilmat/simpan_approval`,
        data: data,
    }).then( (response)=>{
...
const get_data_barang = async(spbi_id)=>{
    data_barang_temp = [];
    let req = await $.ajax({
        type: "GET",
        url: `${BASE_URL}spbi/spbi_proses/get_data_barang`,
        data: { spbi_id: spbi_id },
    });
    ...
    for (const i of data_barang_temp) {
        barang_text += `
            <tr>
                ...
                <td>${i.keterangan}</td>
                <td>
                    <input class="ess_permohonan_spbi_barang_id" type="hidden" value="${i.id}">
