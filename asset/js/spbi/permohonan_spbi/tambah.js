...
const get_atasan = async()=>{
    ...
    await $.ajax({
        type: "POST",
        url: `${BASE_URL}spbi/permohonan_spbi/get_atasan`,
...
const get_all_karyawan = async()=>{
    ...
    let temp = await $.ajax({
        type: "POST",
        url: `${BASE_URL}spbi/spbi_proses/all_karyawan`,
        data: {},
    });
    Swal.close();
    return temp;
...
const collect_data = async()=>{
    ...
    for (const i of $('#formulir_tambah').find('.input-detail')) {
        if( i.name!=='pilih_pengawal' ) data[i.name] = i.value;
    }
...
$("#btn-cancel").on('click', function(e){
    Swal.fire({
        ...
        if (result.isConfirmed) {
            window.location = `${BASE_URL}spbi/permohonan_spbi`;
        }
    })
});
...
const submit_process = async()=>{
    ...
    $.ajax({
        type: "POST",
        url: `${BASE_URL}spbi/permohonan_spbi/simpan`,
        data: data,
    }).then( (response)=>{
